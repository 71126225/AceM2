...
begin
  if boClose then
    Exit;
  if Application.MessageBox('Are you sure you want to close the Login gate?',
      'Exit',
      ...
      if UserSession.Socket <> nil then begin
        if not UserSession.boConnectCheck then begin
          if (GetTickCount - UserSession.dwUserTimeOutTick) >
            ...
            MainOutMessage('Null Connection:  ' + UserSession.sRemoteIPaddr, 3);
            {UserSession.Socket.Close;
            UserSession.Socket := nil;
            UserSession.SocketHandle := -1;
            UserSession.MsgList.Clear; }
          ...
          if (GetTickCount - UserSession.dwUserTimeOutTick) >
            ...
            MainOutMessage('Connect Timeout:  ' + UserSession.sRemoteIPaddr, 3);
            UserSession.Socket.Close;
            UserSession.Socket := nil;
            UserSession.SocketHandle := -1;
            //UserSession.MsgList.Clear;
...
begin
  ...
  while (sMsg <> '') do begin
    ...
    if not UserSession.boConnectCheck then begin
      if sCode <> '+' then begin
        sMsg := '';
        MainOutMessage('Illegal Connection: ' + UserSession.sRemoteIPaddr, 3);
        AttackDispose(UserSession.sRemoteIPaddr, UserSession.Socket);
        Break;
      end
      else begin
...
begin
  try
    MainOutMessage('Login Gate Starting...', 0);
    ...
    MainOutMessage('LoginGate started successfully', 0);
...
var
  ...
  //IPList: TList;
begin
  MainOutMessage('Login Gate stopping...', 3);
  ...
  MainOutMessage('Login Gate Stopped...', 3);
end;
...
begin
  if Application.MessageBox('Are you sure you want to stop Login Gate？', 'Confirmation', MB_YESNO + MB_ICONQUESTION) = IDYES then
    StopService();
end;
...
begin
  if Application.MessageBox('Are you sure you want to clear the log？', 'Confirmation', MB_OKCANCEL + MB_ICONQUESTION) <> IDOK then
    Exit;
  MemoLog.Clear;
end;
...
begin
  ...
  if not boSend then begin
    MainOutMessage('Illegal Packets: ' + UserSession.sRemoteIPaddr, 3);
    AttackDispose(UserSession.sRemoteIPaddr, UserSession.Socket);
  end
  else
