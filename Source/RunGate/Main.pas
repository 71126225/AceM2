  ...
  try
    ...
    if not boDecodeMsgLock then begin
      ...
      try
        if (GetTickCount - dwRefConsoleMsgTick) >= 1000 then begin
          ...
          if not boShowBite then begin
            LabelReviceMsgSize.Caption := 'Recieve: ' + IntToStr(nReviceMsgSize div 1024);
            ...
            LabelReviceMsgSize.Caption := 'Recieve: ' + IntToStr(nReviceMsgSize);
          ...
          LabelSelfCheck.Caption := 'Communi Self-Test: ' + IntToStr(dwCheckServerTimeMin) + '/' + IntToStr(dwCheckServerTimeMax);
      ...
      if (GetTickCount - dwRefConsolMsgTick) > 1000 then begin
        ...
        LabelSendLimitTime.Caption := 'Send processing limit: ' + IntToStr(dwProcessSendMsgTimeLimit);
        LabelReceTime.Caption := 'Recieve: ' + IntToStr(dwProcessClientMsgTime);
  ...
  try
    ...
    if (UserData.nSocketIdx >= 0) and (UserData.nSocketIdx < RUNATEMAXSESSION) then begin
      if (UserData.nSocketHandle = SessionArray[UserData.nSocketIdx].nSckHandle) and
              ...
              if (nDataLen >= DEFBLOCKSIZE) then begin
                if SessionArray[UserData.nSocketIdx].boStartLogon then begin
                  ...
                  if (GetCertification(sData, sAccount, sChrName, nSessionID, nClientVersion, boFlag)) and
                    ...
                    AddMainLogMsg('[Global Validation Fails]：' + SessionArray[UserData.nSocketIdx].sRemoteAddr, 2);
                    SessionArray[UserData.nSocketIdx].sSocData := '';
        ...
        AddMainLogMsg('Packet Attacks: ' + SessionArray[UserData.nSocketIdx].sRemoteAddr + ' ' +
...
begin
  if Application.MessageBox('Are you sure you want to stop this gate?',
    'Stop Gate', MB_YESNO + MB_ICONQUESTION) = IDYES then
    StopService();
end;
...
begin
  ...
  if boGateReady and boCenterReady then begin
    ...
    if Socket.nIndex > -1 then begin
      ...
      //Socket.nIndex := nSockIdx;
      AddMainLogMsg('Begin to connect: ' + sRemoteAddress, 5);
    end
    else begin
      Socket.nIndex := -1;
      Socket.Close;
    ...
    Socket.nIndex := -1;
    Socket.Close;
    AddMainLogMsg('Disable the connection: ' + sRemoteAddress, 3);
  end;
end;
      ...
      if nReviceLen > nNomClientPacketSize then begin
        ...
        if (nMsgCount > nMaxClientMsgCount){ or (nReviceLen > nMaxClientPacketSize)} then begin
          if bokickOverPacketSize then begin
            AddMainLogMsg('Kicked connection: IP(' + sRemoteAddress + '), Number of messages(' + IntToStr(nMsgCount) + ')', 1);
    ...
    for i := 0 to RUNATEMAXSESSION - 1 do begin
      ...
      if UserSession.Socket <> nil then begin
        if UserSession.boStartLogon and ((GetTickCount - UserSession.dwSendClientCheckTick) > dwClientTimeOutTime) then begin
          ...
          UserSession.Socket := nil;
          UserSession.nSckHandle := -1;  }
          AddMainLogMsg('Null Connection:  ' + UserSession.sRemoteAddr, 3);
...
begin
  if not boGateReady then
    AddMainLogMsg('Connected to(' + Socket.RemoteAddress + ':' + IntToStr(Socket.RemotePort) + ')...', 0);
  boGateReady := True;
  dwCheckServerTick := GetTickCount();
  RestSessionArray();
  boServerReady := True;
...
begin
  if Application.MessageBox('Are you sure you want to reload configuration?',
    'Reload Configuration', MB_OKCANCEL + MB_ICONQUESTION) <> IDOK
    then
    Exit;
  LoadConfig();
...
begin
  if boOpen then begin
    ...
    AddMainLogMsg('CC attack is detected, Is on defense...', 0);
  end else begin
    ...
    AddMainLogMsg('The CC defense has shut down, Total intercept '+ IntToStr(nCCHeadOffCount) +' Attacks...', 0);
  end;
end;
