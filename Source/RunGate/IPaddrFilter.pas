...
begin
  sIPaddress := '';
  if not InputQuery('Filter IP segment information', 'Please enter the starting IP address: ', sIPaddress) then Exit;
  nBeginaddr := inet_addr(PChar(sIPaddress));
  if nBeginaddr = INADDR_NONE then begin
    Application.MessageBox('Incorrect address format input!', 'Error', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
  if not InputQuery('Filter IP segment information', 'Please enter the end of the IP address: ', sIPaddress) then Exit;
  ...
  if LongWord(nEndaddr) >= LongWord(nBeginaddr) then begin
    ...
    Application.MessageBox('The end of the address can not be less than the start address', 'Message', MB_OK +
      MB_ICONINFORMATION);
end;
...
begin
  if (ListBoxActiveList.ItemIndex >= 0) and (ListBoxActiveList.ItemIndex <
    ...
    if
      Application.MessageBox(PChar('Are you sure this IP added dynamic filter list? Your filter list, all connections to this IP will be forced to interrupt.'),
      PChar('Confirmation - ' +
...
begin
  if (ListBoxActiveList.ItemIndex >= 0) and (ListBoxActiveList.ItemIndex <
    ...
    sIPaddr := ListBoxActiveList.Items.Strings[ListBoxActiveList.ItemIndex];
    if
      Application.MessageBox(PChar('Are you sure this IP added to the permanent filter list? Your filter list, all connections to this IP will be forced to interrupt.'),
      PChar('Confirmation - ' +
      ListBoxActiveList.Items.Strings[ListBoxActiveList.ItemIndex]),
      MB_OKCANCEL + MB_ICONQUESTION
      ) <> IDOK then
...
begin
  if (ListBoxActiveList.ItemIndex >= 0) and (ListBoxActiveList.ItemIndex <
    ListBoxActiveList.Items.Count) then begin
    if Application.MessageBox(PChar('Are you sure this is disconnected?'),
      PChar('Confirmation - ' +
...
begin
  ...
  if not IsIPaddr(sIPaddress) then begin
    Application.MessageBox('Enter the address format error!!!',
      'Error', MB_OK +
      MB_ICONERROR);
    Exit;
  end;
...
begin
  ...
  if not IsIPaddr(sIPaddress) then begin
    if
      Application.MessageBox('Input address format is incomplete, Whether to add?',
      'Error',
