...
begin
  sIPaddress := '';
  if not InputQuery('Enter IP','Please enter the IP adress you wish to temporarily ban: ', sIPaddress) then
    Exit;
...
begin
  sIPaddress := '';
  if not InputQuery('Enter IP','Please enter the IP adress you wish to permanently ban: ',
    sIPaddress) then
    Exit;
  if not IsIPaddr(sIPaddress) then begin
    if
      Application.MessageBox('Are you sure you want to permanantly block this IP?','Ban IP',
