...
begin
  ...
  if CheckBoxIP2.Checked and (sExtIPAddr2 = '') or not IsIPaddr(sExtIPAddr2) then begin
    Application.MessageBox('IP Address 2 is invalid', 'Error', MB_OK + MB_ICONEXCLAMATION);
...
begin
  ...
  if (nPort1 < 0) or (nPort1 > 65535) then begin
    Application.MessageBox('Port1 entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
    EditRunGate_GatePort1.SetFocus;
    Exit;
  end;
  if (nPort2 < 0) or (nPort2 > 65535) then begin
    Application.MessageBox('Port2 entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
    EditRunGate_GatePort2.SetFocus;
    Exit;
  end;
  if (nPort3 < 0) or (nPort3 > 65535) then begin
    Application.MessageBox('Port3 entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
    EditRunGate_GatePort3.SetFocus;
    Exit;
  end;
  if (nPort4 < 0) or (nPort4 > 65535) then begin
    Application.MessageBox('Port4 entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
    EditRunGate_GatePort4.SetFocus;
    Exit;
  end;
  if (nPort5 < 0) or (nPort5 > 65535) then begin
    Application.MessageBox('Port5 entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
    EditRunGate_GatePort5.SetFocus;
    Exit;
  end;
  if (nPort6 < 0) or (nPort6 > 65535) then begin
    Application.MessageBox('Port6 entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
    EditRunGate_GatePort6.SetFocus;
    Exit;
  end;
  if (nPort7 < 0) or (nPort7 > 65535) then begin
    Application.MessageBox('Port7 entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
    EditRunGate_GatePort7.SetFocus;
    Exit;
  end;
  if (nPort8 < 0) or (nPort8 > 65535) then begin
    Application.MessageBox('Port8 entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
...
begin
  ...
  if (nGatePort < 0) or (nGatePort > 65535) then begin
    Application.MessageBox('Gate Port entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
    EditLoginServerGatePort.SetFocus;
    Exit;
  end;
  if (nServerPort < 0) or (nServerPort > 65535) then begin
    Application.MessageBox('Server Port entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
    EditLoginServerServerPort.SetFocus;
    Exit;
  end;
  if (nMonPort < 0) or (nMonPort > 65535) then begin
    Application.MessageBox('Mon Port entered incorrectly', 'Error', MB_OK + MB_ICONERROR);
    EditLoginServerServerPort.SetFocus;
    Exit;
  end;
    ...
    if (DataListInfo <> nil) and (DataListInfo.Data <> nil) and (DataListInfo.DataSize > 0) then begin
      ...
      if dlgSave.Execute(Handle) then begin
        if FileExists(dlgSave.FileName) then begin
          if Application.MessageBox('File already exist，Overwrite？', 'Message',
            MB_OKCANCEL + MB_ICONQUESTION + MB_DEFBUTTON2) = IDCANCEL then
          begin
            Exit;
          end;
          if not DeleteFile(dlgSave.FileName) then begin
            Application.MessageBox('Filed to delete the file！', 'Error', MB_OK + MB_ICONSTOP);
            Exit;
          end;
        end;
        ...
        Try
          if not FileStream.Write(DataListInfo.Data^, DataListInfo.DataSize) = DataListInfo.DataSize then begin
            Application.MessageBox('Failed to save the file', 'Error', MB_OK + MB_ICONSTOP);
          end else
            Application.MessageBox('Done！', 'Message', MB_OK + MB_ICONINFORMATION);
        Finally
          FileStream.Free;
        End;
      end;
  ...
  for I := 0 to g_GetDatList.Count - 1 do begin
    ...
    if (sFileName <> '') and (sFileTime <> '') then begin
      ...
      if Item = nil then begin
        ...
        Item.SubItems.Add('0');
        Item.SubItems.Add('Not Updated');
        DataListInfo.Item := Item;
      end else begin
        if StrToDateTimeDef(Item.SubItems[0], 0) < StrToDateTimeDef(sFileTime, 0) then begin
          Item.SubItems[2] := 'New Version';
        end;
      end;
    end;
  end;
  g_GetDatList.Clear;
  GroupBox21.Caption := Format('Data List(%d)', [ListViewDataList.Items.Count]);
  MainOutDataListMessage('List Updated...');
...
begin
  ...
  if (g_GetDatList.Count > 0) then begin
    if g_GetDatList.Objects[0] <> nil then begin
      ...
      Item.SubItems.Strings[2] := 'Updating';
      MainOutDataListMessage('Updating[' + DataListInfo.sFileName + ']...');
      ...
      try
        ...
        while CSocket.Active and (CSocket.Socket.SendBuf(SendBuffer^, SizeOf(TDefMessage) + nMsgLen) = -1) do begin
          ...
          if nSendCount > 10 then begin
            MainOutDataListMessage('Failed to send[' + sData + ']...');
            exit;
          end;
    ...
    MainOutDataListMessage('Data Updated Successfully...');
  end;
end;
...
begin
  ...
  MainOutDataListMessage('Connected to Server Successfully...');
  MainOutDataListMessage('Verifying the connection password...');
  if not SendDataListData(SEC_CHECKPASS, g_sDataListPassWord) then begin
    MainOutDataListMessage('Failed to send verification connection password...');
    Socket.Close;
  end;
end;
...
begin
  MainOutDataListMessage('Connected to the server...');
  btDataListAll.Enabled := False;
  btDataListSelect.Enabled := False;
  btDataListList.Enabled := False;
end;
...
begin
  ErrorCode := 0;
  Socket.Close;
  MainOutDataListMessage('Failed to connect to the server...');
end;
      ...
      if DefMessage.Recog = MSGHEADCODE then begin
        ...
        case DefMessage.Ident of
          SES_CHECKPASS_OK: begin
              if g_GetDatList.Count = 0 then begin
                MainOutDataListMessage('Retrieving data list...');
          ...
          SES_CHECKPASS_FAIR: begin
              MainOutDataListMessage('Password authentication fails...');
              Socket.Close;
              Exit;
            end;
          SES_FILELIST: begin
              if DefMessage.DataSize > 0 then begin
                ...
                if (g_GetDatList.Count > 0) then begin
                  ...
                  MainOutDataListMessage('No data lists can be updated...');
                  Socket.Close;
                  Exit;
                end;
              end else begin
          ...
          SES_FILE: begin
              if (g_GetDatList.Count > 0) and (Integer(g_GetDatList.Objects[0]) = DefMessage.Param) then begin
                ...
                if (DefMessage.DataSize > 0) and (nLen >= DefMessage.DataSize) then begin
                  ...
                  Item.SubItems.Strings[0] := FormatDateTime('yyyy-mm-dd hh:mm:ss', DefMessage.DataTime);
                  Item.SubItems.Strings[1] := IntToStr(DefMessage.DataSize);
                  Item.SubItems.Strings[2] := 'Complete';
                end else begin
                  if (DefMessage.DataTime > 0) and (nLen >= DefMessage.DataSize) then begin
                    ...
                    MainOutDataListMessage('Update File[' + g_GetDatList[0] + ']Failed...');
                    Item.SubItems.Strings[2] := 'Failure';
                  end;
                end;
                g_GetDatList.Delete(0);
...
begin
  ...
  MainOutMessage('Shutting server down...');
...
begin
  if LoginGate.boGetStart and (LoginGate.btStartStatus in [2, 3]) then begin
    ...
    if dwExitCode = STILL_ACTIVE then begin
      ...
      MainOutMessage('LoginGate was shutdown...');
    end;
  end;
    ...
    if dwExitCode = STILL_ACTIVE then begin
      if LoginGate2.btStartStatus = 3 then begin
        if GetTickCount - g_dwStopTick > g_dwStopTimeOut then begin
          ...
          MainOutMessage('LoginGate2 has been forced to stop...');
        end;
        Exit; //如果正在关闭则等待，不处理下面
      end;
      ...
      MainOutMessage('LoginGate2 was shutdown...');
    end;
  end;
  ...
  if SelGate.boGetStart and (SelGate.btStartStatus in [2, 3]) then begin
    ...
    if dwExitCode = STILL_ACTIVE then begin
      ...
      MainOutMessage('SelGate was shutdown');
    end;
  end;
  ...
  if SelGate1.boGetStart and (SelGate1.btStartStatus in [2, 3]) then begin
    ...
    if dwExitCode = STILL_ACTIVE then begin
      ...
      MainOutMessage('SelGate1 was shutdown');
    end;
  end;
  ...
  for I := Low(RunGate) to High(RunGate) do begin
    if RunGate[I].boGetStart and (RunGate[I].btStartStatus in [2, 3]) then begin
      ...
      if dwExitCode = STILL_ACTIVE then begin
        ...
        RunGate[I].btStartStatus := 0;
        case I of
          0: MainOutMessage('RunGate was shutdown');
  ...
  if M2Server.boGetStart and (M2Server.btStartStatus in [2, 3]) then begin
    ...
    if dwExitCode = STILL_ACTIVE then begin
      ...
      MainOutMessage('M2Server was shutdown...');
    end;
  end;
    ...
    if dwExitCode = STILL_ACTIVE then begin
      if LoginServer.btStartStatus = 3 then begin
        if GetTickCount - g_dwStopTick > g_dwStopTimeOut then begin
          ...
          MainOutMessage('LoginSrv timeout...');
        end;
        Exit; //如果正在关闭则等待，不处理下面
      end;
      ...
      MainOutMessage('LoginSrv was shutdown...');
    end;
  end;
  ...
  if LogServer.boGetStart and (LogServer.btStartStatus in [2, 3]) then begin
    ...
    if dwExitCode = STILL_ACTIVE then begin
      ...
      MainOutMessage('Log Server was shutdown...');
    end;
  end;
  ...
  if DBServer.boGetStart and (DBServer.btStartStatus in [2, 3]) then begin
    ...
    if dwExitCode = STILL_ACTIVE then begin
      ...
      MainOutMessage('DBServer was shutdown...');
    end;
  end;
  TimerStopGame.Enabled := False;
    ...
    if (dwExitCode <> STILL_ACTIVE) or (DBServer.ProcessHandle = 0) then begin
      ...
      if nRetCode = 0 then begin
        ...
        if DBServer.MainFormHandle <> 0 then
          MainOutMessage('DBServer was abnormally shut down and has been restarted...');
    ...
    if (dwExitCode <> STILL_ACTIVE) or (LoginServer.ProcessHandle = 0) then begin
      ...
      if nRetCode = 0 then begin
        ...
        if LoginServer.MainFormHandle <> 0 then
          MainOutMessage('LoginSrv was abnormally shut down and has been restarted...');
    ...
    if (dwExitCode <> STILL_ACTIVE) or (LogServer.ProcessHandle = 0) then begin
      ...
      if nRetCode = 0 then begin
        ...
        if LogServer.MainFormHandle <> 0 then
          MainOutMessage('Log server was abnormally shut down and has been restarted...');
    ...
    if (dwExitCode <> STILL_ACTIVE) or (M2Server.ProcessHandle = 0) then begin
      ...
      if nRetCode = 0 then begin
        ...
        if M2Server.MainFormHandle <> 0 then
          MainOutMessage('M2Server was abnormally shut down and has been restarted...');
      ...
      if (dwExitCode <> STILL_ACTIVE) or (RunGate[I].ProcessHandle = 0) then begin
        ...
        if nRetCode = 0 then begin
          ...
          if RunGate[I].MainFormHandle <> 0 then begin
            case I of
              0: MainOutMessage('RunGate was abnormally shut down and has been restarted...');
    ...
    if (dwExitCode <> STILL_ACTIVE) or (SelGate.ProcessHandle = 0) then begin
      ...
      if nRetCode = 0 then begin
        ...
        if SelGate.MainFormHandle <> 0 then
          MainOutMessage('SelGate was abnormally shut down and has been restarted...');
    ...
    if (dwExitCode <> STILL_ACTIVE) or (SelGate1.ProcessHandle = 0) then begin
      ...
      if nRetCode = 0 then begin
        ...
        if SelGate1.MainFormHandle <> 0 then
          MainOutMessage('SelGate1 was abnormally shut down and has been restarted...');
    ...
    if (dwExitCode <> STILL_ACTIVE) or (LoginGate.ProcessHandle = 0) then begin
      ...
      if nRetCode = 0 then begin
        ...
        if LoginGate.MainFormHandle <> 0 then
          MainOutMessage('LoginGate was abnormally shut down and has been restarted...');
    ...
    if (dwExitCode <> STILL_ACTIVE) or (LoginGate2.ProcessHandle = 0) then begin
      ...
      if nRetCode = 0 then begin
        ...
        if LoginGate2.MainFormHandle <> 0 then
          MainOutMessage('LoginGate2 was abnormally shut down and has been restarted...');
    ...
    case wIdent of
      ...
      SG_STARTNOW: begin
          if LoginGate.MainFormHandle = Handle then begin
            MainOutMessage('LoginGate Starting...');
          end else
          if LoginGate2.MainFormHandle = Handle then begin
            MainOutMessage('LoginGate2 Starting....');
          end;
          //MainOutMessage(sData);
        end;
      SG_STARTOK: begin
          if LoginGate.MainFormHandle = Handle then begin
            LoginGate.btStartStatus := 2;
            MainOutMessage('LoginGate Started Successfully...');

          end else
          if LoginGate2.MainFormHandle = Handle then begin
            LoginGate2.btStartStatus := 2;
            MainOutMessage('LoginGate2 Started Successfully...');
          end;
          //LoginGate.btStartStatus := 2;
          //MainOutMessage(sData);
        end;
...
begin
  ...
  if Handle <> 0 then begin
    case wIdent of
      ...
      SG_STARTNOW: begin
          if SelGate.MainFormHandle = Handle then begin
            MainOutMessage('SelGate Starting...');
          end else
          if SelGate1.MainFormHandle = Handle then begin
            MainOutMessage('SelGate1 Starting...');
          end;
        end;
      SG_STARTOK: begin
          if SelGate.MainFormHandle = Handle then begin
            SelGate.btStartStatus := 2;
            MainOutMessage('SelGate Started Successfully...');

          end else
          if SelGate1.MainFormHandle = Handle then begin
            SelGate1.btStartStatus := 2;
            MainOutMessage('SelGate1 Started Successfully...');
          end;
        end;
    end;
  end;
      ...
      SG_STARTNOW: begin
          for I := Low(RunGate) to High(RunGate) do begin
            if RunGate[I].MainFormHandle = Handle then begin
              case I of
                0: MainOutMessage('Starting RunGate');
      ...
      SG_STARTOK: begin
          for I := Low(RunGate) to High(RunGate) do begin
            if RunGate[I].MainFormHandle = Handle then begin
              ...
              case I of
                 0: MainOutMessage('RunGate started successfully');
...
begin
  if m_nStartStatus = 2 then begin
    if
      Application.MessageBox('Server is still running, do you want to also close the Server?', 'Message', MB_YESNO +
  ...
  if Application.MessageBox('Close GameCenter?', 'Message',
...
begin
  ...
  Timer.Enabled := True;
  MainOutMessage('GameCenter Started...');
  Result := True;
end;
{
...
begin
  ...
  Application.MessageBox('Save succesful', 'Message', MB_OK +
    MB_ICONINFORMATION);
...
var
  sPort: string;
begin
  g_sDataListAddrs := InputBox('Enter Information', 'Please Enter the IP', g_sDataListAddrs);
  sPort := InputBox('Enter Information', 'Please Enter the Port', IntToStr(g_wDataListPort));
  g_wDataListPort := StrToIntDef(sPort, g_wDataListPort);
  g_sDataListPassWord := InputBox('Enter Information', 'Please Enter the Connection password', g_sDataListPassWord);
  CSocket.Active := False;
  CSocket.Host := g_sDataListAddrs;
  CSocket.Port := g_wDataListPort;
  CSocket.Active := True;
...
begin
  if m_nStartStatus = 0 then begin
    ...
    End;
    Application.MessageBox('Clean-Up Completed！', 'Message', MB_OK + MB_ICONINFORMATION);
  end else
    Application.MessageBox('Place the server in a stopped state before Cleaning！', 'Message', MB_OK + MB_ICONWARNING);
end;
...
begin
  ...
  if sSource = '' then begin
    Application.MessageBox('Please Select the Data Directory！！！', 'Message',
      MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
  if sDest = '' then begin
    Application.MessageBox('Please Select the Backup Directory！！！', 'Message',
      MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
  if g_BackUpManager.FindObject(sSource) <> nil then begin
    Application.MessageBox('This Data Directory already exist！！！',
...
begin
  ...
  if ListItem <> nil then begin
    if g_BackUpManager.DeleteObject(ListItem.Caption) then begin
      RefBackListToView();
      Application.MessageBox('Deleted Successfully！！！', 'Message', MB_OK +
        MB_ICONINFORMATION);
    end
    else
      Application.MessageBox('Delete Failed！！！', 'Message', MB_OK +
        MB_ICONERROR);
  end;
end;
...
begin
  ...
  if ListItem <> nil then begin
    ...
    BackUpObject.m_boZip := CheckBoxZip.Checked;
    Application.MessageBox('Modified Successfully！！！', 'Message', MB_OK +
      MB_ICONINFORMATION);
  end;
end;
...
begin
  ...
  if SelectDirectory('Please Select the folder of files you want to backup', '', NewDir, Handle) then begin
...
begin
  ...
  if SelectDirectory('Please Select a backup folder', '', NewDir, Handle) then begin
