...
begin
  ...
  try
    ...
    if (EndInt >= g_WMImages.ImageCount) then begin
      Application.MessageBox('Ending Number Error，不能大于总数据数量', 'Error', MB_OK or MB_ICONASTERISK);
      exit;
    end;
    if (StartInt > EndInt) then begin
      Application.MessageBox('Starting Number Error，不能大于结号编号', 'Error', MB_OK or MB_ICONASTERISK);
      exit;
    end;
