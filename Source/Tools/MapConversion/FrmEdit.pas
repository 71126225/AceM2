...
begin
  if seEnd.Value < seBegin.Value then begin
    Application.MessageBox('End Number is smaller than Start Number!', 'Error', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
...
begin
  if Application.MessageBox('Are you sure you reload the log file configuration£¡', 'Confirmation', MB_YESNO + MB_ICONQUESTION) = IDYES then
  begin
    if LoadWriteImageInfo(FFileName) then begin
      AddWriteImageToNode;
    end else
      Application.MessageBox('Failed to reload the Log File!', 'Error', MB_OK + MB_ICONSTOP);
  end;
end;

procedure TFormEdit.btnSaveToFileClick(Sender: TObject);
begin
  if Application.MessageBox('Are you sure you save the settings, The operation will not be restored, Recommanded take Backup before the operation.', 'Confirmation', MB_YESNO + MB_ICONQUESTION) = IDYES then
  begin
    SaveWriteImageInfo(FFileName);
    Application.MessageBox('Save the setting Successfully', 'Message', MB_OK + MB_ICONINFORMATION);
  end;
end;
...
begin
  ...
  if LoadWriteImageInfo(sFileName) then begin
    ...
    Application.MessageBox('Failed to load the log file!', 'Error', MB_OK + MB_ICONSTOP);
end;
