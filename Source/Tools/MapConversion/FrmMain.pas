...
begin
  ...
  lblOpenFileHint.Caption := 'Call the image data being analyzed...';
  AnalyseImagesInfo();
end;
...
begin
  ...
  LoadFileLock(True);
  Try
    WriteLog('Verifying data completed...');
    nMaxCount := 0;
    nNowCount := 0;
    pb1.Position := 0;
    if FOutCount <= 0 then begin
      Application.MessageBox('Export Quantity must be more than 0', 'Error', MB_OK + MB_ICONINFORMATION);
      Exit;
    end;
    if FInCount < FOutCount then begin
      Application.MessageBox('Import Quantity must be more than Export Quantity', 'Error', MB_OK + MB_ICONINFORMATION);
      Exit;
    end;
    if (FWMImages = nil) or (not FWMImages.boInitialize) then begin
      Application.MessageBox('Failed to open the file you want to import', 'Error', MB_OK + MB_ICONINFORMATION);
      Exit;
    end;
    ...
    Try
            ...
            if (nImage >= 0) and (nImage <= $7FFF) then begin
              if LoadList[nIdx] = nil then begin
                if FileExists(edtOutData.Text + '\' + GetObjectsName(nIdx)) then begin
                  ...
                  Application.MessageBox(PChar('Open the file you want to export[' + GetObjectsName(nIdx) + ']Failure'), 'Error', MB_OK + MB_ICONINFORMATION);
                  Exit;
                end;
              end;
      ...
      for I := Low(LoadList) to High(LoadList) do begin
        if LoadList[I] <> nil then begin
          WriteLog('Data to a new file ' + GetObjectsName(I) + 'Being Imported...');
          if not WriteImageToNewFile(edtOutData.Text + '\' + GetObjectsName(I), LoadList[I], I, FInIndex, nMaxCount, nNowCount) then begin
            Application.MessageBox(PChar('Export [' + GetObjectsName(I) + '] File Failed'), 'Error', MB_OK + MB_ICONINFORMATION);
            Exit;
          end;
        end;
      end;
      pb1.Position := 100;
      WriteLog('New map file is being output...');
      WriteNewMapInfo('.\' + ExtractFileName(edtFileName.Text));

      WriteLog('Saving Import Data...');
      ...
      Application.MessageBox(PChar('Successfully converted the map file'), 'Message', MB_OK + MB_ICONINFORMATION);
    Finally
      for I := Low(LoadList) to High(LoadList) do begin
...
var
  NewDir: string;
begin
  NewDir := BrowseForFolder(Handle,'Please select the data directory');
...
begin
  ...
  if FileExists(sFileName) then begin
    ...
    try
      lblOpenFileHint.Caption := 'Map information is being read...';
      ...
      if (MapWidth <= 1000) and (MapHeight <= 1000) then begin
        ...
        lblOpenFileHint.Caption := 'Map file being analyzed...';
        AnalyseMapInfo;
        lblOpenFileHint.Caption := 'Analysis complete...';
      end
      else begin
        lblOpenFileHint.Caption := 'Failed to read the map information';
      end;
    finally
      aMapFile.Free;
      LoadFileLock(False);
    ...
    Application.MessageBox('Failed to open the file', 'Error', MB_OK + MB_ICONINFORMATION);
end;
...
begin
  ...
  if FileExists(sFileName) then begin
    ...
    Application.MessageBox('Failed to open the file£¬file does not exist', 'Error', MB_OK + MB_ICONWARNING);
  end;
end;
