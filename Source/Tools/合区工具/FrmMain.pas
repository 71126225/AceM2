...
begin
  ...
  begin
    Application.MessageBox('Please Enter the correct Text Address', 'Error', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
...
var
  i: Integer;
begin
  if Application.MessageBox('Are you sure you want to delete the selected Text Messages?', 'Confirmation', MB_OKCANCEL + MB_ICONQUESTION) <> IDOK then
    exit;
...
begin
  if Application.MessageBox('Are you sure you want to Empty the added merge Text info？', 'Confirmation', MB_OKCANCEL + MB_ICONQUESTION) <> IDOK then
    exit;
...
var
  FileName: string;
begin
  FileName := BrowseForFolder(Handle, 'Please Select the directory');
...
begin
  ...
  if not DirectoryExists(SaveDir) then begin
    Application.MessageBox('Output Folder Location incorrect！', 'Error', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
  ...
  CreateDir(SaveDir + 'Deleted\');

  CreateDir(SaveDir + 'Text\');
  
  DeleteFile(SaveDir + 'Mir.DB');
  DeleteFile(SaveDir + 'Hum.DB');
  DeleteFile(SaveDir + 'ID.DB');
  DeleteFile(SaveDir + 'Deleted\' + 'Main.Mir.DB');
  DeleteFile(SaveDir + 'Deleted\' + 'Main.Hum.DB');
  DeleteFile(SaveDir + 'Deleted\' + 'From.Mir.DB');
  DeleteFile(SaveDir + 'Deleted\' + 'From.Hum.DB');
  DeleteFile(SaveDir + 'Deleted\' + 'Main.ID.DB');
  DeleteFile(SaveDir + 'Deleted\' + 'From.ID.DB');
  ...
  DelMainMirDB := TFileStream.Create(SaveDir + 'Deleted\' + 'Main.Mir.DB', fmCreate);
  DelMainHumDB := TFileStream.Create(SaveDir + 'Deleted\' + 'Main.Hum.DB', fmCreate);
  DelSubMirDB := TFileStream.Create(SaveDir + 'Deleted\' + 'From.Mir.DB', fmCreate);
  DelSubHumDB := TFileStream.Create(SaveDir + 'Deleted\' + 'From.Hum.DB', fmCreate);
  DelMainIDDB := TFileStream.Create(SaveDir + 'Deleted\' + 'Main.ID.DB', fmCreate);
  DelSubIDDB := TFileStream.Create(SaveDir + 'Deleted\' + 'From.ID.DB', fmCreate);
  try
    if Trim(EditMainID.Text) <> '' then begin
      ShowHint('Loading Main ID.DB...');
      if not LoadIDDB(Trim(EditMainID.Text), @MainUnite, True, False) then begin
        ShowHint('Failed to Load Main ID.DB. The file does not exist or version does not match...', True);
        Exit;
      end;

      ShowHint('Loadin From ID.DB...');
      if not LoadIDDB(Trim(EditSubID.Text), @SubUnite, False, False) then begin
        ShowHint('Failed to Load From ID.DB. The file does not exist or version does not match...', True);
        Exit;
      end;
    end;

    ShowHint('Merging Guild...');
    if not UniteGuild(SaveDir + 'GuildBase\', @GuildRename) then begin
      ShowHint('Failed to Merge Guild Information...', True);
      Exit;
    end;

    ShowHint('Loading Main Hum.DB...');
    if not LoadHumDB(Trim(EditMainHum.Text), @MainUnite) then begin
      ShowHint('Failed to Load Main Hum.DB, The file does not exist or does not match the version...', True);
      Exit;
    end;
    
    ShowHint('Loading Main Mir.DB...');
    if not LoadMirDB(Trim(EditMainMir.Text), @MainUnite, True) then begin
      ShowHint('Failed to Load Main Mir.DB, The file does not exist or does not match the version...', True);
      Exit;
    end;

    ShowHint('Loading From Hum.DB...');
    if not LoadHumDB(Trim(EditSubHum.Text), @SubUnite) then begin
      ShowHint('Failed to Load From Hum.DB, The file does not exist or does not match the version...', True);
      Exit;
    end;
    
    ShowHint('Loading From Mir.DB...');
    if not LoadMirDB(Trim(EditSubMir.Text), @SubUnite, False) then begin
      ShowHint('Failed to Load From Mir.DB, The file does not exist or does not match the version...', True);
      Exit;
    end;
    ...
    UpdateTextFileList(SaveDir + 'Text\');
    ...
    MainUnite.DelMirDB.SaveToFile(CurrentDir + 'Main.DelHumLog.txt');
    SubUnite.DelMirDB.SaveToFile(CurrentDir + 'From.DelHumLog.txt');
    MainUnite.DelIDDB.SaveToFile(CurrentDir + 'Main.DelIDLog.txt');
    SubUnite.DelIDDB.SaveToFile(CurrentDir + 'From.DelIDLog.txt');
...
begin
  CanClose := False;
  if Application.MessageBox('Are you sure you want to close？', 'Message', MB_OKCANCEL + MB_ICONQUESTION) = IDOK then begin
    CanClose := True;
  end;
end;
                  ...
                  if not boMain then begin
                    if AllID.GetIndex(sNewName) <> -1 then begin
                      ...
                      if sNewName = '' then begin
                        Application.MessageBox('Failed to rename the Account！', 'Error', MB_OK + MB_ICONINFORMATION);
                        Exit;
                      end;
              ...
              if boDel then begin
                ...
                if boMain then begin
                  ...
                  if MainUnite.MirDB.GetIndex(HumRecord.Data.sChrName) <> -1 then begin
                    ...
                    if sName = '' then begin
                      Application.MessageBox('Failed to rename Character Name！', 'Error', MB_OK + MB_ICONINFORMATION);
                      Exit;
                    end;
    ...
    for I := 0 to SubList.Count - 1 do begin
      if FileExists(SubGuildDir + 'Guilds\' + SubList[I] + '.ini') then begin
        ...
        if MainList.IndexOf(sName) <> -1 then begin
          ...
          if sName = '' then begin
            Application.MessageBox('Failed to rename the name of Guild！', 'Error', MB_OK + MB_ICONINFORMATION);
            Exit;
          end;
          MainList.Add(sName);
