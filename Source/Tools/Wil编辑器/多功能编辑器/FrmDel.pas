...
begin
  ...
  Try
    ...
    if (EndInt >= g_WMImages.ImageCount) then begin
      Application.MessageBox('数据结束编号设置错误，不能大于总数据数量', 'Error', MB_OK or MB_ICONASTERISK);
      exit;
    end;
    if (StartInt > EndInt) then begin
      Application.MessageBox('Starting Number Error，Cant be more than the end number', 'Error', MB_OK or MB_ICONASTERISK);
      exit;
    end;
    ...
    if (StartPos > 0) and (EndPos > 0) and (EndPos >= StartPos) then begin
      ...
      Application.MessageBox('Delete Data is complete', 'Message', MB_OK or MB_ICONASTERISK);
      Close;
    end;
  Finally
