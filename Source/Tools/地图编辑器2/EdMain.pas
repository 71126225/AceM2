...
begin
  ...
  if Edited then begin
    r := MessageDlg('File Updated，Save?',
      mtWarning,
      mbYesNoCancel,
      0);
    if r = mrYes then
...
var
  ...
  n: Integer;
begin
  NewDir := BrowseForFolder(Handle, 'Select Data Directory');
  if NewDir <> '' then begin
    ...
    while n = 0 do begin
      if (not FileExists(SaveDir + sr.Name + '.bmp')) and LoadFromFile(NewDir + '\' + sr.Name) then begin
        ...
        Except
          Application.MessageBox(PChar('Save [' + sr.Name + '] Error！'), 'Error', MB_OK + MB_ICONINFORMATION);
        End;
      end;
      n := FindNext(sr);
    ...
    FindClose(sr);
    Application.MessageBox('Save all complete！', 'Message', MB_OK + MB_ICONINFORMATION);
    SaveAllMap := False;
  end;
end;
    ...
    if Execute and (FileName <> '') then begin
      ...
      for I := 0 to cbb1.Items.Count - 1 do begin
        ...
        if (nX > 0) and (nY > 0) and (nWidth > 0) and (nHeight > 0) and ((nX + nWidth) < MapWidth) and ((nY + nHeight) < MapHeight) then begin
          ...
          Application.MessageBox('Erroneous output data！', 'Error', MB_OK + MB_ICONSTOP);
          Exit;
        end;
