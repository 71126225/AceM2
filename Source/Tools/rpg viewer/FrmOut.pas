...
begin
  ...
  try
    ...
    if (StartInt < 0) then begin
      Application.MessageBox('Starting Number Error', 'Error', MB_OK or MB_ICONASTERISK);
      exit;
    end;
    if (EndInt < 0) then begin
      Application.MessageBox('Ending Number Error', 'Error', MB_OK or MB_ICONASTERISK);
      exit;
    end;
    if (EndInt >= g_WMImages.ImageCount) then begin
      Application.MessageBox('Ending Number Error, higher endnumber then file imagecount', 'Error', MB_OK or MB_ICONASTERISK);
      exit;
    end;
    if (StartInt > EndInt) then begin
      Application.MessageBox('Starting Number Error, higher startnumber then endnumber', 'Error', MB_OK or MB_ICONASTERISK);
      exit;
    end;
    ...
    StringList.Free;
    Application.MessageBox('Export Data is complete', 'Message', MB_OK or MB_ICONASTERISK);
    Close
  finally
...
var
  sStr: string;
begin
  sStr := BrowseForFolder(Handle, 'Please Select the Output folder');
  if sStr <> '' then begin
