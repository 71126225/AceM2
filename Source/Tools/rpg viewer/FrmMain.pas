...
uses
  ...
  ExtDlgs, Dialogs, ExtCtrls, TDX9Devices, ImgList, Menus, StdCtrls, TeCanvas,
  ComCtrls, ToolWin, Grids, TDX9Textures, MyDXBase,
...
type
  TFormMain = class(TForm)
    ...
    MEMU_FILE_OPEN_MIR3_wtl: TMenuItem;
    Rebuildtowzl1: TMenuItem;
    ...
    procedure MEMU_FILE_OPEN_MIR3_wtlClick(Sender: TObject);
    procedure FormResize(Sender: TObject);
    procedure Rebuildtowzl1Click(Sender: TObject);
  private
    FAutoTick: LongWord;
    FBlend: Cardinal;
    FImageX: Integer;
...
uses
  wmM2Def, wmM2Wis, wmMyImage, wmM2Zip, wmM3Zip, wmM3Wtl, Hutil32, wmM3Def, wmWoool, FrmAdd, FrmDel, FrmOut, FrmAlpha, Bass, Registry, MyCommon;
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := 'Mir2 Files(565)|*_16.wil';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := 'Mir2 Standard Files |*.wil';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := 'Mir2 Encrypted Files |*.wis';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := 'Mir2 Compressed Files |*.wzl';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := 'Mir3 Standard Files |*.wil';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := 'Mir3 Compressed Files |*.miz';
  Initializeg_WMImages(t_wmM3Zip);
end;

procedure TFormMain.MEMU_FILE_OPEN_MIR3_wtlClick(Sender: TObject);
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := 'Mir3 Standard Files |*.wtl';
  Initializeg_WMImages(t_wmM3Wtl);
end;

procedure TFormMain.MEMU_FILE_OPEN_WOOOLClick(Sender: TObject);
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := 'WOOOL Files |*.sgl';
...
//  FRenderSurface.Format := D3DFMT_DXT1;   //thedeath
...
begin
  if g_WMImages <> nil then begin
    StatusBar.Panels[0].Text := Format('No.: %d/%d', [g_SelectImageIndex, g_WMImages.ImageCount - 1]);
    ...
    if g_WMImages is TWMM2DefImages then begin
      if TWMM2DefImages(g_WMImages).bo16bit then
        StatusBar.Panels[1].Text := 'Format: MIR2 Standard File(16Bit)'
      else
      if TWMM2DefImages(g_WMImages).NewFmt then
        StatusBar.Panels[1].Text := 'Format: MIR2 Standard File(New)'
      else
        StatusBar.Panels[1].Text := 'Format: MIR2 Standard File(Old)';
    end
    else if g_WMImages is TWMM2DefBit16Images then
      StatusBar.Panels[1].Text := 'Format: MIR2 Specific File(16Bit)'
    else if g_WMImages is TWMM2ZipImages then
      StatusBar.Panels[1].Text := 'Format: MIR2 Compression File'
    else if g_WMImages is TWMM2WisImages then
      StatusBar.Panels[1].Text := 'Format: MIR2 Encryption File'
    else if g_WMImages is TWMWoolDefImages then
      StatusBar.Panels[1].Text := 'Format: WOOOL File'
    else if g_WMImages is TWMM3DefImages then begin
      if TWMM3DefImages(g_WMImages).NewFmt then
        StatusBar.Panels[1].Text := 'Format: MIR3 Standard File(New)'
      else
        StatusBar.Panels[1].Text := 'Format: MIR3 Standard File(Old)';
    end
    else if g_WMImages is TWMM3ZipImages then
      StatusBar.Panels[1].Text := 'Format: MIR3 Compression File'
    else if g_WMImages is TWMMyImageImages then
      StatusBar.Panels[1].Text := 'Format: Custom Image File'
    else if g_WMImages is TWMM3WtlImages then begin
      StatusBar.Panels[1].Text := TWMM3WtlImages(g_WMImages).GetLibType(TWMM3WtlImages(g_WMImages).FHeader.VerFlag)
    end;
  end
  else begin
    StatusBar.Panels[0].Text := 'No.: 0/0';
    StatusBar.Panels[1].Text := 'Format: Unknown';
  end;
  StatusBar.Panels[2].Text := Format('w * h: %d * %d', [g_TextureInfo.nWidth, g_TextureInfo.nHeight]);
  StatusBar.Panels[3].Text := Format('X: %d', [g_TextureInfo.px]);
  StatusBar.Panels[4].Text := Format('Y: %d', [g_TextureInfo.py]);
...
begin
  if (g_WMImages <> nil) and g_WMImages.boInitialize then begin
    SetDrawGridIndex(StrToIntDef(InputBox('Jump', 'Please enter a picture index number', IntToStr(g_SelectImageIndex)), 1));
  end;
end;
...
{$define thedeath}
procedure TFormMain.Rebuildtowzl1Click(Sender: TObject);
var
  fhandle: THandle;
  sFileName: string;
  WMImageHeader: wmM2Zip.TWZIndexHeader;
  i: integer;
  wmFile: TWMBaseImages;
  wmFilem2zip: TWMM2ZipImages;
  btType: Byte;
  bitmap:TBitmap;
  bo16bit:boolean;
begin
  {$IFDEF thedeath}
  SaveDialog.FileName := '';
  SaveDialog.Filter := 'shanda image library(*.wzl)|*.wzl';
  if SaveDialog.Execute(Handle) and (SaveDialog.FileName <> '') then begin
    sFileName := SaveDialog.FileName;
    //if CompareText(RightStr(sFileName, 4), MYFILEEXT) <> 0 then
    //  sFileName := sFileName + MYFILEEXT;
    ...
    if FileExists(sFileName) then begin
      if MessageBox(Handle, 'overwrite?' ,'File excists', MB_OKCANCEL + MB_ICONWARNING) =
        IDCANCEL then
        exit;
      DeleteFile(sFileName);
      DeleteFile(ExtractFilePath(sFileName) + ExtractFileNameOnly(sFileName) + '.WZX');
    end;
    //create the header
    fhandle := FileCreate(sFileName, fmOpenWrite);
    if fhandle > 0 then begin
    ...
    //save all the data
    wmFile := wil.CreateWMImages(t_wmM2Zip);
    if wmFile <> nil then begin
      wmFile.FileName := sFileName;
      wmFile.LibType := ltLoadBmp;
      //wmFile.ReadOnly := false;
      wmFile.Initialize;

      if (wmFile = nil) or  (wmFile.boInitialize = false) then begin
        MessageBox(Handle, 'Failed to initialize new file', 'Error',MB_OK);
        exit;
      end;
      wmFilem2Zip := TWMM2ZipImages(wmFile);

      for i := 0 to g_WMImages.ImageCount - 1 do begin
        try
          bitmap := g_WMImages.Bitmap[i,btType];
          bo16bit:=false;
          if bitmap <> nil then
            bo16bit:=bitmap.PixelFormat = pf16bit;
          wmFilem2zip.AddImage(bitmap, g_WMImages.LastImageInfo.px, g_WMImages.LastImageInfo.py,bo16bit);
          bitmap.Free;
        except
    ...
    wmFilem2zip.SaveIndex();
    MessageBox(Handle,'Finished Rebuilding wzl','Finished',MB_OK);
    end;
  end;
  {$endif}
end;
