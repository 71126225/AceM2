...
begin
  ...
  if NoneOption.Visible then begin
    ...
    if Image_Offset.Checked then
      Image_Offset_File.Caption := 'Use the file list'
    else
      Image_Offset_File.Caption := 'Coordinate file of the same name';
  ...
  GroupBox2.Caption := 'File List (' + IntToStr(File_List.Count) + ')';
...
var
  ...
  FileListInfo: pTFileListInfo;
begin
  sStr := BrowseForFolder(Handle, 'Please Select the folder');
  if sStr <> '' then begin
...
begin
  ...
  if Mode_Insert.Checked and (StartInt >= g_WMImages.ImageCount) then begin
    Application.MessageBox('Start Number Error', 'Error！', MB_OK or MB_ICONASTERISK);
    exit;
  end;
  if Mode_Bestrow.Checked and (EndInt >= g_WMImages.ImageCount) then begin
    Application.MessageBox('End Number Error', 'Error！', MB_OK or MB_ICONASTERISK);
    exit;
  end;
  ...
  if ImageStream.Checked and Image_Offset.Checked and Image_Offset_App.Checked then begin
    ...
    if (sX = '') or (sY = '') then begin
      Application.MessageBox('Coordinates not set correctly,Can not update!', 'Error!', MB_OK or MB_ICONASTERISK);
      exit;
    end;
    case sX[1] of
      '+': begin
    ...
    if Application.MessageBox('Complete,Exit?', 'Message', MB_OKCANCEL + MB_ICONQUESTION) = IDOK
      then begin
      Close;
    end;
    exit;
  ...
  if Mode_Bestrow.Checked and ((EndInt - StartInt) <> File_List.Items.Count - 1) then begin
    Application.MessageBox('The number of files and the number is not set to the same, Error!', 'Error', MB_OK or
      MB_ICONASTERISK);
    exit;
  end;
  ...
  if (File_List.Items.Count <= 0) then begin
    Application.MessageBox('请先选项要打包的文件！', 'Message', MB_OK + MB_ICONINFORMATION);
    exit;
  end;
    ...
    for I := 0 to File_List.Items.Count - 1 do begin
      ...
      if ImageStream.Checked then begin
        if Image_Offset.Checked then begin //更新坐标
          ...
          if Bitmap = nil then begin
            if Application.MessageBox(PChar(FileListInfo.FileName + #13#10 + 'Imcorrect Format，Continue？'),
              'Message', MB_OKCANCEL + MB_ICONWARNING + MB_DEFBUTTON2) = IDOK then begin
              if Mode_after.Checked then begin
          ...
          if (Bitmap.Width < MINIMAGESIZE) or (Bitmap.Height < MINIMAGESIZE) or (Bitmap.Width > MAXIMAGESIZE) or (Bitmap.Height > MAXIMAGESIZE) then begin
            ...
            //everything is ok and load the 'bmp' file
        ...
        try
          ...
          if (DataBufferLen > MAXFILESIZE) or (DataBufferLen <= 0) then begin
            if MessageBox(Handle, PChar(FileListInfo.FileName + #13#10 +
              'File exceeds the size limit，Size(' + IntToStr(DataBufferLen) + ')，Continue？'),
              'Message', MB_OKCANCEL + MB_ICONWARNING + MB_DEFBUTTON2) = IDOK then begin
              if Mode_after.Checked then begin
                g_WMImages.AddIndex(-1, 0);
              end
        ...
        if DataBufferLen = 0 then begin
          if Application.MessageBox(PChar(FileListInfo.FileName + #13#10 + 'Compress the file failed，Continue？'),
            'Message', MB_OKCANCEL + MB_ICONWARNING + MB_DEFBUTTON2) = IDOK then begin
            if Mode_after.Checked then begin
              g_WMImages.AddIndex(-1, 0);
            end
      ...
      if (DataBufferLen > 0) and (DataBuffer <> nil) then begin
        if DataBufferLen + SizeOf(ImageInfo) > MAXBUFFERLEN then begin
          Application.MessageBox(PChar(FileListInfo.FileName + #13#10 +
            'Memory overlow，Size(' + IntToStr(DataBufferLen) + ')'), 'Error', MB_OK + MB_ICONSTOP);
          break;
        end;
          ...
          if (DataBufferLen + SizeOf(ImageInfo) + BufferLen) > MAXBUFFERLEN then begin
            if Mode_Insert.Checked then begin
              ...
              if (StartPos <= 0) then begin
                Application.MessageBox('File Index Error！', 'Error', MB_OK + MB_ICONSTOP);
                break;
              end;
  ...
  if MessageBox(Handle, 'Complete，Exit？', 'Message', MB_OKCANCEL + MB_ICONQUESTION) = IDOK then
    begin
    Close;
  end;
end;
...
begin
  if Image_Blend.ItemIndex = g_CustomIndex then begin
    nBlend := StrToIntDef(InputBox('Mix', 'Please Enter a Mixed Mode', '$' +
