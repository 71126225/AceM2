...
begin
  n := 0;
  if Application.MessageBox('Delete invalid image project point?', 'Message', MB_OKCANCEL + MB_ICONQUESTION) = IDOK then
  begin
    for I := PieceList.Count - 1 downto 0 do begin
      p := PTPieceInfo(PieceList[i]);
  ...
  Pbox.Repaint;
  Application.MessageBox(PChar('Cleanup is finished，total clear' + IntToStr(n) + 'Invalid data!'), 'Message', MB_OK + MB_ICONINFORMATION);
end;
