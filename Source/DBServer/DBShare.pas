...
//  sLogPath: string = '.\Log\';
  ...
  sSQLM2Server: String = 'Mir2_M2Server';
  sSQLDBServer: String = 'Mir2_DBServer';
  sSQLInstance: String = '(local)';
  sMapFile: string;
...
begin
  ...
  try
    ...
    MainOutMessage('IP Table ' + sServerIPConfFileNmae + ' failed to load');
  end;
end;
...
begin
  ...
  if Conf <> nil then begin
    ...
    sSQLM2Server := Conf.ReadString(sConfClass, 'SQLM2Server', sSQLM2server);
    sSQLDBServer := Conf.ReadString(sConfClass, 'SQLDBServer', sSQLDBServer);
    sSQLInstance := Conf.ReadString(sConfClass, 'SQLInstance', sSQLInstance);
    ...
    Conf.WriteString(sConfClass, 'SQLM2Server', sSQLM2Server);
    Conf.WriteString(sConfClass, 'SQLDBServer', sSQLDBServer);
    Conf.WriteString(sConfClass, 'SQLInstance', sSQLInstance);
...
function dateForSQL(dModDate: TDateTime): string; //Sams date/time converter so mysql can be updated without errors.
var
