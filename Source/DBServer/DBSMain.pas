...
begin
  ...
  if not CheckServerIP(sIPaddr) then begin
    MainOutMessage('Unknown server ip connection: ' + sIPaddr);
    Socket.Close;
    Exit;
  end;
...
begin
  ...
  n4ADC24 := 0;
  n4ADC28 := 0;
  SendGameCenterMsg(SG_STARTNOW, 'Starting DBServer...');
  StartTimer.Enabled := True;
  if not g_boTestServer then Caption := 'DBServer - Full Version'
  else Caption := 'DBServer';
end;
...
begin
  ...
  boOpenDBBusy := True;
  HumChrDB := TFileHumDB.Create();
  MainOutMessage(format('Character list database loaded(%d)...',
      [HumChrDB.Count]));
  {if HumChrDB.m_Header.sDesc <> HumDB.sDBHeaderDesc then begin
    ShowMessage('Hum.DB The file version does not match the program requirements' + #13#10 + 'file version£º' + HumChrDB.m_Header.sDesc + #13#10 + 'Required version£º' + HumDB.sDBHeaderDesc);
    Exit;
  end;  }
  HumDataDB := TFileDB.Create();
  {if HumDataDB.m_Header.sDesc <> HumDB.sDBHeaderDesc then begin
    ShowMessage('Hum.DB The file version does not match the program requirements ' + #13#10 + 'file version£º' + HumDataDB.m_Header.sDesc + #13#10 + 'Required version£º' + HumDB.sDBHeaderDesc);
    Exit;
  end; }
  if HumDataDB.Count >= Trunc(PLAYOBJECTINDEXCOUNT / 10 * 8) then begin
    ShowMessage('To many Players in the database, totals: ' + IntToStr(HumDataDB.Count) + '/' + IntToStr(PLAYOBJECTINDEXCOUNT));
    //Close;
    //exit;
  end;
  ...
  SendGameCenterMsg(SG_STARTOK, 'DBServer started successfully...');
...
begin
  if m_boRemoteClose then
    Exit;
  if Application.MessageBox('Are you sure you want to close this server?',
      'Exit', MB_YESNO + MB_ICONQUESTION) = mrYes then begin
    CanClose := True;
    ServerSocket.Active := False;
    MainOutMessage('Closing Server...');
  end
  else begin
    CanClose := False;
  end;
  ...
  try
    ...
    for i := 0 to nRecordCount - 1 do begin
      ...
      if StdItemList.Count = Idx then begin
        ...
        MainOutMessage(format('Loading items (Idx:%d Name:%s)Failed£¡£¡£¡', [Idx, StdItem.Name]));
        Result := -100;
        Exit;
      end;
      Query.Next;
    ...
    Result := nRecordCount;
    MainOutMessage(format('Items Database loaded(%d)...',
      [nRecordCount]));
  finally
    Query.Close;
  end;
...
begin
  ...
  MainOutMessage(format('Skills Database loaded(%d)...', [nRecordCount]));
  Query.Close;
end;
...
begin
  try
    ...
    MainOutMessage('Failed to load Gate ID/IP Table');
  end;
end;
{
