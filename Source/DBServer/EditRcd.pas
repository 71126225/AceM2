...
begin
  RefShow();
  Caption := format('Edit character data [%s]', [m_ChrRcd.Data.sChrName]);
...
var
  ...
  UserItem: TUserItem;
const
  Names: array[0..14] of String[6] = ('Clothing', 'arms', 'helmet', 'necklace', 'Lighting', 'Left bracelet', 'Right bracelet', 'Left ring', 'Right ring', 'Poison', 'belt', 'boots', 'Stone', 'Horse', 'tool');
begin
...
begin
  ...
  if nFileHandle <= 0 then begin
    MessageBox(Handle, 'Error saving file!!!', 'error message', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  FileWrite(nFileHandle, m_ChrRcd, SizeOf(THumDataInfo));
  FileClose(nFileHandle);
  MessageBox(Handle, 'Character data exported successfully!!!', 'Tips', MB_OK + MB_ICONINFORMATION);
end;
...
begin
  ...
  if not FileExists(sLoadFileName) then begin
    MessageBox(Handle, 'The specified file was not found!!!', 'error message', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  nFileHandle := FileOpen(sLoadFileName, fmOpenReadWrite or fmShareDenyNone);

  if nFileHandle <= 0 then begin
    MessageBox(Handle, 'Error opening file!!!', 'error message', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  if not FileRead(nFileHandle, ChrRcd, SizeOf(THumDataInfo)) = SizeOf(THumDataInfo) then begin
    MessageBox(Handle, 'Error reading file!!!'#13#13'The file format may be incorrect', 'error message', MB_OK + MB_ICONEXCLAMATION);
    Exit;
  end;
  ...
  FileClose(nFileHandle);
  RefShow();
  MessageBox(Handle, 'Character data imported successfully!!!', 'Tips', MB_OK + MB_ICONINFORMATION);
end;
...
begin
  ...
  if boSaveOK then begin
    MessageBox(Handle, 'Character data saved successfully!!!', 'Tips', MB_OK + MB_ICONINFORMATION);
  end
  else begin
    MessageBox(Handle, 'Failed to save character data!!!', 'error message', MB_OK + MB_ICONEXCLAMATION);
  end;
end;
