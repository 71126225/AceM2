    ...
    if HumChrDB.FindByAccount(UserInfo.sAccount, ChrList) >= 0 then begin
      try
          for I := 0 to ChrList.Count - 1 do begin
            ...
            if HumChrDB.Get(QuickID.sChrName, HumRecord) and HumRecord.boDeleted and
        ...
        on e: Exception do
          MainOutMessage('[ViewChr]: ' + e.Message);
      end;
    end;
  except
    on e: Exception do
...
begin
  ...
  try
    if (HumDataDB.Index(sChrName) = -1) then begin
...
begin
  try
    ...
    on e: Exception do
      MainOutMessage('[LoadServerInfo]: ' + e.Message);
  end;
end;
    ...
    CM_QUERYCHR: begin
        if not UserInfo.boWaitMsg then begin
          if (not UserInfo.boChrQueryed) or ((GetTickCount - UserInfo.dwChrTick) > 200) then begin
            ...
            MainOutMessage('[Overspeed operation] Search for people ' + UserInfo.sUserIPaddr);
          end;
        end;
      end;
    CM_NEWCHR: begin
        if not UserInfo.boWaitMsg then begin
          if (GetTickCount - UserInfo.dwChrTick) > 1000 then begin
            ...
            MainOutMessage('[Overspeed operation] Creating a character ' + UserInfo.sAccount + '/'
              +
    ...
    CM_DELCHR: begin
        if not UserInfo.boWaitMsg then begin
          if (GetTickCount - UserInfo.dwChrTick) > 500 then begin
            ...
            MainOutMessage('[Overspeed operation] Delete a character' + UserInfo.sAccount + '/'
              +
              UserInfo.sUserIPaddr);
          end;
        end;
    ...
    CM_VIEWDELHUM: begin
        if not UserInfo.boWaitMsg then begin
          if (GetTIckCount - UserInfo.dwChrTick) > 200 then begin
            ...
            //Inc(g_nViewDelHum);
            MainOutMessage('[Overspeed operation] Query deleted characters ' + UserInfo.sUserIPaddr);
          end;
        end;
      end;
    CM_RENEWHUM: begin
        if not UserInfo.boWaitMsg then begin
          if (GetTIckCount - UserInfo.dwChrTick) > 200 then begin
            ...
            //Inc(g_nRenewDelHum);
            MainOutMessage('[Overspeed operation] Restore Character ' + UserInfo.sUserIPaddr);
          end;
        end;
      end;
  else begin
        ...
        try
          //if HumDataDB.OpenEx then begin
            for i := 0 to ChrList.Count - 1 do begin
              ...
              //If the selected ID is incorrect, skip
              //if QuickID.nIndex <> UserInfo.nSelGateID then Continue;

              if HumChrDB.Get(QuickID.sChrName, HumRecord) and
          ...
          on e: Exception do
            MainOutMessage('[QueryChr]: ' + e.Message);
        end;
      end;
    except
      on e: Exception do
...
begin
  ...
  try
      if not HumChrDB.Get(sChrName, HumRecord) then
        exit;
...
begin
  ...
  try
    //if HumChrDB.Open then begin
      ...
      //if n10 >= 0 then begin
      if HumChrDB.Get(sChrName,HumRecord) = true then begin
...
begin
  ...
  for I := 1 to length(sChrName) do begin
    if NOT (char(sChrName[i]) in ValidChars) then nCode := 0;
  end;
  ...
  for i := 0 to GateList.Count - 1 do begin
    ...
    on e: Exception do
      MainOutMessage('[AddNewChr]: ' + e.Message);
  end;
end;
...
begin
  ...
  if not (Length(sChrName) in [3..ActorNameLen]) then nCode := 4;
...
begin
  ...
  if UserInfo.sAccount = sAccount then begin
    try
        ...
        if HumChrDB.FindByAccount(sAccount, ChrList) >= 0 then begin
          for i := 0 to ChrList.Count - 1 do begin
            QuickID := pTQuickID(ChrList.Objects[i]);
            if HumChrDB.Get(QuickId.sChrName, HumRecord) then begin
      ...
      on e: Exception do
        MainOutMessage('[SelectChr]: ' + e.Message);
    end;
    try
        nIndex := HumDataDB.Index(sChrName);
        if nIndex >= 0 then begin
  ...
  if boDataOK then begin
    ...
    if g_boDynamicIPMode then
      sRouteIP := UserInfo.sGateIPaddr //Using Dynamic IP
    else
