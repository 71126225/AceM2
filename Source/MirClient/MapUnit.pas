...
//    m_sMapBase: string;
    m_boNewMap: Boolean;
    m_sFileName: string;
    m_MArr: array[0..MAXX * 3, 0..MAXY * 3] of TSdoMapInfo2012;
    m_boChange: Boolean;
    m_ClientRect: TRect;
...
begin
  ...
  nBy := (nCurrY + 3) * LOGICALMAPUNIT;
  if nLx < 0 then nLx := 0;
  if nTy < 0 then nTy := 0;
  if nBy >= Header.Height then nBy := Header.Height;
  for i := nLx to nRx - 1 do begin
    if (i >= 0) and (i < Header.Width) then begin
      Move(g_LegendMap.MapData[Header.Height * i + nTy], m_MArr[i - nLx, 0], Sizeof(TSdoMapInfo2012) * (nBy - nTy));
    end;
  end;
...
begin
  ...
  if bowalk then
    Map.m_MArr[cx, cy].FrImg := Map.m_MArr[cx, cy].FrImg and $7FFF
  else
    Map.m_MArr[cx, cy].FrImg := Map.m_MArr[cx, cy].FrImg or $8000;
end;
...
begin
  ...
  if (cx < 0) or (cy < 0) or (cx > MAXX * 3) or (cy > MAXY * 3) then
    Exit;
  Result := ((Map.m_MArr[cx, cy].BkImg and $8000) + (Map.m_MArr[cx, cy].FrImg and $8000)) = 0;
  if Result then begin //πÆ∞ÀªÁ
    if Map.m_MArr[cx, cy].DoorIndex and $80 > 0 then begin //πÆ¬¶¿Ã ¿÷¿Ω
      if (Map.m_MArr[cx, cy].DoorOffset and $80) = 0 then
...
begin
  ...
  if (cx < 0) or (cy < 0) or (cx > MAXX * 3) or (cy > MAXY * 3) then
    Exit;
  Result := (Map.m_MArr[cx, cy].FrImg and $8000) = 0;
  if Result then begin //πÆ∞ÀªÁ
    if Map.m_MArr[cx, cy].DoorIndex and $80 > 0 then begin //πÆ¬¶¿Ã ¿÷¿Ω
      if (Map.m_MArr[cx, cy].DoorOffset and $80) = 0 then
        Result := FALSE; //πÆ¿Ã æ» ø≠∑»¿Ω.
...
begin
  ...
  if (cx < 0) or (cy < 0) or (cx > MAXX * 3) or (cy > MAXY * 3) then
    Exit;
  if Map.m_MArr[cx, cy].DoorIndex and $80 > 0 then begin
    Result := Map.m_MArr[cx, cy].DoorIndex and $7F;
  end;
end;
...
begin
  ...
  if (cx < 0) or (cy < 0) or (cx > MAXX * 3) or (cy > MAXY * 3) then
    Exit;
  if Map.m_MArr[cx, cy].DoorIndex and $80 > 0 then begin
    Result := (Map.m_MArr[cx, cy].DoorOffset and $80 <> 0);
  end;
end;
...
begin
  ...
  if (cx < 0) or (cy < 0) or (cx > MAXX * 3) or (cy > MAXY * 3) then
    Exit;
  if Map.m_MArr[cx, cy].DoorIndex and $80 > 0 then begin
    idx := Map.m_MArr[cx, cy].DoorIndex and $7F;
    for i := cx - 10 to cx + 10 do
      for j := cy - 10 to cy + 10 do begin
        if (i > 0) and (j > 0) then
          if (Map.m_MArr[i, j].DoorIndex and $7F) = idx then
            Map.m_MArr[i, j].DoorOffset := Map.m_MArr[i, j].DoorOffset or $80;
      end;
  end;
end;
  ...
  if Map.m_MArr[cx, cy].DoorIndex and $80 > 0 then begin
    ...
    for i := cx - 8 to cx + 10 do
      for j := cy - 8 to cy + 10 do begin
        if (Map.m_MArr[i, j].DoorIndex and $7F) = idx then
          Map.m_MArr[i, j].DoorOffset := Map.m_MArr[i, j].DoorOffset and $7F;
      end;
  end;
end;
