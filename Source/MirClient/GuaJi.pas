...
begin
  ...
  m_nTargetX := -1; //目标座标
  m_nTargetY := -1; //目标座标
  FRunTick := FrmMain.m_CurrentTick;
  FAttackTick := FrmMain.m_CurrentTick;
end;
...
begin
  ...
  case nDir of
    DR_UP: if sny > nFlag - 1 then
        Dec(sny, nFlag);
    DR_DOWN: if sny < (g_LegendMap.MapHeader.Height - nFlag) then
        Inc(sny, nFlag);
    DR_LEFT: if snx > nFlag - 1 then
        Dec(snx, nFlag);
    DR_RIGHT: if snx < (g_LegendMap.MapHeader.Width - nFlag) then
        Inc(snx, nFlag);
    DR_UPLEFT:
      begin
    ...
    DR_UPRIGHT:
      begin
        if (snx > nFlag - 1) and (sny < (g_LegendMap.MapHeader.Height - nFlag)) then
        begin
          Inc(snx, nFlag);
          Dec(sny, nFlag);
        end;
    ...
    DR_DOWNLEFT:
      begin
        if (snx < (g_LegendMap.MapHeader.Width - nFlag)) and (sny > nFlag - 1) then
        begin
          Dec(snx, nFlag);
          Inc(sny, nFlag);
        end;
    ...
    DR_DOWNRIGHT:
      begin
        if (snx < (g_LegendMap.MapHeader.Width - nFlag)) and (sny < (g_LegendMap.MapHeader.Height - nFlag)) then
        begin
          Inc(snx, nFlag);
          Inc(sny, nFlag);
        end;
...
begin
  ...
  if FStarted and (FrmMain.m_CurrentTick - FRunTick > 200) then
  begin
    FRunTick := FrmMain.m_CurrentTick;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (FrmMain.m_CurrentTick - FAttackTick > 1000) and (frmMain.ActionKey = 0) then
          begin
            FAttackTick := FrmMain.m_CurrentTick;
