...
begin
  if lstItems.ItemIndex <> -1 then begin
    ...
    if GetStdItem(seIndex.Value, g_Myself.m_Abil.Level, g_Myself.m_btJob).Name = '' then begin
      Application.MessageBox(PChar('Failed to save, Item [' + IntToStr(seIndex.Value) + '] does not exist'), '', MB_OK +
        MB_ICONINFORMATION);
      exit;
    end;
    ...
    if (seFlute1.Value > 0) then begin
      StdItem := GetStdItem(seFlute1.Value, g_Myself.m_Abil.Level, g_Myself.m_btJob);
      if (StdItem.Name = '') or (StdItem.StdMode2 <> 46) or (StdItem.Shape <> 3) then begin
        Application.MessageBox(PChar('Failed to save, Slot 1 item number is not set correctly!'), '', MB_OK + MB_ICONINFORMATION);
        exit;
      end;
    end;
    if (seFlute2.Value > 0) then begin
      StdItem := GetStdItem(seFlute2.Value, g_Myself.m_Abil.Level, g_Myself.m_btJob);
      if (StdItem.Name = '') or (StdItem.StdMode2 <> 46) or (StdItem.Shape <> 3) then begin
        Application.MessageBox(PChar('Failed to save, Slot 1 item number is not set correctly!'), '', MB_OK + MB_ICONINFORMATION);
        exit;
      end;
    ...
    if (seFlute3.Value > 0) then begin
      StdItem := GetStdItem(seFlute3.Value, g_Myself.m_Abil.Level, g_Myself.m_btJob);
      if (StdItem.Name = '') or (StdItem.StdMode2 <> 46) or (StdItem.Shape <> 3) then begin
        Application.MessageBox(PChar('Failed to save, Slot 1 item number is not set correctly!'), '', MB_OK + MB_ICONINFORMATION);
        exit;
      end;
    ...
    ClientItem.s := GetStdItem(seIndex.Value, g_Myself.m_Abil.Level, g_Myself.m_btJob);
