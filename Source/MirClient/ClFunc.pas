...
function UpdateItemBagByUserItem(cu: TUserItem; boEffect:boolean = False): Boolean;
...
function CountItems(wIdx:integer): integer;
...
procedure FillUserKeys(cu:TNewClientItem);
...
function FindBadApps(): String;
function GetApps(): string;
implementation
uses
  MShare, MNShare, FState, ClMain, EDcodeEx, TLHelp32;
...
begin
  result:=0;
  StdItem := GetStdItem(wIdx, g_Myself.m_Abil.Level, g_Myself.m_btJob);
  if (StdItem.Name <> '') and IsUserKeyItem(@StdItem) then begin
    for I := Low(g_ItemArr) to High(g_ItemArr) do begin
      if (g_ItemArr[i].s.Name <> '') and (g_ItemArr[i].s.Idx = StdItem.Idx) then begin
        if (sm_Superposition in StdItem.StdModeEx) and (StdItem.DuraMax > 1) then begin
          ...
          if (StdItem.StdMode2 = 2) then begin
            Inc(result, g_ItemArr[i].UserItem.Dura div 1000);
          end else
           Inc(result);
      end;
    end;
   if g_boItemMoving and  (g_MovingItem.ItemType = mtBagItem) and (g_MovingItem.Item.s.Idx = StdItem.Idx) then begin
      if (sm_Superposition in StdItem.StdModeEx) and (StdItem.DuraMax > 1) then begin
        Inc(result, g_MovingItem.Item.UserItem.Dura);
      end else
        if (StdItem.StdMode2 = 2) then begin
          Inc(result, g_MovingItem.Item.UserItem.Dura div 1000);
        end else
         Inc(result);
    end;
  end;
...
begin
  if CheckItem <> nil then begin
    ...
    if (Item.UserItem.wIndex > 0) and (Item.s.name = '') then
      Item.s := GetStdItem(Item.UserItem.wIndex, g_Myself.m_Abil.Level, g_Myself.m_btJob);
    if IsUserKeyItem(@Item.s) then begin
      for I := Low(g_UserKeySetup) to High(g_UserKeySetup) do begin
        ...
        //we just found our item
          g_UserKeyIndex[i] := CountItems(g_UserKeySetup[i].wIndex);
          if g_userkeyindex[i] = 0 then begin //if the item is now used up, remove it
    ...
    for I := Low(g_UserKeySetup) to High(g_UserKeySetup) do begin
      if g_UserKeySetup[i].btType = UKTYPE_ITEM then begin
        StdItem := GetStdItem(g_UserKeySetup[i].wIndex, g_Myself.m_Abil.Level, g_Myself.m_btJob);
        if (StdItem.Name <> '') and IsUserKeyItem(@StdItem) then begin
  ...
  if (boSave)  and (FrmMain.m_CurrentTick - FrmMain.nLastQuickslotChange > 500) then begin
    FrmMain.nLastQuickSlotChange := FrmMain.m_CurrentTick;
...
begin
  ...
  if not (IsUserKeyItem(@cu.s)) then exit;
  boSave:=False;

  //first loop to see if item already excists
  for I := Low(g_UserKeySetup) to 5 do begin
    if (g_UserKeySetup[i].btType = UKTYPE_ITEM) and (g_UserKeySetup[i].wIndex = cu.UserItem.wIndex) then exit;//if item is already in quickslots
  end;

  for I := Low(g_UserKeySetup) to 5 do begin
    if (g_UserKeySetup[i].btType = UKTYPE_ITEM) and (g_UserKeySetup[i].wIndex = cu.UserItem.wIndex) then exit;//if item is already in quickslots
    if (g_UserKeySetup[i].wIndex = 0) then begin
      if (CountItems(cu.UserItem.wIndex) <> 0) then begin
        g_UserKeySetup[i].btType := UKTYPE_ITEM;
        g_UserKeySetup[i].wIndex := cu.UserItem.wIndex;
        g_UserKeyIndex[i] := CountItems(g_UserKeySetup[i].wIndex);
        boSave:=True;
      end;
      break;
...
begin
  ...
  Item.s := GetStditem(wIdent, g_Myself.m_Abil.Level, g_Myself.m_btJob);
...
var
  ...
  TempList: TList;
begin
exit;//experiment can break alot!
...
var
  ...
  function ISInValidModule(Name: string): string;
  var
    Loop: String;
    i: integer;
  begin
    Result := '';
    for I := 0 to g_BadAppsList.Count - 1 do begin
      if g_BadAppsList[i] = EncodeString(Name) then begin
        result := g_BadAppsList[i];
        exit;
      end;
    end;
  end;
  function ISInValidProcess(Name: string): string;
  var
    i: integer;
    Loop: String;
  begin
  ...
  function CheckModules(id:DWord): String;
  var
    FModuleHandle: THandle;
    FModuleEntry: TModuleEntry32;
    ContinueLoop: Bool;
    sModName: string;
  begin
    Result := '';
    try
      FModuleHandle:= CreateToolHelp32SnapShot(TH32CS_SNAPMODULE, id);
      FModuleEntry.dwSize := SizeOf(FModuleEntry);
      ContinueLoop := Module32First(FModuleHandle, FModuleEntry);
      while Integer(ContinueLoop) <> 0 do begin
        sModName := IsInValidModule(AnsiLowerCase(FModuleEntry.szModule));
        if (sModName <> '') then begin
          Result:= sModName;
          exit;
        end;
        ContinueLoop := Module32Next(FModuleHandle, FModuleEntry);
      end;
      CloseHandle(FModuleHandle);
    except
      DebugOutStr('Warn thedeath about this crash: error 3');
    end;
  end;
begin
  Result := '';
  try
    ...
    while Integer(ContinueLoop) <> 0 do begin
      sProcessName := IsInValidProcess(AnsiLowerCase(fProcessEntry32.szExeFile));
      if (sProcessName <> '') then begin
        Result:=sProcessName;
        exit;
      end;
    //DebugOutStr(FProcessEntry32.szExeFile);
      sProcessName := CheckModules(FProcessEntry32.th32ProcessID);
      if sProcessName <> '' then begin
        Result:= sProcessName;
        exit;
      end;
    ...
    DebugOutStr('Warn thedeath about this crash: error 2');
  end;
end;
...
var
  ...
  nIdx:integer;
  App:TApp;
  function AddModules(id: DWord; nIdx:integer): String;
  var
    FModuleHandle: THandle;
    FModuleEntry: TModuleEntry32;
    ContinueLoop: Bool;
  ...
  begin
    ...
    try
      ...
      while Integer(ContinueLoop) <> 0 do begin
        App.id := nIdx;
        //Move(FModuleEntry.szModule[1],App.sName[1],SizeOf(App.sName));
        App.sName := String(FModuleEntry.szModule);
        App.Size := FModuleEntry.dwSize;
        result := result + EncodeBuffer(@App,SizeOf(App)) + '/';
        ContinueLoop := Module32Next(FModuleHandle, FModuleEntry);
      end;
      CloseHandle(FModuleHandle);
    except
...
begin
  nIdx := 0;
   try
    ...
    while Integer(ContinueLoop) <> 0 do begin
      App.id := nIdx;
      App.sName := String(FProcessEntry32.szExeFile);
      App.Size := FProcessEntry32.dwSize;
      output := output + EncodeBuffer(@App, SizeOf(App)) + '/' + AddModules(FProcessEntry32.th32ProcessID,nIdx);
      inc(nidx);
      ContinueLoop := Process32Next(FSnapShotHandle, FProcessEntry32);
    end;
    CloseHandle(FSnapShotHandle);
