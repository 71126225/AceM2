...
{$ENDIF}
  ...
  g_BannedAppsDataStream: TMemoryStream;
  ...
  g_BadAppsList: TStringList;
  g_Cmd_UserMove: string = '@Teleport';
  g_Cmd_Auth: string = '@AllowAlliance';
  g_Cmd_AuthCancel: string = '@EndAlliance';
  g_Cmd_AllMsg: string = '@All';
  g_Cmd_EndGuild: string = '@LeaveGuild';
  g_Cmd_TakeOnHorse: string = '@RideHorse';
  g_Cmd_TakeOffHorse: string = '@DismountHorse';
  g_Cmd_MemberFunction: string = '@MemberFunction';
  g_Cmd_MemberFunctionEx: string = '@MemberFunctionEx';
...
function GetDefMagicIcon(MagIcon: Integer): TDXTexture;
  ...
  while (True) do begin
    ...
    if (sMapName <> '') and (CompareText(sMapName, g_sMapTitle) = 0) and (nX <> -1) and (nY <> -1) then begin
      ...
      if High(g_nMiniMapPath) > 2 then begin
        ...
        g_nScriptGotoX := -1;
        g_nScriptGotoY := -1;
        DScreen.AddSysMsg('[Unable to reach the destination]', clRed);
      end;
      exit;
    end;
  end;
  DScreen.AddSysMsg('[Not available on this map]', clRed);
end;
...
begin
  if g_MySelf = nil then exit;
  Result := 'There are currently no tasks';
  sBACKMSG := '';
...
var
  ...
  sLabel: string;
  n14: integer;
  sWar, sWizard, sTaos, sAssassin, sArcher, sWar_W, sWizard_W, sTaos_W, sAssassin_W, sArcher_W: string;
  sMan, sWoMan: string;
begin
  nC := 0;
  tempstr := sMsg;
    ...
    if s10 <> '' then begin
      if (pos('/', s10) <= 0) and (s10[1] = '^') then begin
        ...
        if CompareLStr(sLabel, VAR_BJOB, Length(VAR_BJOB)) then begin
          ...
          s14 := GetValidStr3(s14, sAssassin, [',']);
          s14 := GetValidStr3(s14, sArcher, [',']);
          if g_MySelf <> nil then begin
            case g_MySelf.m_btJob of
              ...
              3: sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', sAssassin);
              4: sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', sArcher);
        ...
        if CompareLStr(sLabel, VAR_BJS, Length(VAR_BJS)) then begin
          ...
          s14 := GetValidStr3(s14, sAssassin_W, [',']);
          s14 := GetValidStr3(s14, sArcher_W, [',']);
          if g_MySelf <> nil then begin
            case g_MySelf.m_btJob of
              0: begin
              ...
              3: begin
                  if g_MySelf.m_btSex = 0 then
                    sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', sAssassin)
                  else
                    sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', sAssassin_W);
                end;
              4: begin
                  if g_MySelf.m_btSex = 0 then
                    sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', sArcher)
                  else
                    sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', sArcher_W);
                end
            else sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', '????');
            end;
          end else
...
var
  d: TDXTexture;
begin
  if MagIcon > 2000 then
    d := g_WMagIconImages.Images[MagIcon - 2000]
  else if MagIcon > 1000 then
    d := g_WDefMagIcon2Images.Images[MagIcon - 1000]
  else
    d := g_WDefMagIconImages.Images[MagIcon];
...
begin
  ...
  g_BadAppsList := TStringList.Create;
