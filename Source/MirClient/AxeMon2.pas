...
type
  ...
  TSpiritMonster = class(TSkeletonOma)
    public
    procedure DrawChr(dsurface: TDirectDrawSurface; dx, dy: integer; blend: Boolean; boFlag: Boolean); override;
  end;
...
procedure TOmaKing.CalcActorFrame;
var
  pm: PTMonsterAction;
begin
  inherited;
...
procedure TOmaKing.LoadSurface;
begin
  ...
  case m_wAppearance of
    219: begin
        case m_nCurrentAction of
          ...
          SM_LIGHTING: begin
              if m_boCasted then begin
                PlayScene.NewMagic(Self, 11, 37, m_nCurrX, m_nCurrY, m_nCurrX,
                m_nCurrY, m_nRecogId, mt13, False, 30, m_boUseEffect);
                m_boCasted := FALSE;
              end;
              m_boUseEffect := TRUE;
          end;
...
procedure TOmaKing.DrawChr(dsurface: TDirectDrawSurface; dx, dy: integer;
...
begin
  ...
  if m_btRace = 115 then begin //Ghostly Image
    Blend := True;
    if (ceff = ceNone) or (ceff = ceBright) then
      Ceff := ceTeal;
  end;
  ...
  if m_boUseEffect then
    if EffectSurface <> nil then begin
      ...
      DrawEffect(dsurface, dx + ax + m_nShiftX, dy + ay + m_nShiftY, EffectSurface, ceTeal, True, 1) //Ghostly Magic
      else
...
begin
  ...
  EffectSurface := nil;
  EffectSurfaceEx := nil;
  EffectTick := FrmMain.m_CurrentTick;
  EffectIdx := 0;
  m_boOutside := True;
end;
...
begin
  m_btDir := 0;
  if FrmMain.m_CurrentTick > EffectTick then begin
    EffectTick := FrmMain.m_CurrentTick + 120;
    Inc(EffectIdx);
...
procedure TSpiritMonster.DrawChr(dsurface: TDirectDrawSurface; dx, dy: integer;
  blend: Boolean; boFlag: Boolean);
var
  ceff: TColorEffect;
begin
