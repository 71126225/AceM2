...
begin
  ...
  //SysMsgBoardTop := 0;
  m_HintSurface := nil;
  m_nShowSysTick := FrmMain.m_CurrentTick;
  //m_HintBGSurface := nil;
  m_SysInfoList := TList.Create;
  SurfaceRefTick := FrmMain.m_CurrentTick;
  boShowSurface := False;
  NpcTempList := TStringList.Create;
  AutoImgTime := FrmMain.m_CurrentTick;
  nShowIndex := 0;
...
begin
  ...
  with SaySurface do begin
    ...
    SayHint.AddTime := FrmMain.m_CurrentTick;
    SayHint.EffectTime := FrmMain.m_CurrentTick;
...
begin
  ...
  if g_nInterface = 1 then
    ...
    g_nDefaultSayWidth := 147;

  SayMessage := NewSayMsgEx(g_nDefaultSayWidth + g_FScreenWidth - DEFSCREENWIDTH, SAYLISTHEIGHT, UserSayType, BColor);
    ...
    for I := 1 to Length(WideStr) do begin
      ...
      if boImage then begin
        if tstr = '#' then begin
          ...
          if nIndex in [Low(g_FaceIndexInfo)..High(g_FaceIndexInfo)] then begin
            if (nLen + SAYFACEWIDTH) > (g_nDefaultSayWidth + g_FScreenWidth - DEFSCREENWIDTH - 1) then begin
      ...
      if boClickName then begin
        if tstr = #7 then begin
          ...
          if (nLen + nTextLen) > (g_nDefaultSayWidth + g_FScreenWidth - DEFSCREENWIDTH - 1) then begin
            WideStr2 := cmdstr;
            cmdstr := '';
            AddStr2 := '';
            for ii := 1 to Length(WideStr2) do begin
              ...
              if (nLen + g_DXCanvas.TextWidth(AddStr2 + tstr2)) > (g_nDefaultSayWidth + g_FScreenWidth - DEFSCREENWIDTH - 1) then begin
                ...
                TextOutEx(nLen, 2, AddStr2, nFColor, nBColor);
                Inc(nLen, nTextLen);
                AddStr2 := '';
            ...
            ClickName.nRight := nLen + nTextLen;
            ClickName.Index := ClickIndex;
            TextOutEx(nLen, 2, cmdstr, nFColor, nBColor);
            Inc(nLen, nTextLen);
            cmdstr := '';
            if SayMessage.ClickList = nil then SayMessage.ClickList := TList.Create;
            SayMessage.ClickList.Add(ClickName);
      ...
      if boBeginColor then begin
        ...
        if (nLen + g_DXCanvas.TextWidth(AddStr + tstr)) > (g_nDefaultSayWidth + g_FScreenWidth - DEFSCREENWIDTH - 1) then begin
          TextOutEx(nLen, 2, AddStr, nFColor, nBColor);
          Inc(nLen, g_DXCanvas.TextWidth(AddStr));
          AddStr := '';
          cmdstr := '';
...
//if g_nInterface = 1 then FrmDlg.DSayUpDown.MaxPosition := _MAX(0, m_SayTransferList.count - FrmDlg.DWndSay.Height div SAYLISTHEIGHT)  // original line ripman
if g_nInterface = 1 then FrmDlg.DSayUpDown.MaxPosition := _MAX(0, (m_NewSayMsgList.count + 5) - FrmDlg.DWndSay.Height div SAYLISTHEIGHT)  //ripman fix this!!
else if g_nInterface = 2 then FrmDlg.DSayUpDown.MaxPosition := _MAX(0, (m_NewSayMsgList.count + 3) - FrmDlg.DWndSay.Height div SAYLISTHEIGHT);

 {   if not g_SayUpDownLock then
      FrmDlg.DSayUpDown.Position := FrmDlg.DSayUpDown.MaxPosition; }
      if ((g_nInterface = 1) and (FrmDlg.DSayUpDown.Position = (FrmDlg.DSayUpDown.MaxPosition - 6)))
      or ((g_nInterface = 2) and (g_nEnlargeChat = 0)) and (FrmDlg.DSayUpDown.Position = (FrmDlg.DSayUpDown.MaxPosition - 1))
      or ((g_nInterface = 2) and (g_nEnlargeChat = 1)) and (FrmDlg.DSayUpDown.Position = (FrmDlg.DSayUpDown.MaxPosition - 4)) then
        FrmDlg.DSayUpDown.Position := FrmDlg.DSayUpDown.MaxPosition;
...
//  addh := 0;
  ...
  if (FrmMain.m_CurrentTick > SurfaceRefTick) or (nShowIndex <> ShowIndex) then begin
    nShowIndex := ShowIndex;
    SurfaceRefTick := FrmMain.m_CurrentTick + 100;
    ClearHint(True);
    HintSurface.Clear;
    HintWidth := 0;
    HintHeight := 0;
      ...
      if data <> '' then begin
        while (pos('<', data) > 0) and (pos('>', data) > 0) and (data <> '') do
          ...
          if cmdstr <> '' then begin
            ...
            else if CompareLStr(cmdstr, '^', Length('^')) then begin
...
Function  TDrawScreen.NewSayMsgEx(nWidth, nHeight:Integer; UserSayType: TUserSayType; BColor: Cardinal): pTSayMessage;  //ripman
var
...
begin
  ...
  if FrmMain.m_CurrentTick > m_nShowSysTick then begin
    ...
    m_SysInfoList.Clear;
    m_nShowSysTick := FrmMain.m_CurrentTick + 1000;
  end;
  //ax := 360;
  ...
  while True do begin
    ...
    if I = ADDSAYCOUNT then begin
      if (boTop) and (SayHint.SaySurface <> nil) then begin
        MSurface.Draw(ax, ay, SayHint.SaySurface.ClientRect, SayHint.SaySurface, (250 - nAlpha) shl 24 or $FFFFFF, fxBlend);
      end;
      break;
    end;
    if (((FrmMain.m_CurrentTick - SayHint.AddTime) > 4000) or
      (m_SysMsgList.Count > ADDSAYCOUNT)) and (I = 0) then begin
      boTop := True;
      if FrmMain.m_CurrentTick > SayHint.EffectTime then begin
        SayHint.EffectTime := FrmMain.m_CurrentTick + EffectTime;
      ...
      if SayHint.EffectIndex >= 8 then begin
        ...
        if (SayHint.SaySurface <> nil) then
