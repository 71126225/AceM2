...
begin
  if nEffigyState = -1 then begin
    ...
    d := GetWWeaponImg(nWeapon, 0, 0, nOffset, 1, nHpx, nHpy);
    if d <> nil then begin
      CopyByGrayScale(d, m_StatuarySurface, 80 + nHpx, 110 + nHpy);
      m_boShowStatuary := True;
    end;

    d := GetWHumImg(nDress, nWeapon, 0, 0, nOffset, nHpx, nHpy);
    if d <> nil then begin
      CopyByGrayScale(d, m_StatuarySurface, 80 + nHpx, 110 + nHpy);
      m_boShowStatuary := True;
    end;
...
begin
  if FrmMain.m_CurrentTick > m_dwEffectTick then begin
    m_dwEffectTick := FrmMain.m_CurrentTick + 150;
    Inc(m_nEffectIndex);
  ...
  if not m_boRunEffect then begin
    case m_boBoxState of
      0: begin
          m_boRunEffect := True;
          m_nEffectIndex := 0;
          m_dwEffectTick := FrmMain.m_CurrentTick + 100;
        end;
    end;
  end;
end;
    ...
    if m_boBoxState = 0 then begin
      if FrmMain.m_CurrentTick > m_dwEffectTick then begin
        ...
        if m_nEffectIndex < 6 then Inc(m_nEffectIndex)
          ...
          m_boBoxState := 1;
          m_dwEffectTick := FrmMain.m_CurrentTick + 900;
        end;
      end;
    end else
    if m_boBoxState = 1 then begin
