...
uses
  ...
  MShare, HGEGUI, WIL, HGETextures, Grobal2, Grids, Share, WMFile, IniFiles;

const
  MakeMagicName: array[0..mm_MaxCount] of String[8] = ('test1', 'test2', 'test3', 'test4', 'test5', 'test6', 'test7', 'test8', 'test9', 'test10');
...
begin
  ...
  for I := 1 to 5 do begin
    if (FMakeGoods.Item[I].s.Name <> '') and (FMakeGoods.Item[I].s.Idx = Item.s.Idx) then begin
      if (FMakeGoods.Item[i].s.StdMode = tm_ore) and (FMakeGoods.Item[i].UserItem.Dura > 9) then begin
        if (FMakeGoods.Item[i].UserItem.Dura <= Item.UserItem.Dura) then begin
          Result := TRUE;
        end
      end else
        Result := TRUE;
...
begin
  ...
  FMakeItemHint := 'Result';
  FMakeItemHint2 := '';
  FMakeItemRate := 0;
  FMakeItemShowEffect := FALSE;
  FMakeItemShowTick := FrmMain.m_CurrentTick;
...
begin
  ...
  FStrengthenHint := 'Required to Strengthen Equipment';
  FStrengthenMoney := 0;
...
begin
  ...
  case nID of
    2: FrmDlg.DMessageDlg('Need to place at least a mysterious crystal.', []);
    3: FrmDlg.DMessageDlg('Not allowed to Strengthen this Item.', []);
    4: FrmDlg.DMessageDlg('Unopened item can not be strengthened.', []);
    5: FrmDlg.DMessageDlg('You do not have enough ' + g_sGoldName + ' or ' + g_sBindGoldName + '!', []);
    6: FrmDlg.DMessageDlg('Do not have the required materials.', []);
    7: FrmDlg.DMessageDlg('[System Error]: Unable to complete the build.', []);
    12: FrmDlg.DMessageDlg('You do not have enough bag space.', []);
    -8: begin
        FMakeItemLock := TRUE;
...
begin
  ...
  case nID of
    ...
    6: FrmDlg.DMessageDlg('Need to place at least a mysterious crystal.', []);
    8: begin //升级成功
        ...
        FStrengthenShowTick := FrmMain.m_CurrentTick;
        SafeFillChar(FStrengthenLevel, SizeOf(FStrengthenLevel), #0);
        SafeFillChar(FStrengthenAss, SizeOf(FStrengthenAss), #0);
      end;
    9: begin //升级破碎
...
begin
  ...
  case nID of
    8: begin //升级成功
      ...
      FrmDlg.DMessageDlg('Current equipment does not require re-opening.', []);
    end;
  end;
end;
    ...
    if sm_ArmingStrong in g_MovingItem.Item.s.StdModeEx then begin
      if (not CheckItemBindMode(@g_MovingItem.Item.UserItem, bm_NoMake)) and
        ...
        if not CheckItemBindMode(@g_MovingItem.Item.UserItem, bm_Unknown) then begin
          ...
          FrmDlg.DMessageDlg('Unopened items are not allowed to be strenghthened.', []);
        end;
      end
      else begin
        FrmDlg.CancelItemMoving;
        FrmDlg.DMessageDlg('This Item can not be strengthened', []);
      end;
    end
    else begin
      FrmDlg.CancelItemMoving;
...
begin
  ...
  if FStrengthenRate > 0 then begin
    ...
    if not boOne then begin
      FrmDlg.DMessageDlg('no upgrade slots availabile.', []);
      exit;
    end;
    for I := 0 to 4 do begin
      if FStrengthenAss[I].Item.s.name <> '' then
    ...
    FrmDlg.DMessageDlg('Need to place at least a mysterious crystal.', []);
  end;
end;
  ...
  with Sender as TDGrid do begin
    ...
    if idx in [Low(FStrengthenAss)..High(FStrengthenAss)] then begin
      if not g_boItemMoving then begin
        ...
        if FStrengthenItem.Item.s.Name = '' then begin
          FrmDlg.CancelItemMoving;
          FrmDlg.DMessageDlg('Please put in equipment to strengthen.', []);
          exit;
        end;
        if (tm_MakeStone = g_MovingItem.Item.s.StdMode) and (g_MovingItem.Item.s.Shape in [1, 2]) then begin
          if FStrengthenAss[idx].Item.s.Name <> '' then begin
  ...
  with Sender as TDGrid do begin
    ...
    if idx in [Low(FStrengthenLevel)..High(FStrengthenLevel)] then begin
      if not g_boItemMoving then begin
        ...
        if FStrengthenItem.Item.s.Name = '' then begin
          FrmDlg.CancelItemMoving;
          FrmDlg.DMessageDlg('Please put in equipment to strenghthen.', []);
          exit;
        end;
        if (tm_MakeStone = g_MovingItem.Item.s.StdMode) and (g_MovingItem.Item.s.Shape = 0) then begin
          if not CheckItemArmStrengthenLevel(FStrengthenItem.Item.UserItem.Value.StrengthenInfo.btStrengthenCount,
            ...
            FrmDlg.CancelItemMoving;
            FrmDlg.DMessageDlg('Can not strengthen this item.', []);
            exit;
          end;
          if FStrengthenLevel[idx].Item.s.Name <> '' then begin
            AddItemBag(FStrengthenLevel[idx].Item, FStrengthenLevel[idx].Index2);
...
begin
  ...
  if dbtnBoxGetItem.Caption = 'Close' then begin
    if (OpenBoxIndex < 4) and (g_MySelf.m_nGold >= OpenBoxGold) and (g_MySelf.m_nGameGold >= OpenBoxGameGold) then begin
      if mrYes <> FrmDlg.DMessageDlg('You still have ' + IntToStr(4 - OpenBoxIndex) + ' more trys, are you sure you want to exit?', [mbYes, mbNo]) then begin
        exit;
      end;
    end;
    ...
    if (OpenBoxIndex = 0) and (g_MySelf.m_nGold >= OpenBoxGold) and (g_MySelf.m_nGameGold >= OpenBoxGameGold)  then begin
      if mrYes <> FrmDlg.DMessageDlg('You have 4 trys, would you like to recieve award?', [mbYes, mbNo]) then begin
        exit;
      end;
    end;
  ...
  with Sender as TDControl do begin
    ...
    if Tag in [Low(OpenBoxItems)..High(OpenBoxItems)] then begin
      ...
      if (OpenBoxItems[Tag].ItemType <> bit_None) and (OpenBoxSelectIndex = Tag) then begin
        if ((FrmMain.m_CurrentTick - dwndBox.AppendTick) mod 300 > 150) and (not OpenBoxMove) then
  ...
  with Sender as TDButton do begin
    ...
    if Tag in [Low(OpenBoxItems)..High(OpenBoxItems)] then begin
      case OpenBoxItems[Tag].ItemType of
        ...
        bit_BindGold: begin
            DScreen.ShowHint(nHintX, nHintY, 'Private Gold:' + IntToStr(OpenBoxItems[Tag].Item.UserItem.MakeIndex),
              clwhite, FALSE, Tag);
          end;
        bit_GameGold: begin
...
begin
  ...
  OpenCanStop := FALSE;
  OpenBoxIndex := nIndex;
  OpenBoxMoveTick := FrmMain.m_CurrentTick;
  OpenBoxMoveIndex := 0;
      ...
      if (OpenBoxGold > 0) or (OpenBoxGameGold > 0) then begin
        ...
        if (OpenBoxGameGold > 0) then begin
          ...
          if g_MySelf.m_nGameGold >= OpenBoxGold then sMsg := sMsg + '\'
            ...
            sMsg := sMsg + ' (You do not have enough ' + g_sGameGoldName + '!)\';
            boNotGold := TRUE;
          end;
        end;
...
begin
  if (FrmMain.m_CurrentTick - FMissionLogoutTick) < 2000 then exit;
  if DTrvwMission.Select <> nil then begin
    ...
    if ClientMissionInfo <> nil then begin
      if ClientMissionInfo.ClientMission.wLogoutIdx > 0 then begin
        if mrYes = FrmDlg.DMessageDlg('Are you sure you want to cancel task?[' + ClientMissionInfo.ClientMission.sMissionShowName + ']?', [mbYes, mbNo]) then begin
          FrmMain.SendClientMessage(CM_CLEARMISSION, ClientMissionInfo.ClientMission.wLogoutIdx, 0, 0, 0, '');
          FMissionLogoutTick := FrmMain.m_CurrentTick;
        end;
      end;
    end;
  end;
...
begin
  with Sender as TDControl do begin
    ...
    if Sender = dbtnMissionTrack then begin
      ShowMsg := 'The right side of screen displays incomplete tasks.';
    end;
    if ShowMsg <> '' then
  ...
  with Sender as TDWindow do begin
    ...
    with g_DXCanvas do begin
      if Sender = DDGSSetup then begin
        ...
        TextOut(ax + 22, ay + 18, clYellow, 'Graphics Settings');
        TextOut(ax + 258, ay + 18, clYellow, 'Graphics Options');
        TextOut(ax + 22, ay + 108, clYellow, 'Volume Settings');
        TextOut(ax + 258, ay + 108, clYellow, 'Audio Options');

        TextOut(ax + 5, ay + 40, $C0C0C0, 'Monitor:');
        TextOut(ax + 5, ay + 60, $C0C0C0, 'Resolution:');
        TextOut(ax + 5, ay + 80, $C0C0C0, 'Colour Depth:');

        if g_FScreenMode <> DDGSXY.ItemIndex then
          TextOut(ax + 134, ay + 80, $0000FF, 'Must exit game for settings to take effect.');

        if g_nInterface <> g_nInterfaceState then
          TextOut(ax + 134, ay + 80, $0000FF, 'Must exit game for settings to take effect.');

        TextOut(ax + 5, ay + 130, $C0C0C0, 'BGM:');
        TextOut(ax + 5, ay + 150, $C0C0C0, 'Game Sound:');
      end else
      ...
      if Sender = DDGProtect then begin
        TextOut(ax + 22, ay + 18, clYellow, 'AutoPot Settings');
       ...
       // TextOut(ax + 127, ay + 142, clSilver, 'HP');
        TextOut(ax + 235, ay + 46, clSilver, 'Seconds');
        TextOut(ax + 235, ay + 70, clSilver, 'Seconds');
        TextOut(ax + 235, ay + 94, clSilver, 'Seconds');
        TextOut(ax + 235, ay + 118, clSilver, 'Seconds');
        //TextOut(ax + 235, ay + 142, clSilver, 'Second');
      end else
      if Sender = DDItems then begin
        TextOut(ax + 22, ay + 8, clYellow, 'Name');
        TextOut(ax + 184, ay + 8, clYellow, 'DisplayImg');
        TextOut(ax + 260, ay + 8, clYellow, 'Automatic');
        TextOut(ax + 336, ay + 8, clYellow, 'DisplayName');

        TextOut(ax + DDitemFind.Left + 3, ay + DDitemFind.Top + 2, $808080, '[Enter items to search]');
      ...
      if Sender = DDMagic then begin
        ...
        {TextOut(ax + 22, ay + 132, clYellow, 'Automatic Practise');
        TextOut(ax + 138, ay + 154, clSilver, 'Second');
        TextOut(ax + 294, ay + 132, clYellow, 'Hang up skills');    }
      end else
    ...
    with g_DXCanvas do begin
      if Sender = DDItems then begin
        if DDitemFind.Text = '' then
          TextOut(ax + DDitemFind.Left + 3, ay + DDitemFind.Top + 2, $808080, '[Enter item to search]');
      end;
    end;
  end;
end;
...
begin
  with Sender as TDControl do begin
    ...
    if Sender = DDMYS then begin
      ShowMsg := 'Enable or disable Magic Lock for Healing Skill';    //使用治愈术是否锁定治愈目标
    end else
    if Sender = DDFHideHelmet then begin
      ShowMsg := 'Show or Hide Characters Helmets';
    end else
    if Sender = DDFAllyHum then begin
      ShowMsg := 'Select to toggle Ally names (<BLUE/FCOLOR=$FF0000> names).\';
      ShowMsg := ShowMsg + 'Example: In war, can quickly distinguish hostile characters.';
    end else
    if Sender = DDFAroundHum then begin
      ShowMsg := 'Will hide display around character, and improves game speed\';
      ShowMsg := ShowMsg + 'Example: In sage zone can quickly locate NPC.';
    end else
    if Sender = DDFCtrl then begin
      ShowMsg := 'Check to display ground items names.\';
      ShowMsg := ShowMsg + 'Use <N/FCOLOR=$FFFF> key to change setting during gameplay.';
    end else
    if Sender = DDFHPShow then begin
      ShowMsg := 'Show or hide Damage indicators';
    end else
    if Sender = DDPickupAllItem then begin
      ShowMsg := 'Will pick up all ground items.\';
      ShowMsg := ShowMsg + 'Use shortcut <TAB/FCOLOR=$FFFF> to quickly pick items.';
    end;
    if ShowMsg <> '' then
      DScreen.ShowHint(x, y, ShowMsg, clWhite, FALSE, Integer(Sender));
  end;
...
begin
  ...
  if boVisible then begin
    ...
    if (g_nInterface = 2) and (g_nInterfaceState = 2) then DDGSInterface.Checked := TRUE;
    if (g_nInterface = 2) and (g_nInterfaceState = 1) then DDGSInterface.Checked := FALSE;
    if (g_nInterface = 1) and (g_nInterfaceState = 2) then DDGSInterface.Checked := TRUE;
    if (g_nInterface = 1) and (g_nInterfaceState = 1) then DDGSInterface.Checked := FALSE;
...
begin
  FrmDlg.DMessageDlg('Enter Character name to add to Guild:', [mbOk, mbAbort]);
...
begin
  if mrOk = FrmDlg.DMessageDlg(' Other guild must allow alliance.\' +
    ' Leaders must face each other.\' +
    'Are you sure you want to Ally?', [mbOk, mbCancel]) then
...
begin
  if (GuildAllySelectIndex > -1) and (GuildAllySelectIndex < GuildAllyList.Count) then begin
    ...
    if FrmDlg.DMessageDlg('Are you sure you want to cancel [' + sGuildName + ']alliance?', [mbOk, mbCancel]) = mrOk then begin
      frmMain.SendSay(g_Cmd_AuthCancel + ' ' + sGuildName);
    end;
  end
  else if FrmDlg.DMessageDlg('Please enter guild you wish to break alliance with:', [mbOk, mbCancel, mbAbort]) = mrOk then begin
...
begin
  ...
  if (GuildSelectIndex > -1) and (GuildSelectIndex < MemberList.Count) then begin
    ...
    FrmDlg.DMessageDlg('Enter Character name to remove from Guild', [mbOk, mbAbort]);
    UserName := FrmDlg.DlgEditText;
  end;
  if UserName <> '' then begin
    if CompareText(UserName, g_MySelf.m_UserName) = 0 then begin
      FrmDlg.DMessageDlg('[Message] You can not remove yourself!', []);
      exit;
    end;
    for I := 0 to GuildMemberList.Count - 1 do begin
      GuildMemberInfo := pTGuildMemberInfo(MemberList.Objects[I]);
      if CompareText(GuildMemberInfo.UserName, UserName) = 0 then begin
        if mrYes = FrmDlg.DMessageDlg('Are you sure you want to remove [' + UserName + ']?', [mbYes, mbNo]) then begin
          frmMain.SendGuildDelMem(UserName);
        end;
        exit;
      end;
    end;
    FrmDlg.DMessageDlg('[Message] ' + UserName + '  not members of bank!', []);
  end;
end;
...
begin
  if Sender = DGDEditFind then begin
    ...
    GuildSaveNoticeTick := FrmMain.m_CurrentTick + 10000;
    Inc(g_GuildIndex[1]);
  end
    ...
    if DMemoGuildMember.Visible then begin
      ...
      GuildSaveMemberTick := FrmMain.m_CurrentTick + 10 * 1000;
      DGDEditNoticeClick(DGDEditGrade, 0, 0);
    end
    else begin
      frmMain.SendGuildMemberList(g_GuildMemberIndex);
      GuildRefMemberTick := FrmMain.m_CurrentTick + 60 * 1000;
    end;
  end;
end;
...
var
  ...
  valstr: string;
begin
  FrmDlg.DMessageDlg('Please enter ' + g_sGoldName + 'quantity, must be above 100000', [mbOk, mbAbort]);
  ...
  if (DGold <= g_ClientGuildInfo.nGuildMoney) and (DGold > 99999) then begin
    //FrmMain.SendClientMessage(CM_GUILDGOLDCHANGE, DGold, 0, 0, 2, ''); //this distributes money to everyone online
    FrmMain.SendClientMessage(CM_GUILDGOLDCHANGE, DGold, 0, 0, 1, ''); //this takes money to leader
  end;
end;
...
var
  ...
  valstr: string;
begin
  FrmDlg.DMessageDlg('Please enter ' + g_sGoldName + ' amount to deposit, must be above 10000', [mbOk, mbAbort]);
  GetValidStrVal(Trim(FrmDlg.DlgEditText), valstr, [' ']);
  DGold := StrToIntDef(valstr, 0);
...
begin
  with Sender as TDControl do begin
    ...
    if Sender = DGDUpLevel then begin
      ShowMsg := 'To upgrade to next level, requirements are:\';
      ShowMsg := ShowMsg + 'Guild Funds:' + IntToStr(g_ClientGuildInfo.nLevelGuildMoney) + '\';
      ShowMsg := ShowMsg + 'Construction Value:' + IntToStr(g_ClientGuildInfo.nLevelBuildPoint) + '\';
      ShowMsg := ShowMsg + 'Flourish Value:' + IntToStr(g_ClientGuildInfo.nLevelFlourishingPoint) + '\';
      ShowMsg := ShowMsg + 'Stability Value:' + IntToStr(g_ClientGuildInfo.nLevelStabilityPoint) + '\';
      ShowMsg := ShowMsg + 'Activity Value:' + IntToStr(g_ClientGuildInfo.nLevelActivityPoint) + '\';
      ShowMsg := ShowMsg + 'Guild Bonus after Level Up\';
      ...
      ShowMsg := ShowMsg + 'Experience increased:' + IntToStr(nCount) + '%\';
      ShowMsg := ShowMsg + 'Damage Increased:' + IntToStr(nCount) + '%\';
      ShowMsg := ShowMsg + 'Increase Guild Member Limit:5\';
    end else
    if Sender = DGDGetMoney then begin
      ShowMsg := 'Withdraw guild funds.';
    end else
    if Sender = DMakeMagicxAutoMake then begin
      ShowMsg := 'Turning on will build the objective item according\to the ammount of materials placed in the box.';
    end;
    if ShowMsg <> '' then
      DScreen.ShowHint(x, y, ShowMsg, clWhite, FALSE, Integer(Sender));
  end;
...
begin
 ...
 { Reg := TRegistry.Create;
  ...
  if DDGSBits.ItemIndex = 1 then boBITDEPTH := TRUE else boBITDEPTH := FALSE;
  if DDGSWindow.Checked then g_boFullScreen := FALSE else g_boFullScreen := TRUE;
  if DDGSInterface.Checked then g_nInterfaceState := 2 else g_nInterfaceState := 1;
  ...
  if ini <> nil then begin
    ini.WriteBool('Options', 'FullScreen', g_boFullScreen);
    ini.WriteInteger('Options', 'InterfaceType', g_nInterfaceState);
    ini.WriteInteger('Options', 'ScreenMode', DDGSXY.ItemIndex);
    ini.WriteBool('Options', 'BitDepth', boBITDEPTH);
    ini.WriteInteger('Options', 'MP3Volume', g_btMP3Volume);
    ini.WriteInteger('Options', 'SoundVolume', g_btSoundVolume);
    ini.WriteBool('Options', 'BGSound', g_boBGSound);
    ini.WriteBool('Options', 'Sound', g_boSound);
    //Options Tab
    ini.WriteBool('Basic Settings', 'DropItemsShow', g_SetupInfo.boShowItemName);
    ini.Free;
  end;
...
{  if g_boEasyNotShift then begin
    ...
    if g_boShiftOpen then DScreen.AddSysMsg('[Auto Shift On]', cllime)
    else DScreen.AddSysMsg('[Auto Shift Off]', cllime);
  end else begin
    if not g_SetupInfo.boExemptShift then begin
      if g_boShiftOpen then
        DScreen.AddSysMsg('[Auto Shift Off]', cllime);
      g_boShiftOpen := FALSE;
    end;
  end;           }

  if g_FScreenMode <> DDGSXY.ItemIndex then
    DScreen.AddSysMsg('[Resolution settings changed, please exit and re-enter game to apply.]', cllime);

  if g_nInterface <> g_nInterfaceState then
    DScreen.AddSysMsg('[Interface settings changed, please exit and re-enter game to apply.]', cllime);
 ...
 // DDMCS
  ...
  {if FrmMain.m_CurrentTick > g_dwChangeGroupModeTick then begin
    g_dwChangeGroupModeTick := FrmMain.m_CurrentTick + 500; //timeout 5檬
...
begin
  ...
  if DDItems.Visible then begin
    if mrYes = FrmDlg.DMessageDlg('Restore items filter to default settings?', [mbYes, mbNo]) then begin
      ClearItemShowList;
  ...
  with Sender as TDWindow do begin
    ...
    with g_DXCanvas do begin
      if Sender = DGuildLogDlg then begin
         TextOut(ax + 100, ay + 8, clWhite, 'Noticeboard');
        if DMemoGuildNotice.ReadOnly then begin
          DGDEditNoticeSave.Visible := FALSE;
          DGDEditNotice.SetImgIndex(g_WMain99Images, 2352);
          DGDEditNotice.Caption := 'Edit';
          DMemoGuildNotice.DFColor := $00B5C79C;
        end
        else begin
          DGDEditNoticeSave.Visible := TRUE;
          DGDEditNoticeSave.Enabled := FrmMain.m_CurrentTick > GuildSaveNoticeTick;
          DGDEditNotice.SetImgIndex(g_WMain99Images, 2352);
          DGDEditNotice.Caption := 'Cancel';
          DMemoGuildNotice.DFColor := clWhite;
        end;
        DGDEditNotice.Enabled := (g_ClientGuildInfo.btMyRank = 1) and (FrmMain.m_CurrentTick > GuildSaveNoticeTick);
        DGDEditNotice.Visible := g_ClientGuildInfo.btMyRank = 1;
      end
      else if Sender = DGuildInfoDlg then begin
      TextOut(ax + 104, ay + 14, clWhite, 'Information');
      TextOut(ax + 104, ay + 147, clWhite, 'Guild Stats');
      TextOut(ax + 104, ay + 292, clWhite, 'Guild Bonus');
        ...
        TextOut(ax + 9, ay + 47, clWhite, 'Name:');
        TextOut(ax + 9, ay + 63, clWhite, 'Leader:');
        TextOut(ax + 9, ay + 79, clWhite, 'Members:');
        TextOut(ax + 9, ay + 100, clWhite, 'Creator:');
        TextOut(ax + 9, ay + 116, clWhite, 'Created:');

        TextOut(ax + 78, ay + 47, clWhite, g_ClientGuildInfo.sGuildName);
        TextOut(ax + 78, ay + 63, clWhite, GuildMasterName);
        TextOut(ax + 78, ay + 79, clWhite, Format('%d/%d Online(%d)', [GuildMemberList.Count, g_ClientGuildInfo.btMaxMeberCount, GuildOnLineMemberList.Count]));
        TextOut(ax + 78, ay + 100, clWhite, g_ClientGuildInfo.sCreateName);
        TextOut(ax + 78, ay + 116, clWhite, FormatDateTime('YYYY MM DD ', g_ClientGuildInfo.dwCreateTime));

        TextOut(ax + 9, ay + 177, clWhite, 'Level:');
        TextOut(ax + 9, ay + 196, clWhite, 'Funds:');
        ...
        TextOut(ax + 9, ay + 215, clWhite, 'Build:');
        TextOut(ax + 9, ay + 231, clWhite, 'Flourish:');
        TextOut(ax + 9, ay + 247, clWhite, 'Stability:');
        TextOut(ax + 9, ay + 263, clWhite, 'Activity:');
        ...
        TextOut(ax + 9, ay + 322, clWhite, 'Experience Up:');
        TextOut(ax + 9, ay + 340, clWhite, 'Attack Up:');
      ...
      TextOut(ax + 110, ay + 8, clWhite, 'Alliances');
      TextOut(ax + 110, ay + 206, clWhite, 'Enemies');
...
begin
  with Sender as TDWindow do begin
    ...
    if (mbRight = Button) and (nIndex > -1) and (nIndex < SocietyList.Count) then begin
      ...
      DPopUpMemuGuild.Item.AddObject('Guild: ' + GuildSocietyInfo.sGuildName, TObject(-1));
      DPopUpMemuGuild.Item.AddObject('-', nil);
      if boAlly then begin
        if (g_ClientGuildInfo.btMyRank = 1) then
          DPopUpMemuGuild.Item.AddObject('Cancel Alliance', TObject(52))
        else
          DPopUpMemuGuild.Item.AddObject('Cancel Alliance', nil);
      end;
      DPopUpMemuGuild.Item.AddObject('Copy Leader Name', TObject(53));
      DPopUpMemuGuild.Item.AddObject('Copy Leader Name', TObject(54));
...
begin
  with Sender as TDWindow do begin
    ...
    with g_DXCanvas do begin
    TextOut(ax + 359, ay + 58, clWhite, 'Members');
    end;
    ...
    if DMemoGuildMember.Visible then begin
      DGDEditGrade.SetImgIndex(g_WMain99Images, 2352);
      DGDEditGrade.Caption := 'Cancel';
      DGDEditRefMemberList.SetImgIndex(g_WMain99Images, 2352);
      DGDEditRefMemberList.Caption := 'Save';
      DGDEditRefMemberList.Enabled := TRUE;
    end
    else begin
      DGDEditGrade.SetImgIndex(g_WMain99Images, 2352);
      DGDEditGrade.Caption := 'Edit Rank';
      DGDEditRefMemberList.SetImgIndex(g_WMain99Images, 2352);
      DGDEditRefMemberList.Caption := 'Members';
      ...
      DGDEditRefMemberList.Enabled := FrmMain.m_CurrentTick > GuildRefMemberTick;
      DGDEditGrade.Enabled := (g_ClientGuildInfo.btMyRank = 1) and (FrmMain.m_CurrentTick > GuildSaveMemberTick);
...
begin
  with Sender as TDControl do begin
    ...
    y := y - Top;
    ShowMsg := '';
    if (x >= 12) and (y >= 214) and (x <= 12 + 144) and (y <= 214 + 16) then begin
      ShowMsg := 'You can use Items to increase Guild Build points\';
    end else
    if (x >= 12) and (y >= 230) and (x <= 12 + 144) and (y <= 230 + 16) then begin
      ShowMsg := 'Every 100 monster killed by members increases Flourish Points\';
    end else
    if (x >= 12) and (y >= 246) and (x <= 12 + 144) and (y <= 246 + 16) then begin
      ShowMsg := 'Each day all active members can increase Stability Points\';
      ShowMsg := ShowMsg + '<Can only be increased once a day per member./FCOLOR=$FFFF>';
    end else
    if (x >= 12) and (y >= 262) and (x <= 12 + 144) and (y <= 262 + 16) then begin
      ShowMsg := 'Activity Points decrease every hour while the Guild is inactive\';
    end;
    if ShowMsg <> '' then
...
begin
  with Sender as TDWindow do begin
    ...
    ay := SurfaceY(Top);
    with g_DXCanvas do begin
      TextOut(ax + 8, ay + 4, clWhite, 'Rank');
      TextOut(ax + 65, ay + 4, clWhite, 'Title');
      TextOut(ax + 155, ay + 4, clWhite, 'Name');
      TextOut(ax + 250, ay + 4, clWhite, 'Status');
      if DCheckBoxShowMember.Checked then begin
        DGuildMemberUpDown.MaxPosition := GuildMemberList.Count - 17;
        MemberList := GuildMemberList;
      end
      else begin
        DGuildMemberUpDown.MaxPosition := GuildOnLineMemberList.Count - 17;
        MemberList := GuildOnLineMemberList;
      end;

      for i := DGuildMemberUpDown.Position to DGuildMemberUpDown.Position + 16 do begin
        ...
        TextOut(ax + 50, ny, FontColor, GuildMemberInfo.RankName);
        TextOut(ax + 144, ny, FontColor, GuildMemberInfo.UserName);
        if GuildMemberInfo.nTime = -1 then
          str := 'Online'
          ...
          str := IntToStr(GuildMemberInfo.nTime) + 'Days Ago'
        else if GuildMemberInfo.nTime < 60 then
          str := 'Last month'
        else if GuildMemberInfo.nTime < 90 then
          str := '2 months ago'
        else
          str := 'Over 2 months ago.';
        TextOut(ax + 266 - TextWidth(str) div 2, ny, FontColor, str);
      end;
    end;
  end;
end;
...
begin
  ...
  with Sender as TDWindow do begin
    ...
    with g_DXCanvas do begin
      TextOut(ax + 4, ay + 4, $A6A49F, 'Description of guild Ranks:');
      TextOut(ax + 4, ay + 18, $A6A49F, '#1 position for leaders.)');
      TextOut(ax + 4, ay + 32, $A6A49F, '#2,#3 Deputy leader of guild, has access to income.(Max 2)');
    end;
  end;
end;
...
begin
  with Sender as TDWindow do begin
    ...
    if (mbRight = Button) and (GuildMoveIndex > -1) and (GuildMoveIndex < MemberList.Count) then begin
      ...
      DPopUpMemuGuild.Item.AddObject('Name: ' + GuildMemberInfo.UserName, TObject(-1));
      DPopUpMemuGuild.Item.AddObject('-', nil);
      if (g_ClientGuildInfo.btMyRank = 1) and (GuildMemberInfo.UserName <> g_MySelf.m_UserName) then begin
        DPopUpMemuGuild.Item.AddObject('Kick from Guild', TObject(2));
      end
      else
        DPopUpMemuGuild.Item.AddObject('Kick from Guild', nil);

      if (GuildMemberInfo.nTime = -1) then
        DPopUpMemuGuild.Item.AddObject('Private Chat', TObject(3))
      else
        DPopUpMemuGuild.Item.AddObject('Private Chat', nil);
      if InFriendList(GuildMemberInfo.UserName) then
        DPopUpMemuGuild.Item.AddObject('Send Message', TObject(8))
      else
        DPopUpMemuGuild.Item.AddObject('Send Message', nil);
      ...
      if (GuildMemberInfo.nTime = -1) and (not InFriendList(GuildMemberInfo.UserName)) and
        ...
        DPopUpMemuGuild.Item.AddObject('Add as Friend', TObject(4))
      else
        DPopUpMemuGuild.Item.AddObject('Add as Friend', nil);

      if (GuildMemberInfo.nTime = -1) and (not InGroupList(GuildMemberInfo.UserName)) then
        DPopUpMemuGuild.Item.AddObject('Invite to Group', TObject(5))
      else
        DPopUpMemuGuild.Item.AddObject('Invite to Group', nil);
      DPopUpMemuGuild.Item.AddObject('-', nil);
      DPopUpMemuGuild.Item.AddObject('Copy Name', TObject(6));
      DPopUpMemuGuild.Item.AddObject('Copy Guild Title', TObject(7));
      DPopUpMemuGuild.RefSize;
        ...
        if (FMakeGoods.Item[idx].s.Name <> '') and (g_MovingItem.Item.s.Idx = FMakeGoods.Item[idx].s.Idx) then begin
          if (sm_Superposition in g_MovingItem.Item.s.StdModeEx) and (g_MovingItem.Item.s.DuraMax > 1) then begin
            if g_MovingItem.Item.UserItem.Dura < FMakeGoods.MakeItem.ItemArr[idx].wCount then begin
              FrmDlg.CancelItemMoving;
              FrmDlg.DMessageDlg('Incorrect number of items.', []);
              exit;
            end;
            {if g_MovingItem.Item.UserItem.Dura > FMakeGoods.MakeItem.ItemArr[idx].wCount then begin
         ...
         if (FMakeGoods.Item[idx].s.StdMode = tm_ore) and (FMakeGoods.Item[idx].UserItem.Dura > 9) then begin
            newDura := round(g_MovingItem.Item.UserItem.Dura / 1000) * 1000;
          if (FMakeGoods.Item[idx].UserItem.Dura > newDura) then begin
            FrmDlg.CancelItemMoving;
            FrmDlg.DMessageDlg('Incorrect Quality of item.', []);
            exit;
          end;
         end;
          if FMakeItemLevel[idx].Item.s.Name <> '' then begin
...
begin
  ...
  for I := 1 to 5 do begin
    if (FMakeGoods.Item[I].s.Name <> '') then begin
      if FMakeItemLevel[I].Item.s.Name = '' then begin
        FrmDlg.DMessageDlg('Not enough materials for build.', []);
        exit;
      end;
    end;
  end;
...
begin
  with DControl do begin
    case Integer(TDPopUpMemu(Sender).Item.Objects[ItemIndex]) of
      ...
      52: begin
          if FrmDlg.DMessageDlg('Are you sure you want to cancel alliance with [' + UserName + ']?', [mbOk, mbCancel]) = mrOk then begin
...
const
  JobName: array[0..3] of string[8] = ('Warrior', 'Wizard', 'Taoist', 'Assassin');
  SexName: array[0..1] of string[7] = ('Male', 'Female');
var
  ax, ay: integer;
  sTitle: string;
  I, n: Integer;
...
begin
  with Sender as TDWindow do begin
    ...
    with g_DXCanvas do begin
      ...
      TextOut(ax + 41, ay + 90, $33FFCC, 'Ranking');
      TextOut(ax + 93, ay + 90, $33FFCC, 'Character Names');
      TextOut(ax + 173, ay + 90, $33FFCC, 'Job');
      TextOut(ax + 243 - TextWidth(sTitle) div 2, ay + 90, $33FFCC, sTitle);

      TextOut(ax + 41 + 251, ay + 90, $33FFCC, 'Ranking');
      TextOut(ax + 93 + 251, ay + 90, $33FFCC, 'Character Names');
      TextOut(ax + 173 + 251, ay + 90, $33FFCC, 'Job');
...
begin
  with Sender as TDWindow do begin
    ...
    if d <> nil then
      DrawWindow(dsurface, ax, ay, d);
    with g_DXCanvas do begin
      TextOut(ax + 36, ay + 37, $A6A49F, 'Strengthening equipment requires crystal.)');
      TextOut(ax + 36, ay + 100, $A6A49F, 'Strenthening equipment auxillary materials(hold)');
      TextOut(ax + 176, ay + 182, $A6A49F, FStrengthenHint);

      TextOut(ax + 29, ay + 234, $A6A49F, 'Strengthening Cost:');
      TextOut(ax + 29 + 60, ay + 234, clWhite, IntToStr(FStrengthenMoney) + g_sGoldName + ' （Priority Use ' + g_sBindGoldName + '）');

      TextOut(ax + 29, ay + 260, $A6A49F, 'Probability of Success:');
      TextOut(ax + 29 + 60, ay + 260, clWhite, IntToStr(FStrengthenRate) + '%/' + IntToStr(FStrengthenMaxRate) + '%');

      TextOut(ax + 162, ay + 260, $A6A49F, 'Downgrade probability:');
    ...
    if FStrengthenShowEffect then begin
      if FrmMain.m_CurrentTick > FStrengthenShowTick then begin
        FStrengthenShowTick := FrmMain.m_CurrentTick + 150;
      ...
      if FrmMain.m_CurrentTick > FStrengthenShowTick then begin
        ...
        FStrengthenShowTick := FrmMain.m_CurrentTick + 100;
        Inc(FStrengthenSchedulePosition);
      end;
      FStrengthenHint := 'Ongoing strenghtening...';
      ...
      if FStrengthenSchedulePosition >= 100 then begin
        ...
        case FStrengthenLevelType of
          1: begin //成功
              FStrengthenHint := 'Congratulations, strengthen was successful.';
          ...
          2: begin //破碎
              FStrengthenHint := 'Equipment Broke';
              SafeFillChar(FStrengthenItem, SizeOf(FStrengthenItem), #0);
              FStrengthenShowEffect := TRUE;
          ...
          3: begin //降级
              FStrengthenHint := 'Reduced the level';
              FStrengthenItem.Item.UserItem := FStrengthenUseItem;
              ClearStrengthenInfoEx;
              FStrengthenMoney := GetStrengthenMoney(FStrengthenItem.Item.UserItem.Value.StrengthenInfo.btStrengthenCount);
              FStrengthenMaxRate := GetStrengthenMaxRate(FStrengthenItem.Item.UserItem.Value.StrengthenInfo.btStrengthenCount);
...
begin
  ...
  FStrengthenHint := 'To strengthen equipment';
  if boVisible then begin
...
begin
  with Sender as TDWindow do begin
    ...
    if OpenBoxMove then begin
      if (FrmMain.m_CurrentTick > OpenBoxMoveTick) then begin
        ...
        if OpenBoxStop then begin
          ...
          if OpenCanStop then begin
            if OpenBoxMoveIndex < 5 then OpenBoxMoveTick := FrmMain.m_CurrentTick + LongWord(OpenBoxMoveIndex * 100)
            else begin
              OpenBoxStop := FALSE;
              OpenBoxMove := FALSE;
              OpenCanStop := FALSE;
              dbtnBoxNext.Caption := 'Please Recieve Awards';
              dbtnBoxGetItem.Caption := 'Recieve Awards';
            end;
          end else  OpenBoxMoveTick := FrmMain.m_CurrentTick + 50
        end else begin
          if OpenBoxMoveIndex < 4 then OpenBoxMoveTick := FrmMain.m_CurrentTick + LongWord((5 - OpenBoxMoveIndex) * 100)
          else OpenBoxMoveTick := FrmMain.m_CurrentTick + 50
        end;
      end;
    end;
    if OpenGetItem and OpenBoxShowEffect then begin
      if FrmMain.m_CurrentTick > OpenBoxEffectTick then begin
        OpenBoxEffectTick := FrmMain.m_CurrentTick + 150;
      ...
      if OpenBoxEffectIdx > 19 then begin
        ...
        OpenGetItem := FALSE;
        OpenGetItemIndex := 0;
        dbtnBoxNext.Caption := 'Continue to turn wheel';
...
begin
  with Sender as TDWindow do begin
    ...
    boMake := TRUE;
    with g_DXCanvas do begin
      TextOut(ax + 190, ay + 37, $A6A49F, 'Required Items: ');
      TextOut(ax + 190, ay + 100, $A6A49F, 'Auxiliary materials to create items (hold): ');
      if FMakeMagicShow and (g_MySelf.m_Abil.Level < FMakeGoods.wLevel) then begin
        TextOut(ax + 330, ay + 174, clRed, 'Level is not high enough.');
        TextOut(ax + 330, ay + 189, clRed, 'Level required: ' + IntToStr(FMakeGoods.wLevel));
        boMake := FALSE;
      end else
      ...
      TextOut(ax + 195, ay + 222, $A6A49F, 'Build Cost: '); //FMakeItemRate
      TextOut(ax + 195, ay + 248, $A6A49F, 'Probability of success: ');

      if FMakeGoods.Item[0].s.name <> '' then begin
        TextOut(ax + 250, ay + 222, clWhite, IntToStr(FMakeGoods.MakeItem.nMoney) + ' ' + g_sGoldName + '/' + g_sBindGoldName);
        if FMakeItemRate < 0 then
          TextOut(ax + 305, ay + 248, clWhite, 'Ore Required..')
        else
          TextOut(ax + 305, ay + 248, clWhite,
    ...
    if FMakeItemShowSchedule then begin
      ...
      if FMakeItemSchedulePosition < 6 then begin
        ...
        if d <> nil then
          DrawBlend(dsurface, ax + 179, ay - 35, d, 1);
      ...
      if FMakeItemSchedulePosition >= 100 then begin
        ...
        if FMakeItemShowEffect then begin
          FMakeItemHint := 'Build Successfull';
          //SafeFillChar(FMakeItemItem, SizeOf(FMakeItemItem), #0);
          FMakeItemItem.Index2 := -1;
          FMakeItemItem.ItemType := mtBagItem;
          FMakeItemItem.Item.UserItem := FMakeItemUseItem;
          FMakeItemItem.Item.s := GetStditem(FMakeItemUseItem.wIndex, g_Myself.m_Abil.Level, g_Myself.m_btJob);
          FMakeItemShowTick := FrmMain.m_CurrentTick;
          FMakeItemSchedulePosition := 0;
          PlaySoundEx(bmg_ItemLevel_OK);
        end
        else begin
          FMakeItemHint := 'Build Failed';

          SafeFillChar(FMakeItemItem, SizeOf(FMakeItemItem), #0);
          if FMakeItemBackItem.Item.s.name <> '' then begin
            FMakeItemItem := FMakeItemBackItem;
            FMakeItemBackItem.Item.s.name := '';
            FMakeItemHint2 := 'Returned:' + FMakeItemItem.Item.s.name;
          end
      ...
      if FrmMain.m_CurrentTick > FMakeItemShowTick then begin
        FMakeItemShowTick := FrmMain.m_CurrentTick + 150;
        Inc(FMakeItemSchedulePosition);
      end;
...
begin
  with Sender as TDWindow do begin
    ...
    if d <> nil then
      DrawWindow(dsurface, ax, ay, d);
    with g_DXCanvas do begin
      str := 'Active Quests(' + IntToStr(g_MissionInfoList.Count) + '/' + IntToStr(High(TMissionInfos)) + ')';
      TextOut(ax + 79 - TextWidth(str) div 2, ay + 61, clWhite, str);

      if ShowCanAccept then TextOut(ax + 210, ay + 61, clWhite, 'Available Quests')
      else TextOut(ax + 214, ay + 61, clWhite, 'Quest Details');
    end;
    dbtnMissionAccept.Visible := not ShowCanAccept;
    dbtnMissionTrack.Visible := FALSE;
    dbtnMissionLogout.Visible := FALSE;
    dbtnMissionTrack.Caption := 'Show Quest';
    if DTrvwMission.Select <> nil then begin
      ClientMissionInfo := pTClientMissionInfo(DTrvwMission.Select.Item);
      if ClientMissionInfo <> nil then begin
        if ClientMissionInfo.ClientMission.wLogoutIdx > 0 then begin
        ...
        if ClientMissionInfo.MissionInfo.boTrack then
          dbtnMissionTrack.Caption := 'Hide Quest';
...
begin
  ...
  GuildSaveNoticeTick := FrmMain.m_CurrentTick;
  GuildSaveMemberTick := FrmMain.m_CurrentTick;
  GuildRefMemberTick := FrmMain.m_CurrentTick;
  GuildMasterName := '';
  ...
  if d <> nil then begin
    ...
    DGuildDlg.Left := (g_FScreenWidth - d.Width) div 2 - 20;
    DGuildDlg.Top := (g_FScreenHeight - d.Height) div 2 - 50;
  end;
  ...
  DGDEditGrade.Top := 379;
  DGDEditGrade.OnDirectPaint := FrmDlg2.DEMailReadDirectPaint;
  DGDEditRefMemberList.SetImgIndex(g_WMain99Images, 2352);
  DGDEditRefMemberList.Left := 312;
  DGDEditRefMemberList.Top := 379;
  DGDEditRefMemberList.OnDirectPaint := FrmDlg2.DEMailReadDirectPaint;
  DGDAddMem.SetImgIndex(g_WMain99Images, 2352);
  DGDAddMem.Left := 312;
  DGDAddMem.Top := 398;
  DGDAddMem.OnDirectPaint := FrmDlg2.DEMailReadDirectPaint;
  DGDDelMem.SetImgIndex(g_WMain99Images, 2352);
  DGDDelMem.Left := 381;
  DGDDelMem.Top := 398;
  DGDDelMem.OnDirectPaint := FrmDlg2.DEMailReadDirectPaint;

  DCheckBoxShowMember.SetImgIndex(g_WMain99Images, 151);
  DCheckBoxShowMember.Left := 479;
  DCheckBoxShowMember.Top := 375;

  DGDEditFind.SetImgIndex(g_WMain99Images, 2324);
  DGDEditFind.Left := 580;
  DGDEditFind.Top := 394;
  DGDEditFind.OnDirectPaint := FrmDlg2.DEMailReadDirectPaint;//DGDAddMemDirectPaint;
  ...
  DGDEditNoticeSave.SetImgIndex(g_WMain99Images, 2352);
  DGDEditNoticeSave.Left := 69;
  DGDEditNoticeSave.Top := 348;
  DGDEditNoticeSave.OnDirectPaint := FrmDlg2.DEMailReadDirectPaint;
  DGDEditNotice.SetImgIndex(g_WMain99Images, 2352);
  DGDEditNotice.Left := 6;
  DGDEditNotice.Top := 348;
  DGDEditNotice.OnDirectPaint := FrmDlg2.DEMailReadDirectPaint;
  ...
  DGDAllyAdd.SetImgIndex(g_WMain99Images, 2352);
  DGDAllyAdd.Left := 5;
  DGDAllyAdd.Top := 174;
  DGDAllyAdd.OnDirectPaint := FrmDlg2.DEMailReadDirectPaint;
  DGDBreakAlly.SetImgIndex(g_WMain99Images, 2352);
  DGDBreakAlly.Left := 76;
  DGDBreakAlly.Top := 174;
  DGDBreakAlly.OnDirectPaint := FrmDlg2.DEMailReadDirectPaint;

  //行会成员
  DGuildMemberDlg.Left := 313;
  DGuildMemberDlg.Top := 80;
  DGuildMemberDlg.Width := 309;
  DGuildMemberDlg.Height := 290;

  DGuildMemberUpDown.SetImgIndex(g_WMain99Images, 120);
  DGuildMemberUpDown.Top := 21;//89 - DGuildMemberDlg.Top;
  DGuildMemberUpDown.Left := 296;//607 - DGuildMemberDlg.Left;
  ...
  DDGSInterface.SetImgIndex(g_WMain99Images, 151);
  DDGSInterface.Left := 268;
  DDGSInterface.Top := 58;
...
begin
  ...
  //Removed these as Users can add there own Trees Server Side...
  {DTreeNodes := DTrvwMission.GetTreeNodes(nil, 'Random Tasks', TRUE);
  if DTreeNodes <> nil then begin
    DTreeNodes.boMaster := TRUE;
    DTreeNodes.boOpen := TRUE;
  end;
  DTreeNodes := DTrvwMission.GetTreeNodes(nil, 'Cycled Tasks', TRUE);
  if DTreeNodes <> nil then begin
    DTreeNodes.boMaster := TRUE;
    DTreeNodes.boOpen := TRUE;
  end;}
