  ...
  try
    for I := 0 to IMAGESCOUNT - 1 do begin
      if m_ImgArr[I].Surface <> nil then begin
        if (FrmMain.m_CurrentTick - m_ImgArr[I].dwLatestTime) > 5 * 60 * 1000 then begin
...
begin
  ...
  FDxTextureArr[index].dwLatestTime := FrmMain.m_CurrentTick;
end;
...
begin
  ...
  try
    if FileExists(g_UibFiles[index]) then begin
      ...
      m_ImgArr[index].dwLatestTime := FrmMain.m_CurrentTick;
