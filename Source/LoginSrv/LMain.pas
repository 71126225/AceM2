...
uses
  ...
  Buttons, IniFiles, MudUtil, Menus, Grobal2, LSShare, SDK, Common, AppEvnts, ADODB;
type
  TConnInfo = record
    sAccount: string;
    sIPaddr: string;
...
begin
  ...
  Config := @g_Config;
  SendGameCenterMsg(SG_STARTNOW, 'Starting LoginSrv...');
  MainOutMessage('Gate Loading...');
  Application.ProcessMessages;

{$IF RUNVAR = VAR_DB}
  AccountDB := TFileIDDB.Create(Config.sSQLInstance, Config.sSQLLoginServer);
{$IFEND}
  ...
  GSocket.Active := true;
{$IF RUNVAR = VAR_SQL}
  MainOutMessage('Connected to SQL Server...');
  FrmSqlSock.SqlSocket.Active := true;
{$IFEND}
  ExecTimer.Enabled := true;

  MainOutMessage('Configuration Loaded......');
  SendGameCenterMsg(SG_STARTOK, 'LoginSrv started successfully...');
end;
      ...
      for i := 0 to ServerList.Count - 1 do begin
        ...
        if sServerName <> '' then begin
          ...
          if TickTime < 30000 then
            ...
            MonitorGrid.Cells[3, I + 1] := 'Time out';
...
var
  ...
  sSQLString: String;
resourcestring
  sAddNewuserFail = '[Error creating new character] %s/%s';
  sLogFlag = 'new';
begin
  try
    nErrCode := -1;
    ...
    if (sUserEntryMsg <> '') and (sUserAddEntryMsg <> '') then begin
      ...
      if bo21 then begin
        try
          ...
          MainOutMessage('AccountCreate: An error has occured trying to process an account');
        end;
      end
      else begin
  ...
  try
    ...
    try
      if {AccountDB.Open and} (length(sNewPassword) >= 3) then begin
        ...
        if (n10 >= 0) and (AccountDB.Get(sLoginID, DBRecord)) then begin
      ...
      MainOutMessage('AccountChangePassword: An error has occured trying to process an account');
    end;
...
begin
  ...
  try
    if (sAccount <> '') {and AccountDB.Open} then begin
      ...
      if (nIndex >= 0) and (AccountDB.Get(sAccount, DBRecord)) then begin
        if (DBRecord.nErrorCount < 5) or ((GetTickCount - DBRecord.dwActionTick) > 180000) then begin
          nCode := -1;
    ...
    MainOutMessage('AccountGetBackPassword: An error has occured trying to process an account');
  end;
  if nCode = 1 then begin
  ...
  try
    ...
    try
      //if AccountDB.Open then begin
        n10 := AccountDB.Index(sLoginID);
        if (n10 >= 0) then begin
          boAccountExists := AccountDB.Get(sLoginID, DBRecord);
          if (boAccountExists) then begin
      ...
      MainOutMessage('AccountLogin: An error has occured trying to process an account');
    end;
...
{$IF RUNVAR = VAR_DB}
      ...
      CM_VALIDATECLIENTCREATION: begin
        case DefMsg.Recog of
          0: begin //account creation sam
            if Config.nDisableClientCreation > 0 then
              DefMsg := MakeDefaultMsg(SM_VALIDATECLIENTCREATION, 1, 0, 0, 0)
            else
              DefMsg := MakeDefaultMsg(SM_VALIDATECLIENTCREATION, 0, 0, 0, 0);
            SendGateMsg(GateIndex, UserInfo.sArryIndex, UserInfo.sSockIndex, EncodeMessage(DefMsg) + EncodeString(Config.sAccountURL));
          end;
          1: begin
            if Config.nDisableClientCreation > 0 then
              DefMsg := MakeDefaultMsg(SM_VALIDATECLIENTCREATION, 3, 0, 0, 0)
            else
              DefMsg := MakeDefaultMsg(SM_VALIDATECLIENTCREATION, 2, 0, 0, 0);
            SendGateMsg(GateIndex, UserInfo.sArryIndex, UserInfo.sSockIndex, EncodeMessage(DefMsg) + EncodeString(Config.sPasswordURL));
          end;
        end;
      end;
{$IFEND}
...
begin
  ...
  Config.sSQLInstance := LoadConfigString(sSectionServer, sSQLInstance, Config.sSQLInstance);
  Config.sSQLLoginServer := LoadConfigString(sSectionServer, sSQLLoginServer, Config.sSQLLoginServer);
  Config.nDisableClientCreation := LoadConfigInteger(sSectionServer, nDisableClientCreation, Config.nDisableClientCreation);
  Config.sAccountURL := LoadConfigString(sSectionServer, sAccountURL, Config.sAccountURL);
  Config.sPasswordURL := LoadConfigString(sSectionServer, sPasswordURL, Config.sPasswordURL);
end;
