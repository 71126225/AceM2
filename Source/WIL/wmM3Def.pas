  ...
  if FileExists(idxfile) then begin
    ...
    if fhandle > 0 then begin
      ...
      if (FIdxHeader.VerFlag and $FFFF0000) = $B13A0000 then
        ...
        FNewFmt := False;
      if not FNewFmt then
        FileSeek(fhandle,sizeof(TWMIndexHeader)-8,0);
  ...
  if FFileStream.Seek(position, 0) = position then begin;
    ...
    try
      if FFileStream.Read(Buffer^, ReadSize) = ReadSize then begin
        ...
        Decode(Buffer, DecodeBuffer, imginfo.DXInfo.nWidth * 2, imginfo.DXInfo.nHeight);
        if pDXTexture.Surface <> nil then begin
        //CopyImageDataToTexture(DecodeBuffer, Texture, imginfo.DXInfo.nWidth, imginfo.DXInfo.nHeight);
          if not CopyImageDataToTexture(DecodeBuffer, pDXTexture.Surface, imginfo.DXInfo.nWidth, imginfo.DXInfo.nHeight) then
          begin
