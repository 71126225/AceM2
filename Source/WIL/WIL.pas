...
type
  ...
  TColorEffect = (ceNone, ceGrayScale, ceBright, ceRed, ceGreen, ceBlue, ceYellow, ceFuchsia, ceLightPurple, ceTeal);
...
uses
  wmM2Def, wmM2Wis, wmMyImage, wmM2Zip, wmM3Zip, wmM3Def
{$IFDEF WORKFILE}
  , wmWoool, wm521g;
  {$ELSE}
  ;
...
begin
  Result := nil;
  if (not FInitialize) or (index < 0) or (index >= FImageCount) or (FLibType <> ltUseCache) then
    exit;
