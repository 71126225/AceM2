...
function CheckEnglishChr(sName: string): Boolean;
...
function CheckClass(NeedClass, nJob: integer): Boolean;
implementation
...
begin
  ...
  for i  := 1 to Length(sName) do begin
    chr := sName[i];
    if not (Ord(Chr) in [32,48..57,65..90,97..122{, 128,161..169,192..232}]) then begin
      Result := False;
      exit;
    end;
  end;     
...
begin
  Result := True;
  if ((NeedClass shr nJob) and 1) = 1 then
    Result := False;
end;
