...
begin
  ...
  with FrmMain.OpenDialog do begin
    Filter := 'Game Plugin(DLL)|zPlugOf*.dll|标准动态链接库(DLL)|*.dll|All Files|*.*';
    Execute(Handle);
...
begin
  ...
  if (PlugInfo <> nil) and Assigned(PlugInfo.UnModule) then begin
    if Application.MessageBox(PChar(Format('Are you sure you uninstall the plug [%s]？',
      [PlugInfo.sDesc])), 'Message', MB_OKCANCEL + MB_ICONQUESTION) = IDOK then begin
