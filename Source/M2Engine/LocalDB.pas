...
uses
  ...
  Grobal2, SDK, ObjNpc, objplay, UsrEngn, IniFiles;
...
{$IFEND}
    ...
    function GetColour(name_of_colour: String): String;
    function SaveGoodRecord(NPC: TMerchant; sFile: String):Integer;
    function LoadGoodRecord(NPC: TMerchant; sFile: String): Integer;
    { Public declarations }

    function LoadMine(nMineIdx:integer): Integer;
    procedure AddAuctionItem(SellItem: pTUserShopSellItem; Player: TPlayObject); overload;
    procedure DellAuctionItem(SellItem: pTUserShopSellItem; Player: TPlayObject); overload;
    procedure UpdateAuctionItem(SellItem: pTUserShopSellItem; Player: TPlayObject); overload;
    procedure AddAuctionItem(SellItem: pTUserShopBuyItem; Player: TPlayObject); overload;
    procedure DellAuctionItem(SellItem: pTUserShopBuyItem; Player: TPlayObject); overload;
    procedure UpdateAuctionItem(SellItem: pTUserShopBuyItem; Player: TPlayObject); overload;
    procedure InitiateAuction();
    function dateForSQL(dModDate: TDateTime): string;
  end;
...
begin
  Result := False;
  try
    //Add out sql string to our ado query
    Query.SQL.Clear;
    ...
    try
      ...
      for i := 0 to Query.RecordCount - 1 do begin
        //create a new instance of an admin.
        New(AdminInfo);

        //pass it our values from sql.
        AdminInfo.nLv := Query.FieldByName('Permission').AsInteger;
        AdminInfo.sChrName := Query.FieldByName('Name').AsString;
        AdminInfo.sIPaddr := Query.FieldByName('IPAddress').AsString;;

        //commit our current instance of admin
        UserEngine.m_AdminList.Add(AdminInfo);

        //move onto a new instance of admin and repeat the loop
        Query.Next;
      end;
    finally
...
begin
  ...
  if MyFileExists(sFileName) then begin
    ...
    for i := 0 to tGuardList.Count - 1 do begin
      s14 := tGuardList.Strings[i];  //s14 = full line
      if (s14 <> '') and (s14[1] <> ';') then begin
        s14 := GetValidStrCap(s14, s1C, [' ', #9]);   //s1c = first word? mon name
        if (s1C <> '') and (s1C[1] = '"') then
          ArrestStringEx(s1C, '"', '"', s1C);
        s14 := GetValidStr3(s14, s20, [' ', #9]);  //s20 : map name
        s14 := GetValidStr3(s14, s24, [' ', #9]);  //s24 : map location
        if (s24 <> '') and (s24[1] = '[') and (s1C <> '') and (s20 <> '') then begin
          ...
          if nCount > 0 then begin
            ...
            s14 := GetValidStr3(s14, s2C, [' ', #9, ':']);//s2c : direction
  ...
  try
    try
      ...
      for i := 0 to Query.RecordCount - 1 do begin
        ...
        StdItem.ExpRate := Query.FieldByName('ExpRate').AsInteger;
        StdItem.HPorMPRate := Query.FieldByName('HporMPRate').AsInteger;
        StdItem.AC2Rate := Query.FieldByName('AC2Rate').AsInteger;
        StdItem.MAC2Rate := Query.FieldByName('MAC2Rate').AsInteger;
        StdItem.Holy := Query.FieldByName('Holy').AsInteger;
        StdItem.Deadliness := Query.FieldByName('Deadliness').AsInteger;
        StdItem.ShowInGroup := Query.FieldByName('ShowInGroup').AsInteger;
        StdItem.NeedClass := Query.FieldByName('NeedClass').AsInteger;
        ...
        if UserEngine.StdItemList.Count = Idx then begin
          ...
          Dispose(StdItem);
          Memo.Lines.Add(format('Loading items (Idx:%d Name:%s) Failed.', [Idx, StdItem.Name]));
          Result := -100;
          Exit;
        end;
        Query.Next;
...
begin
  ...
  while (I < LoadList.Count) do begin //Jason 071209修改
    if Ing > 5000 then begin
      MainOutMessage('#CALL the cycle of death ' + sLoadName);
      break;
    end;
    ...
    if (s14 <> '') and (s14[1] = '#') and (CompareLStr(s14, '#CALL', Length('#CALL'))) then begin
      ...
      if nIdx > -1 then begin
        ...
        if nCount > 100 then begin
          MainOutMessage('#CALL infinite loop ' + sLoadName + ' ' + s20 + ' ' + s18);
          break;
        end;
      ...
      if not LoadCallList(s34, s18, I + 1, LoadList) then begin
        MainOutMessage('NPC load error: ' + s20 + ' ' + s18);
      end;
      //end;// else
    ...
    if MyFileExists(sFileName) then begin
      ...
      try
          ...
          if (sStr <> '') and (sStr[1] <> ';') then begin
            if not boReadHeader then begin
              ...
              if (nType in [0..4]) and (sName <> '') and (nCount > 0) then begin
                ...
                else if CompareText(sName, 'BindGold') = 0 then begin
                  New(BoxItemInfo);
                  SafeFillChar(BoxItemInfo^, SizeOf(TBoxItemInfo), #0);
                ...
                if BoxItemInfo <> nil then begin
                  ...
                  FrmMain.MemoLog.Lines.Add('Load chest data [' + IntToStr(nIndex) + '.txt]Goods[' + sName + ']Failure.');
                  break;
                end;
              end
              else begin
        ...
        if (BoxInfo <> nil) and (boReadHeader) and (BoxInfo.ItemList[0].Count > 0) and
          ...
          FrmMain.MemoLog.Lines.Add('Load chest data[' + IntToStr(nIndex) + '.txt] Header or ItemCount Failure.');
      finally
        LoadList.Free;
      end;
    end;
...
begin
  ...
  if not FileExists(sFileName) then begin
    LoadList.Add(';Custom GM commands profile');
    LoadList.Add(';The name of the command'#9'The corresponding number');
      ...
      if (s18 <> '') and (s18[1] <> ';') then begin
        if s18[1] = '[' then begin
          ...
          if (nMoney >= 0) and (nRate > 0) and (nMaxRate > 0) and (nMaxRate >= nRate) and (StdItem <> nil) then begin
            ...
            MainOutMessage('Load makeitems failed: ' + s24 + ' Does not exist or the information is not correct.');
          end;
        end
        else begin
          if MakeItem <> nil then begin
            ...
            if (s20 <> '') then begin
              ...
              if (StdItem <> nil) and (nMoney > 0) then begin
                if ((nrate > 0) and (nrate < 6)) then begin  //only let the position be an option if it's valid!
                  if MakeItem.ItemArr[nrate].wIdent = 0 then begin
                    MakeItem.ItemArr[nrate].wIdent := StdItem.Idx + 1;
                    MakeItem.ItemArr[nrate].boNotGet := (sNotGet = '1');
                    if (sm_Superposition in StdItem.StdModeEx) and (StdItem.DuraMax > 1) then
                      MakeItem.ItemArr[nrate].wCount := nMoney
                    else if (nMoney > 9) and (StdItem.StdMode = tm_ore) then
                        MakeItem.ItemArr[nrate].wCount := nMoney //if you 'need' more then 9 and it's an ore > assume they mean puri
                    else
                      MakeItem.ItemArr[nrate].wCount := 1;
                  end;
                end else begin
                  for II := 1 to 5 do begin //if the position is invalid, fill first empty slot
                    if MakeItem.ItemArr[II].wIdent = 0 then begin
                      MakeItem.ItemArr[ii].wIdent := StdItem.Idx + 1;
                      MakeItem.ItemArr[ii].boNotGet := (sNotGet = '1');
                      if (sm_Superposition in StdItem.StdModeEx) and (StdItem.DuraMax > 1) then
                        MakeItem.ItemArr[ii].wCount := nMoney //stackable items can have an amount specified
                      else if (nMoney > 9) and (StdItem.StdMode = tm_ore) then
                        MakeItem.ItemArr[ii].wCount := nMoney //if you 'need' more then 9 and it's an ore > assume they mean puri
                      else
                        MakeItem.ItemArr[ii].wCount := 1;
                      break;
                    end;
                  end;
                end;
                ...
                MainOutMessage('Load makeitems material failed: ' + s20 + ' Does not exist or incorrect parameter.');
                Dispose(MakeItem);
                MakeItem := nil;
              end;
            end;
...
function TFrmDB.GetColour(name_of_colour: String): String;
var
  colour_config, property_of_colour, rgb: TStringList;
  path_to_config, colour: String;
  i: Integer;
begin
  colour_config := TStringList.Create;
  path_to_config := g_Config.sEnvirDir + 'ColorProperties.txt';
  try
    if not FileExists(path_to_config) then begin
      colour_config.Add('BLACK=0,0,0');
      colour_config.Add('BLUE=5,10,30');
      colour_config.Add('GREEN=5,30,10');
      colour_config.Add('RED=30,10,5');
      colour_config.SaveToFile(path_to_config);
    end;

    colour_config.LoadFromFile(path_to_config);
    colour := colour_config.Values[name_of_colour];
    Result := colour;
  finally
    colour_config.Free;
  end;
  ...
  if MyFileExists(sFileName) then begin
    ...
    for i := 0 to LoadList.Count - 1 do begin
      ...
      if (s30 <> '') and (s30[1] = '[') then begin
        ...
        boFB := False;
        nFBCount := -1;
        MapFlag.colour_of_dark := '0,0,0';
        MapFlag.boTwilight := True;
        boJob := True;
        while (True) do begin
          if s30 = '' then
            break;
          ...
          if CompareLStr(s34, 'DARK.DENSITY', Length('DARK.DENSITY')) then begin
            ArrestStringEx(s34, '(', ')', s38);
            if (StrToIntDef(s38, -1) >= 0) and (StrToIntDef(s38, -1) <= 100) then begin
              MapFlag.density_of_dark := StrToIntDef(s38, -1);
            end else begin
              MapFlag.density_of_dark := 0;
            end;
            MapFlag.boDARK := True;
            MapFlag.boTwilight := False;
            Continue;
          end;
          if CompareLStr(s34, 'DARK.COLOUR', Length('DARK.COLOUR')) then begin
            ArrestStringEx(s34, '(', ')', s38);
            MapFlag.colour_of_dark := GetColour(s38);
            MapFlag.boDARK := True;
            MapFlag.boTwilight := False;
            Continue;
          end;
          if CompareLStr(s34, 'TWILIGHT.COLOUR', Length('TWILIGHT.COLOUR')) then begin
            ArrestStringEx(s34, '(', ')', s38);
            MapFlag.colour_of_dark := GetColour(s38);
            Continue;
          end;
          if CompareLStr(s34, 'FB(', Length('FB(')) then begin
            ...
            if sFBName = '' then begin
              Result := -12;
              FrmMain.MemoLog.Lines.Add(sMapName + ' Field cannot be empty.');
              Exit;
            end;
            if not (nFBCount in [2..99]) then begin
              Result := -13;
              FrmMain.MemoLog.Lines.Add(sMapName + ' The number of characters 2~99.');
              Exit;
            end;
            if g_FBMapManager.IndexOf(sFBName) <> -1 then begin
              Result := -14;
              FrmMain.MemoLog.Lines.Add(sMapName + ' The name of the copy [' + sFBName + '] already exists.');
              Exit;
            end;
            boFB := True;
            Continue;
          ...
          if CompareLStr(s34,'THUNDER',length('THUNDER')) then begin
            ArrestStringEx(s34,'(',')',s38);
            MapFlag.nThunder := StrToInt(s38);
            Continue;
          end;
          if CompareLStr(s34,'LAVA',length('LAVA')) then begin
            ArrestStringEx(s34,'(',')',s38);
            MapFlag.nLava := StrToInt(s38);
            Continue;
          end;
          ...
          if CompareLStr(s34, 'MINE', length('MINE')) then begin
            ...
            ArrestStringEx(s34,'(',')',s38);
            if s38 = '' then s38:='0';
            MapFlag.nMineType := StrToInt(s38);
            LoadMine(MapFlag.nMineType);
            Continue;
          end;
          if CompareLStr(s34, 'NOAFKMINE', Length('NOAFKMINE')) then begin
            MapFlag.boAFKMINE := True;
            Continue;
          end;
          ...
          MainOutMessage('[Mapinfo Error]: Map: ' + sMapName + ' Missing map flag: ' + s34);
        end;
        if boFB then begin
          ...
          if g_MapManager.AddMapInfo(sMapName, sMainMapName, sMapDesc, nServerIndex, @MapFlag, QuestNPC) = nil then begin
            FrmMain.MemoLog.Lines.Add(sMapName);
            Result := -10;
            Exit;
          end;
        end;
    ...
    for I := 1 to g_MapManager.Count do begin
      FrmMain.Caption := sCaption + '[Initializing map information(' + IntToStr(g_MapManager.Count) + '/' + IntToStr(I) + ')]';
      if not g_MapManager.LoadMapData(I - 1) then begin
        FrmMain.MemoLog.Lines.Add('Error Loading map: ' + TEnvirnoment(g_MapManager.Items[i-1]).sMapName);
        Result := -10;
        Exit;
      end;
...
begin
  ...
  try
    if MyFileExists(sFileName) then begin
      ...
      MapEventList.Add(';The map event trigger configuration');
  ...
  if MyFileExists(sFileName) then begin
    ...
    for i := 0 to LoadList.Count - 1 do begin
      ...
      if (sLineText <> '') and (sLineText[1] <> ';') then begin
        ...
        MonGenInfo.nMissionX := -1;
        MonGenInfo.nMissionY := -1;
        AddedSpawnTime := '';
        ...
        sLineText := GetValidStr3(sLineText, sData, [' ', #9]);
        AddedSpawnTime := GetValidStr3(sData, sData, [',']);
        if AddedSpawnTime <> '' then begin
          MonGenInfo.dwZenTime := (StrToIntDef(sData, -1) + Random(StrToIntDef(AddedSpawnTime, -1))) * 60 * 1000;
          //MainOutMessage(MonGenInfo.sMonName + ' FinalSpawn Time: ' + intToStr(MonGenInfo.dwZenTime div 60 div 1000) + ' Minutes');
        end else begin
        ...
        sLineText := GetValidStr3(sLineText, sData, [' ', #9]);
        MonGenInfo.nMissionX := StrToIntDef(sData, -1);

        sLineText := GetValidStr3(sLineText, sData, [' ', #9]);
        MonGenInfo.nMissionY := StrToIntDef(sData, -1);
        ...
        if (MonGenInfo.sMapName <> '') and (MonGenInfo.sMonName <> '') and (MonGenInfo.dwZenTime <> 0) then begin
          ...
          if sMapName[1] = '$' then begin
            ...
            if MonGenInfo.Envir = nil then
              MainOutMessage('MonGen: missing map: ' + MonGenInfo.sMapName);
            if MonGenInfo.nRace = -1 then
              MainOutMessage('MonGen: missing mob: ' + MonGenInfo.sMonName);
    ...
    LoadList.Free;
    Result := 1;
    MainOutMessage('Estimated Minimal monster regen speed: ' + inttostr(((g_Config.dwRegenMonstersTime + g_dwmonlimit)*UserEngine.m_MonGenList.Count)div(1000*60)) + ' minutes');
  end;
end;
...
begin
  ...
  try
    ...
    for i := 0 to Query.RecordCount - 1 do begin
      ...
      Monster.btAutoRev := Query.FieldByName('Autorev').AsInteger;
      ...
      Monster.btTameValue := _MAX(0,Query.FieldByName('TameLimit').AsInteger);
      Monster.btCanStun := Query.FieldByName('CantGetStun').AsInteger;
      Monster.btCanTDBStun := Query.FieldByName('CantTDBStun').AsInteger;
      Monster.btSlowResist := Query.FieldByName('SlowResist').AsInteger;
      Monster.btCoreDefense := ROUND(Query.FieldByName('CoreDefence').AsInteger * (g_Config.nMonsterPowerRate / 10));
      Monster.btPoisonDefense := Query.FieldByName('PoisonDefense').AsInteger;
      Monster.btNoTurnUndead := Query.FieldByName('NoTurnUndead').AsInteger;
...
begin
  ...
  sFileName:=format(sFileName + '%S-%d-%d-%d,%d-%d.bak',[sSQLDatabase,year,month,day,hour,min]);
  try
    try
      Query.SQL.Clear;
      Query.SQL.Add(format('BACKUP DATABASE %s TO DISK = ''%s''',[sSQLDatabase,sFileName]));
      Query.ExecSQL();
    except
  ...
  if FileExists(sfileName) then
    ShowMessage('Database ' +  sSQLDatabase + ' has been backed up at ' + sFileName)
  else begin
    //a problem with making a db backup, dont continue to run update code now :
    ShowMessage('Failed to backup database ' + sSQLDatabase + ' at: ' + sFileName);
    FrmMain.Close;
  end;

end;

procedure TFrmDB.UpdateDB();
  procedure UpdateToV0();
  begin
    //create version table
    try
      Query.SQL.Clear;
      Query.SQL.Add('CREATE TABLE Tbl_Version (Version int DEFAULT 0)');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    try
      Query.SQL.Clear;
      Query.SQL.Add('INSERT INTO Tbl_Version (Version) values(1)');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    //add fields to item db
    try
      Query.SQL.Clear;
      Query.SQL.Add('ALTER TABLE tbl_items ADD AddWuXinAttack tinyint DEFAULT 0, ' +
        'DelWuXinAttack tinyint DEFAULT 0, Deadliness tinyint DEFAULT 0, ExpRate tinyint DEFAULT 0, ' +
        'HPorMPRate tinyint DEFAULT 0, AC2Rate tinyint DEFAULT 0, MAC2Rate tinyint DEFAULT 0 ');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    //add fields to monster db
    try
      Query.SQL.Clear;
      Query.SQL.Add('ALTER TABLE tbl_monster ADD AutoRev tinyint DEFAULT 0');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=2');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('ALTER TABLE tbl_items ADD Holy tinyint DEFAULT 0');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
  end;
  //stupid me made v1 update set version to 2 so no choice but to skip v2 :p
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=3');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    //create auction_sell table
    try
      sSqlStr:= 'CREATE TABLE Tbl_Auction_Sell (';
      sSqlStr := sSqlStr
        + 'Owner varchar(20) not null'
        + ', QuickSale int default 0'
        + ', Currency int default 0'
      //values below are only for real auctions
        + ', StartTime datetime, EndTime datetime'
        + ', CurrentBid int default 0, CurrentBidder varchar(20) '
      //end of values for real auctions
      //now come the value for item stats
        + ', MakeIndex int not null, nIndex int not null'
        + ', BindMode1 int, BindMode2 int'
        + ', ExpiryTime bigint, Dura int, MaxDura int, ComLevel int'
        + ', Exp bigint, maxexp bigint, hp int, lvl int, alivetime int'
        + ', colour int, effect int, upgrade int, reserved int, reserved2 int'
        + ', Element int, Bind int, MapDesc nchar(14), MapName nchar(20)'
        + ',CurrX int, Curry int, CanStrengthCount int, StrengthCount int'
        + ',btFluteCount int, Value binary(30), temp1 binary(49)'
        + ',flute binary(6), StrenthInfo binary(30)'
        + ')';
      Query.SQL.Clear;
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=4');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    //create auction_buy table
    try
      sSqlStr:= 'CREATE TABLE Tbl_Auction_Buy (';
      sSqlStr := sSqlStr
        + 'Owner varchar(20) not null'
        + ', QuickSale int default 0'
        + ', Currency int default 0'
      ...
      //end of values for real auctions
      //now come the value for item stats
        + ', nIndex int not null, Idx int not null, Dura int)';
      Query.SQL.Clear;
      Query.SQL.Add(sSqlStr);
      Query.ExecSQL();
    finally
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=5');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    //delete unused auction sell fields
    try
      sSqlStr:= 'ALTER TABLE Tbl_Auction_Sell DROP COLUMN Exp, Hp, Lvl, MaxExp, AliveTime, Bind, MapDesc, MapName, CurrX, CurrY;';
      Query.SQL.Clear;
      Query.SQL.Add(sSqlStr);
      Query.ExecSQL();
    finally
    ...
    try
      sSqlStr:= 'ALTER TABLE Tbl_Auction_Sell REBUILD;';
      Query.SQL.Clear;
      Query.SQL.Add(sSqlStr);
      Query.ExecSQL();
    finally
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=6');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('ALTER TABLE tbl_items ADD ShowInGroup tinyint DEFAULT 0');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=7');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('ALTER TABLE tbl_Monster ADD TameLimit tinyint DEFAULT 0, CantGetStun tinyint DEFAULT (0)');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=8');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('ALTER TABLE tbl_Monster ADD CantTDBStun tinyint DEFAULT 0');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=9');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('ALTER TABLE tbl_Monster ADD SlowResist tinyint DEFAULT 0');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=10');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('ALTER TABLE tbl_items ADD NeedClass tinyint DEFAULT 0');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=11');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('ALTER TABLE tbl_Monster ADD CoreDefence tinyint DEFAULT 0, PoisonDefense tinyint DEFAULT 0');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
  ...
  begin
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('Update Tbl_Version SET Version=12');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
    ...
    try
      Query.SQL.Clear;
      Query.SQL.Add('ALTER TABLE tbl_Monster ADD NoTurnUndead tinyint DEFAULT 0');
      Query.ExecSQL();
    finally
      Query.Close;
    end;
...
begin
  ...
  try
    ...
    if MyFileExists(s24) then begin
      ...
      n1C := 0;
      boRandom := False;
      boForceAdd := False; //Set the forced added value to False
      AddList := ItemList;
      for i := 0 to LoadList.Count - 1 do begin
          ...
          if s28 = ')' then begin
            ...
            if s28 = '(' then begin
              if (n18 > 0) and (n1C > 0) then begin
                ...
                MonItem.MaxPoint := _Max((Round(n1C/g_Config.nDropRate)),1);
                MonItem.boGold := False;
                MonItem.List := TList.Create;
                MonItem.boRandom := boRandom;
                MonItem.ForceAdded := boForceAdd;
                AddList.Add(MonItem);
            ...
            boForceAdd := False; //Return to false so next item isnt added.
            //This will force added item drops if '*' is after item name ( 1/1 ItemName * )
            if CompareText(s30, '*') = 0 then
              boForceAdd:= True;
            nPur := StrToIntDef(s28,0);
            if (n18 > 0) and (n1C > 0) and (s2C <> '') then begin
              ...
              if CompareText(s2C, sSTRING_GOLDNAME) = 0 then begin
                ...
                if (StdItem <> nil) then begin
                  ...
                  MainOutMessage('[Drop Error:] mob: '  + MonName + ' invalid item: ' + s2C);
                  Dispose(MonItem);
                  Continue;
                end;
              end;
      ...
      if (sValue <> '') and (sValue[1] <> ';') then begin
        ...
        if (sValue <> '') and (sName <> '') then begin
          if UserEngine.m_DefiniensConst.IndexOf(sName) = -1 then begin
            ...
            MainOutMessage('NPC Constant setting error repeated[' + sName + ']...');
          end;
        end;
      end;
    end;
    ...
    for i := 0 to LoadList.Count - 1 do begin
      ...
      if (s18 <> '') and (s18[1] <> ';') then begin
        ...
        if (s20 <> '') and (s20[1] = '$') then begin
          ...
          boFlag := False;
        s18 := GetValidStr3(s18, s24, [' ', #9]);  //s24 : npctype
        s18 := GetValidStr3(s18, s28, [' ', #9]);  //s28 : mapname
        s18 := GetValidStr3(s18, s2C, [' ', #9]);  //s2c : x
        s18 := GetValidStr3(s18, s30, [' ', #9]);  //s30 : y
        s18 := GetValidStr3(s18, s34, [' ', #9]);  //s34 : nflag
        s18 := GetValidStr3(s18, s38, [' ', #9]);  //s38 : direction
        s18 := GetValidStr3(s18, s40, [' ', #9]);  //s40 : autochangecolor
        s18 := GetValidStr3(s18, s42, [' ', #9]);  //s42 : autochangecolortime
  ...
  if MyFileExists(sFileName) then begin
    try
      ...
      for i := 0 to LoadList.Count - 1 do begin
        ...
        if (tStr <> '') and (tStr[1] <> ';') then begin
          tStr := GetValidStr3(tStr, s18, [' ', #9]);  //s18 : mapname
          tStr := GetValidStr3(tStr, s1C, [' ', #9]);  //s1c : x
          tStr := GetValidStr3(tStr, s20, [' ', #9]);  //s20 : y
          tStr := GetValidStr3(tStr, s22, [' ', #9]);  //s22 : disable chat (not working)
          tStr := GetValidStr3(tStr, s24, [' ', #9]);  //s24 : range
          tStr := GetValidStr3(tStr, s26, [' ', #9]);  //s26: type (disabled)
          tStr := GetValidStr3(tStr, s28, [' ', #9]);  //s28: pksize
          tStr := GetValidStr3(tStr, s30, [' ', #9]);  //s30: pktype (disabled
...
var
  ...
  begin
    ...
    else if sLabel2 = sVAR_LASTKILLEDMOBNAME then begin
      sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', tVAR_LASTKILLEDMOBNAME);
    end
    else if sLabel2 = sVAR_LASTGROUPKILLEDMOBNAME then begin
      sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', tVAR_LASTGROUPKILLEDMOBNAME);
    end
    ...
    else if sLabel2 = sVAR_BODYLUCKCOUNT then begin
      sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', tVAR_BODYLUCKCOUNT);
    end
  ...
  begin
    with NPC do begin
      ...
      if sLabel = SPLAYOFFLINE then begin
        ...
        else if sLabel = SGROUPKILLMONSTER then
        begin
          FGotoLable[NGROUPKILLMONSTER] := nIdx;
        end
        else begin
  ...
  begin
    ...
    while (I < LoadList.Count) do begin //Jason 071209修改
      if Ing > 5000 then begin
        MainOutMessage('#CALL Call the cycle of death ' + sLoadName);
        break;
      end;
      s14 := Trim(LoadList.Strings[i]);
      if (s14 <> '') and (s14[1] = '#') and (CompareLStr(s14, '#CALL', Length('#CALL'))) then begin
        ...
        if nIdx > -1 then begin
          ...
          Inc(nCount);
          if nCount > 100 then begin
            MainOutMessage('#CALL Call the cycle of death ' + sLoadName + ' ' + s20 + ' ' + s18);
            break;
          end;
          CallNameList.Objects[nIdx] := TObject(nCount);
        end
  ...
  begin
    ...
    if sCmd = sSC_CHECKDAYTIME then begin
      nCMDCode := nSC_CHECKDAYTIME;
      goto L001;
    end;
    ...
    if sCmd = sSC_CHECKECTYPE then begin
      nCMDCode := nSC_CHECKECTYPE;
      goto L001;
    end;
...
{$ENDIF}
  ...
  begin
    ...
    if sCmd = sSC_OPENAUCTION then begin
      nCMDCode := nSC_OPENAUCTION;
      goto L001;
    end;
    if sCmd = sSC_GROUPKILLMONEXPRATE then begin
      nCMDCode := nSC_GROUPKILLMONEXPRATE;
      goto L001;
    end;
    if sCmd = sSC_GROUPPOWERRATE then begin
      nCMDCode := nSC_GROUPPOWERRATE;
      goto L001;
    end;
{$IFDEF PLUGOPEN}
...
begin //0048B684
  ...
  if MyFileExists(sScritpFileName) then begin
    ...
    for i := 0 to LoadList.Count - 1 do begin
      ...
      if (s34 <> '') then begin
        if s34[1] = '%' then begin //price rate (in %)
          s34:=Copy(s34,2,Length(s34) - 1);
          nPriceRate:=StrToIntDef(s34,-1);
          if nPriceRate >= 55 then begin
            TMerchant(NPC).m_nPriceRate:=nPriceRate;
          end else begin
            MainOutMessage('Invalid price rate: ' + s34 + ' on npc: ' + sScritpName);
          end;
          Continue;
        end;
        if s34[1] = '+' then begin//add this value to the allowed stdmodes (999 allows all modes)
          s34:=Copy(s34,2,Length(s34) - 1);
          nPriceRate:=StrToIntDef(s34,-1);//no reason to not reuse this variable :p
          if nPriceRate >= 0 then begin
            TMerchant(NPC).m_ItemTypeList.Add(Pointer(nPriceRate));
          end else begin
            MainOutMessage('Invalid stdmode: ' + s34 + ' on npc: ' + sScritpName);
          end;
          Continue;
        end;
    ...
    if boFlag then begin
      with NPC as TMerchant do begin
        ...
        if m_GoodsList.Count > 0 then begin
          //for ii := 0 to m_GoodsList.Count - 1 do begin
          while m_GoodsList.Count > 0 do begin
            Goods := m_GoodsList.Items[0];
            m_GoodsList.Delete(0);
            s34:='1';
            if Goods <> nil then begin
              s34 := '2';
              for iii := 0 to Goods.SubGoods.Count - 1 do begin
                s34 := '3-' + inttostr(iii);
                Dispose(Goods.SubGoods[iii]);
              end;
              s34 := '4';
              Goods.SubGoods.clear;
              FreeAndNil(Goods.SubGoods);
            end;
            s34 := '5';
            Dispose(Goods);
            s34 := '6';
          ...
          MainOutMessage('[Merchant] ' + Npc.m_sCharName + ' failed to clear goods');
        end;
        ...
        if s34[1] = '(' then begin
          ...
          if s34 <> '' then begin
            while (s34 <> '') do begin
              ...
              if CompareText(s30, sAuction) = 0 then begin
                with NPC as TMerchant do
                  m_boAuction := True;
                Continue;
              end;
      ...
      if (SayingRecord <> nil) then begin
        ...
        if (n6C = 11) then begin
          ...
          if QuestCondition(Trim(s34), QuestConditionInfo) then begin
            ...
            MainOutMessage('Script Error: ' + s34 + ' First:' + IntToStr(i) + ' Row: ' + sScritpFileName);
          end;
        end; //0048C004
        ...
        if (s48 <> '') then begin
          ...
          if Stditem <> nil then begin
            ...
            if MakeItem <> nil then begin
              ...
              MainOutMessage('MakeItem not found: ' + StdItem.Name + ' in MakeItems.txt');
          end else
            MainOutMessage('MakeItem not found: '+ s48 + ' in ' + sScritpFileName);
        end;
      end;
        ...
        if (s48 <> '') then begin
          ...
          with NPC as TMerchant do begin
            ...
            if UserEngine.CopyToUserItemFromNameEx(s48, @Goods.UserItem) then begin
              ...
              MainOutMessage('[Merchant]'  + sScritpName + ' Loading goods failed for: ' + s48);
              Dispose(Goods);
            end;
            except
              MainOutMessage('[Merchant]' + Npc.m_sCharName + ' Loading Goods failed');
            end;
          end;
        end; //0048C2D2
      end; //0048C2D2
    ...
    MainOutMessage('Script File not found: ' + sScritpName);
  end;
  Result := 1;
  ScriptNameList.Free;
  GotoList.Free;
...
begin
 ...
 if (not FileExists(sFileName)) then begin
    MainOutMessage('Saving Upgrades failed for: ' + sFileName);
  end;
 if FileHandle > 0 then begin
...
begin
  ...
  if not FileExists(sFileName) then begin
    MainOutMessage('Saving Goods failed for: ' + sFileName);
    exit;
  end;
  if FileHandle > 0 then begin
    FillChar(Header420,SizeOf(TGoodFileHeader),#0);
    for I := 0 to NPC.m_GoodsList.Count - 1 do begin
      Goods := Npc.m_GoodsList[i];
      if (Goods.SubGoods <> nil) and (Goods.SubGoods.count > 0) then
        Inc(Header420.nItemCount,Goods.SubGoods.Count)
      else
        Inc(Header420.nItemCount);
    end;
    FileWrite(FileHandle,Header420,SizeOf(TGoodFileHeader));
    for I := 0 to NPC.m_GoodsList.Count - 1 do begin
      Goods := Npc.m_GoodsList[i];
      if (Goods.SubGoods = nil) or (Goods.SubGoods.Count = 0) then begin
        UserItem := Goods.UserItem;
        UserItem.Dura := Goods.nStock;
        UserItem.DuraMax := 0;
        FileWrite(FileHandle, UserItem, SizeOf(TUserItem));
      end else begin
        for ii := 0 to Goods.SubGoods.Count - 1 do begin
          FileWrite(FileHandle, Goods.SubGoods[ii]^, SizeOf(TUserItem));
        end;
      end;
    end;
    FileClose(FileHandle);
...
begin
  Result:= -1;
  sFileName:='.\Envir\Market_Saved\' + sFile + '.sav';
  //outputdebugstring(pansichar(sfile));
  if not FileExists(sFileName) then begin
    FileCreate(sFileName);
    if not FileExists(sFileName) then begin
      MainOutMessage('Loading Goods failed for: ' + sFileName);
      exit;
    end;
  end;
...
function TFrmDB.LoadMine(nMineIdx: Integer):integer;
var
  Mine:pTMineInfo;
...
begin
  ...
  for i := 0 to g_MineTable.Count - 1 do begin
    if (g_MineTable[i] <> nil) then begin
      if pTMineInfo(g_MineTable[i]).nIdx = nMineIdx then
        exit;
    end;
  end;
  new(Mine);
  Mine.nIdx := nMineIdx;
  Mine.DetailList := TList.Create;
  Mine.nTotalSlots := 0;
  sFileName := g_Config.sEnvirDir + 'Mines\Mine-' + inttostr(nMineIdx) + '.txt';
  if FileExists(sFileName) then begin
    IniFile := TIniFile.Create(sFileName);
    Mine.nTotalSlots := IniFile.ReadInteger('Setup', 'TotalSlots', 0);
    i := 0;
    while  IniFile.ReadInteger('Item' + inttostr(i), 'MinRate', -1) > 0 do begin
      new(MineDetails);
      MineDetails.nMinRate := IniFile.ReadInteger('Item' + inttostr(i), 'MinRate', 0);
      MineDetails.nMaxRate := IniFile.ReadInteger('Item' + inttostr(i), 'MaxRate', 0);
      MineDetails.nMinDura := IniFile.ReadInteger('Item' + inttostr(i), 'MinDura', 0);
      MineDetails.nMaxDura := IniFile.ReadInteger('Item' + inttostr(i), 'MaxDura', 0);
      MineDetails.nDuraBonusRate := IniFile.ReadInteger('Item' + inttostr(i), 'DuraBonusRate', 0);
      MineDetails.nDuraBonusMax := IniFile.ReadInteger('Item' + inttostr(i), 'DuraBonusMax', 0);
      MineDetails.sItemResult := IniFile.ReadString('Item' + inttostr(i), 'Item', '');
      Mine.DetailList.Add(MineDetails);
      inc(i);
    end;
    IniFile.Free;
  end else begin
    MainOutMessage('Loading Mines failed for: ' + sFileName);
  end;
  g_MineTable.Add(Mine);
end;



procedure TFrmDB.AddAuctionItem(SellItem: pTUserShopSellItem; Player: TPlayObject);
function setBoolean(b: Boolean): Integer;
begin
  result := 0;
  if b then
    result := 1;
...
var
  ...
  i: integer;
begin
  {$Ifdef DisableAuction}
    exit;
  {$endif}
  if player = nil then exit;
  if sellitem = nil then exit;
  if sellitem.UserItem = nil then exit;
  
  SetLength(Temp,Length(SellItem.UserItem.temp1));
  for I := 0 to Length(SellItem.UserItem.temp1) - 1 do
    Temp[i]:=SellItem.UserItem.temp1[i];

  SetLength(Values,Length(SellItem.UserItem.Value.btValue));
  for i := 0 to Length(SellItem.UserItem.Value.btValue) - 1 do
    Values[i]:=SellItem.UserItem.Value.btValue[i];

  SetLength(Flute,Length(SellItem.UserItem.Value.wFlute)*2);
  for i := 0 to Length(SellItem.UserItem.Value.wFlute) - 1 do begin
    Flute[i*2] := LoByte(SellItem.UserItem.Value.wFlute[i]);
    Flute[1+i*2] := HiByte(SellItem.UserItem.Value.wFlute[i]);
  end;

  SetLength(Strength,Length(SellItem.UserItem.Value.StrengthenInfo.btStrengthenInfo));
  for i := 0 to length(SellItem.UserItem.Value.StrengthenInfo.btStrengthenInfo) - 1 do
    Strength[i] := SellItem.UserItem.Value.StrengthenInfo.btStrengthenInfo[i];

  sSqlStr := 'insert into tbl_Auction_Sell (Owner,QuickSale,Currency,StartTime,EndTime'
    + ',CurrentBid,CurrentBidder,MakeIndex,nIndex,BindMode1,BindMode2,ExpiryTime,Dura'
    + ',MaxDura,ComLevel,colour,effect,upgrade,reserved'
    + ',reserved2,Element,CanStrengthCount,StrengthCount'
    + ',btFluteCount,Value,temp1,flute,StrenthInfo'
    + ') values(' + quotedstr(Player.m_sCharName) + ', ' + IntToStr(SellItem.nMoney) + ',' + inttostr(SetBoolean(SellItem.boGameGold)) + ',' + QuotedStr(dateForSql(now())) + ', ' + QuotedStr(dateForSql(now()))
    + ', 0,' + Quotedstr('') + ',' + IntToStr(SellItem.UserItem.MakeIndex) + ',' + IntToStr(SellItem.UserItem.wIndex) + ',' + IntToStr(SellItem.UserItem.btBindMode1) + ',' + IntToStr(SellItem.UserItem.btBindMode2) + ',' + IntToStr(SellItem.UserItem.TermTime) + ',' + IntToStr(SellItem.UserItem.Dura)
    + ',' + IntToStr(SellItem.UserItem.DuraMax) + ',' + IntToStr(SellItem.UserItem.ComLevel) + ',' + IntToStr(SellItem.UserItem.EffectValue.btColor) + ',' + IntToStr(SellItem.UserItem.EffectValue.btEffect) + ',' + IntToStr(SellItem.UserItem.EffectValue.btUpgrade) + ',' + IntToStr(SellItem.UserItem.EffectValue.btReserved)
    + ',' + IntToStr(SellItem.UserItem.EffectValue.btReserved2) + ',' + IntToStr(SellItem.UserItem.Value.btWuXin) + ',' + IntToStr(SellItem.UserItem.Value.StrengthenInfo.btCanStrengthenCount) + ',' + IntToStr(SellItem.UserItem.Value.StrengthenInfo.btStrengthenCount)
    + ',' + IntToStr(SellItem.UserItem.Value.btFluteCount) + ', :value , :temp , :flute , :strength )';
  try
    try
      ADOCommand:=TADOCommand.Create(nil);
      ADOCommand.Connection:=AdoConnection;
      ADOCommand.Parameters.Clear;
      ADOCommand.CommandText := sSqlStr;
      ...
      on e: Exception do begin
        MainOutMessage(e.Message);
        MainOutMessage(sSqlStr);
        MainOutMessage('[Auction]Error inserting auction item for player:' + Player.m_sCharName);
      end;
    end;
  finally
    ADOCommand.Free;
...
procedure TFrmDB.DellAuctionItem(SellItem: pTUserShopSellItem; Player: TPlayObject);
begin
  {$Ifdef DisableAuction}
    exit;
  {$endif}
  //shouldnt be possible but safety first
  if player = nil then exit;
  if sellitem = nil then exit;
  if sellitem.UserItem = nil then exit;
  //simply remove the item from database using charname and makeindex
  try
    try
      Query.SQL.Clear;
      Query.SQL.Add(Format('Delete from Tbl_Auction_Sell where owner =''%s'' and makeindex=%d',[Player.m_sCharName,SellItem.UserItem.MakeIndex]));
      Query.ExecSQL();
    except
      on e: Exception do begin
        MainOutMessage(e.Message);
        MainOutMessage('[Auction] Error deleting auction item for player:' + Player.m_sCharName);
      end;
    end;
  finally
    Query.Close;
...
procedure TFrmDB.UpdateAuctionItem(SellItem: pTUserShopSellItem; Player: TPlayObject);
begin
  {$Ifdef DisableAuction}
    exit;
  {$endif}
  ...
  if sellitem = nil then exit;
  if sellitem.UserItem = nil then exit;
  //simply adjust the items dura in database using charname and makeindex
  try
    try
      Query.SQL.Clear;
      Query.SQL.Add(Format('update Tbl_Auction_Sell set dura = %d where owner =''%s'' and makeindex=%d',[SellItem.UserItem.Dura, Player.m_sCharName,SellItem.UserItem.MakeIndex]));
      Query.ExecSQL();
    except
      on e: Exception do begin
        MainOutMessage(e.Message);
        MainOutMessage('[Auction] Error updating auction item for player:' + Player.m_sCharName);
      end;
    end;
  finally
    Query.Close;
...
procedure TFrmDB.AddAuctionItem(SellItem: pTUserShopBuyItem; Player: TPlayObject);
function setBoolean(b: Boolean): Integer;
begin
  result := 0;
  if b then
...
begin
  ...
  if player = nil then exit;
  if sellitem = nil then exit;
  if sellitem.wIdent = 0 then exit;

  sSqlStr := 'insert into tbl_Auction_Buy (Owner,QuickSale,Currency,StartTime,EndTime'
    + ',CurrentBid,CurrentBidder,nIndex, idx, Dura'
    + ') values(' + quotedstr(Player.m_sCharName) + ', ' + IntToStr(SellItem.nMoney) + ',' + inttostr(SetBoolean(SellItem.boGameGold)) + ',' + QuotedStr(dateForSql(now())) + ', ' + QuotedStr(dateForSql(now()))
    + ', 0,' + Quotedstr('') + ',' + IntToStr(SellItem.wIdent) + ',' + IntToStr(SellItem.btIdx) + ',' + IntToStr(SellItem.wCount) + ')';
  try
    try
      ADOCommand:=TADOCommand.Create(nil);
      ADOCommand.Connection:=AdoConnection;
...
procedure TFrmDB.DellAuctionItem(SellItem: pTUserShopBuyItem; Player: TPlayObject);
begin
  {$Ifdef DisableAuction}
    exit;
  {$endif}
  ...
  try
    try
      Query.SQL.Clear;
      Query.SQL.Add(Format('Delete from Tbl_Auction_Buy where owner =''%s'' and idx=%d',[Player.m_sCharName,SellItem.btIdx]));
      Query.ExecSQL();
    except
      on e: Exception do begin
        MainOutMessage(e.Message);
...
procedure TFrmDB.UpdateAuctionItem(SellItem: pTUserShopBuyItem; Player: TPlayObject);
begin
  {$Ifdef DisableAuction}
    exit;
  {$endif}
  ...
  try
    try
      Query.SQL.Clear;
      Query.SQL.Add(Format('update Tbl_Auction_Buy set dura = %d where owner =''%s'' and idx=%d',[SellItem.wCount, Player.m_sCharName,SellItem.btIdx]));
      Query.ExecSQL();
    except
      on e: Exception do begin
        MainOutMessage(e.Message);
...
procedure TFrmDB.InitiateAuction;
begin
  try
    AdoConnection:=TADOConnection.Create(nil);
    AdoConnection.LoginPrompt:=False;//dont ask for the login parameters
    AdoConnection.ConnectionString:=Query.ConnectionString;
  ...
  try
    Query.SQL.Clear;
    Query.SQL.Add('Delete from tbl_auction_sell');
    Query.ExecSQL();
  finally
    Query.Close;
  end;
  try
    Query.SQL.Clear;
    Query.SQL.Add('Delete from tbl_auction_buy');
    Query.ExecSQL();
  finally
    Query.Close;
  end;
...
function TFrmDB.dateForSQL(dModDate: TDateTime): string; //Sams date/time converter so mysql can be updated without errors.
var
  delphiDateTime, sqlDate, sqlTime, days, months, years, time: String;
  timeList, dateList: TStringList;
begin
  delphiDateTime := DateTimeToStr(dModDate); //format dd/mm/yyyy hh:mm:ss
  timeList := TStringList.Create;
  dateList := TStringList.Create;

  if POS(' ', delphiDateTime) > 0 then begin
    timeList.Delimiter := ' ';
    timeList.DelimitedText := delphiDateTime;
    sqlTime := timeList[1];

    dateList.Delimiter := '/';
    dateList.DelimitedText := timeList[0];
    days := dateList[0];
    months := dateList[1];
    years := dateList[2];
    sqlDate := years + '-' + months + '-' + days;

    Result := sqlDate + ' ' + sqlTime;
  end else begin
    dateList.Delimiter := '/';
    dateList.DelimitedText := delphiDateTime;
    days := dateList[0];
    months := dateList[1];
    years := dateList[2];
    sqlDate := years + '-' + months + '-' + days;
