      ...
      if not m_boRunAwayMode then begin
        if not m_boNoAttackMode then begin
          if m_TargetCret <> nil then begin
            ...
            if m_boSpawnMission then begin //If we have a spawn mission
              m_nTargetX := m_nSpawnMissionX; //Set our target to mission spawn
              m_nTargetY := m_nSpawnMissionY;
              if (abs(m_nCurrX - m_nTargetX) < 4) and (abs(m_nCurrY - m_nTargetY) < 4) then begin //If we are within range of our mission
                if m_PEnvir.GetMovingObject(m_nTargetX, m_nTargetY, True) <> nil then begin //If spot is taken
                  m_nTargetX := m_nCurrX; //Wait around
                  m_nTargetY := m_nCurrY;
                end;
              end;
            end;
        ...
        if (FollowObject <> nil) then begin
          if CanFollowObject(FollowObject) then begin
            if FollowObject.m_TargetCret <> nil then begin
              if m_TargetCret = nil then
                m_TargetCret := FollowObject.m_TargetCret;
            end else begin
              m_nTargetX:= FollowObject.m_nCurrX;
              m_nTargetY:= FollowObject.m_nCurrY;
              if (abs(m_nCurrX - m_nTargetX) < 4) and (abs(m_nCurrY - m_nTargetY) < 4) then begin //If were near object
                m_nTargetX := m_nCurrX;
                m_nTargetY := m_nCurrY;
                FollowObject.GetFrontPosition(nX,nY); // find front position of follow mob
                if (abs(m_nCurrX - nX) < 2) and (abs(m_nCurrY - nY) < 2) then begin //If were in front of, get out the way.
                  nNextDir := GetNextDirection(nX, nY, m_nCurrX, m_nCurrY);
                  m_PEnvir.GetNextPosition(nX, nY, nNextDir, 3, m_nTargetX, m_nTargetY);
                end;
              end;
            end;
          end;
        ...
        if m_Master <> nil then begin
          if m_PEnvir.m_boFB and (m_PEnvir <> m_Master.m_PEnvir) then begin
            ...
            if m_Master.m_boSlaveRelax then m_Master.SysMsg('Mysterious force teleported your pets.', c_Red, t_Hint);
          end else begin
            if m_TargetCret = nil then begin
              m_Master.GetBackPosition(nX, nY);
              if (abs(m_nTargetX - nX) > 1) or (abs(m_nTargetY - nY {nX}) > 1) then begin
...
begin
  if not bo554 and (CanPerformActions) then begin
...
//  m_dwWalkTick := GetTickCount + 2000; //this 'stuns' the skele when you do stuff like blessed armor
end;
    ...
    if m_boStoneMode then begin
      ...
      if ((GetTickCount - m_dwSearchEnemyTick) > 8000) or
        ...
        if (m_nDangerLevel > m_WAbil.HP / m_WAbil.MaxHP * 5) and (m_nDangerLevel
          > 0) then begin
          Dec(m_nDangerLevel);
          if (m_btRaceServer <> 68) then begin
            CallSlave();
          end;
        end;
...
procedure TFiredrakeGeneralMonster.MagicAttack(attack_direction: Integer);
var
  nPwr:Integer;
begin
  m_btDirection := attack_direction;
  SendRefMsg(RM_LIGHTING,m_btDirection,m_nCurrX,m_nCurrY,Integer(m_TargetCret),'');

  if IsProperTarget(m_TargetCret) then begin
    nPwr:=(HiWord(m_WAbil.MC) div 2) + Random(HiWord(m_WAbil.MC) div 2);
    if nPwr > 0 then
  ...
  if m_Master = nil then begin
    if (m_nMasterIndex = -1) or (GetTickCount > m_MasterGhostTick) then begin
      ...
      if PlayObject <> nil then begin
        ...
        m_Master := PlayObject;
        PlayObject.SysMsg(Format('%s waiting for you at: %s(%d,%d)', [FilterShowName(m_sCharName), m_PEnvir.sMapDesc, m_nCurrX, m_nCurrY]), c_Red, t_Hint);
        RefNameColor;
      end;
    end;
  end else
      ...
      if not m_boRunAwayMode then begin
        if (m_Master <> nil) and (m_PEnvir = m_Master.m_PEnvir) and
          ...
          if (m_Master <> nil) and (not m_boHintMaster) and (m_Master.m_btRaceServer = RC_PLAYOBJECT) then begin
            m_Master.SysMsg('Cart out of range.', c_Red, t_Hint);
            m_boHintMaster := True;
          end;
        end;
      end
                ...
                for I := m_ItemList.Count - 1 downto 0 do begin
                  ...
                  if UserItem <> nil then begin
                    if StdItem.StdMode = tm_house then begin
                      UserItem.btLevel := Self.m_Abil.Level + Random(6) - Random(6);
                      UserItem.dwExp := Random(g_Config.HorseLevelExp[Self.m_Abil.Level]);
                      UserItem.dwMaxExp := g_Config.HorseLevelExp[Self.m_Abil.Level];
                    end;
                    BaseObject.m_ItemList.Add(UserItem);
                    TPlayObject(BaseObject).SendAddItem(UserItem);
                    if StdItem.NeedIdentify = 1 then
                      AddGameLog(BaseObject, LOG_ADDITEM, StdItem.Name, UserItem.MakeIndex, UserItem.Dura, m_sCharName, '0', '0', 'Ñ±»¯', UserItem);
                    m_ItemList.Delete(I);
                  end;
                end;
              end;
            end;
            BaseObject.SysMsg('succesfully tamed [' + FilterShowName(m_sCharName) + ']', c_Red, t_Hint);
            MakeGhost;
          end;
        end;
      end;
