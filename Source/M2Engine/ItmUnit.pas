...
type
  ...
  public
    ...
    function HasRandomStat(UserItem: pTUserItem): Integer;
  end;
...
begin
  ...
  if sm_Arming in StdItem.StdModeEx then begin
    ...
    GetStrengthenAbility(UserItem, StdItem);
    ...
    StdItem.PoisonAttack := StdItem.PoisonAttack + UserItem.Value.btValue[tb_PoisonAttack];
    StdItem.Freezing := StdItem.Freezing + UserItem.Value.btValue[tb_SlowDown];
    if StdItem.HitSpeed > 10 then
      StdItem.HitSpeed := StdItem.HitSpeed + UserItem.Value.btValue[tb_HitSpeed]
    else
      if (StdItem.HitSpeed > UserItem.Value.btValue[tb_HitSpeed]) then
        StdItem.HitSpeed := StdItem.HitSpeed - UserItem.Value.btValue[tb_HitSpeed]
      else
        StdItem.HitSpeed := (10 + UserItem.Value.btValue[tb_HitSpeed]) - StdItem.HitSpeed;
...
function TItemUnit.HasRandomStat(UserItem: pTUserItem): Integer;
var
 i: Integer;
begin
  result := 0;
  for i := 0 to High(UserItem.Value.btValue) do begin
    if UserItem.Value.btValue[i] > 0 then
      result:= 1;
  end;
  if useritem.Value.btFluteCount > 0 then
    result := 1;
  if useritem.Value.StrengthenInfo.btStrengthenCount > 0 then
    result := 1;
end;
