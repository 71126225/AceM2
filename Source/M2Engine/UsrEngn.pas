...
{$IFEND}
  ...
  public
    ...
    function GetStdItem(nItemIdx, playerlevel: Integer; playerjob: Byte): pTStdItem; overload;
    ...
    function ChangeItemByLevel(tempitem: PTStdItem; playerlevel: Word): PTStdItem;
    function ChangeItemByJob(tempitem: PTStdItem; playerjob: Byte): PTStdItem;
...
uses IdSrvClient, Guild, ObjMon, EDcodeEx, ObjGuard, ObjAxeMon, M2Share, OjbKindMon, RegDllFile,  
  ...
  Castle, {$IFDEF PLUGOPEN}PlugOfEngine,{$ENDIF}  Common, ObjMon3, ObjMon4, ItmUnit;
{ TUserEngine }
...
{$IFEND}
  ...
  FProcessUserMessage[CM_CROSSHIT] := ProcessMessageMove;
  FProcessUserMessage[CM_TWINHIT] := ProcessMessageMove;
  FProcessUserMessage[CM_FLASHDASHHIT] := ProcessMessageMove;
  FProcessUserMessage[CM_POISONSWORDHIT] := ProcessMessageMove;
  FProcessUserMessage[CM_WIDEHIT] := ProcessMessageMove;
  FProcessUserMessage[CM_FIREHIT] := ProcessMessageMove;
  FProcessUserMessage[CM_DOUBLESLASH] := ProcessMessageMove;
...
begin
  Result := '';
  Stditem := GetStdItem(UserItem.wIndex, 0, 0);
  if Stditem <> nil then
  begin
      ...
      if Merchant <> nil then begin
        ...
        if Merchant.m_PEnvir <> nil then begin
          ...
          if Merchant.m_boAddtoMapSuccess and (not Merchant.m_boIsHide) then begin
            MainOutMessage(Merchant.m_sCharName + 'Failed to initialize Merchant Transaction... ');
          ...
          MainOutMessage(Merchant.m_sCharName + 'Failed to initialize Merchant Transaction... (m.PEnvir=nil)');
          m_MerchantList.Delete(i);
          Merchant.Free;
        end;
        FrmMain.Caption := sCaption + '[Initialize Merchants Transaction (' + IntToStr(m_MerchantList.Count) + '/' + IntToStr(m_MerchantList.Count - i) + ')]';
      ...
      if NormNpc <> nil then begin
        ...
        if NormNpc.m_PEnvir <> nil then begin
          ...
          if NormNpc.m_boAddtoMapSuccess and (not NormNpc.m_boIsHide) then begin
            MainOutMessage(NormNpc.m_sCharName + 'Failed to initialize NPC management... ');
          ...
          MainOutMessage(NormNpc.m_sCharName + 'Failed to initialize NPC management... (m.PEnvir=nil) ');
          QuestNPCList.Delete(i);
          NormNpc.Free;
        end;
      end;
      FrmMain.Caption := sCaption + '[Initialize NPC management(' + IntToStr(QuestNPCList.Count) + '/' + IntToStr(QuestNPCList.Count - i) + ')]';
      Application.ProcessMessages;
    end;
  finally
    FrmMain.Caption := sCaption;
  ...
  begin
    for i := 0 to m_MonGenList.Count - 1 do begin
      ...
      if (MonGen <> nil) and (not MonGen.boFB) and (MonGen.sMonName <> '') and not g_Config.boVentureServer then begin
        ...
        nGenCount := _MAX(1, ROUND(_MAX(1, MonGen.nCount) / (g_Config.nMonGenRate / 10)));                          {GetZenTime(MonGen.dwZenTime)}
        boRegened := True;
        if nGenCount > 0 then
          boRegened := RegenMonsters(MonGen, nGenCount);
        if boRegened then begin
          MonGen.dwStartTick := GetTickCount();
        end;
        //check if mob uses evilmir's race and set inactive timer.
        {
        for j := 0 to MonGen.CertList.Count - 1 do begin
          BaseObject := TBaseObject(MonGen.CertList.Items[j]);
          if (BaseObject <> nil) and (BaseObject.m_btRaceServer = 75) then
            BaseObject.m_dwRespawnTime := MonGen.dwZenTime;
        end;
        }
      end;
    end;
...
begin
  ...
  try
    ...
    if ((GetTickCount - dwRegenMonstersTick) > g_Config.dwRegenMonstersTime) then begin
      ...
      if (MonGen <> nil) and (not MonGen.boFB) and (MonGen.sMonName <> '') and not g_Config.boVentureServer then begin
        if (MonGen.dwStartTick = 0) or ((GetTickCount - MonGen.dwStartTick) >= MonGen.dwZenTime) then begin
          ...
          boRegened := True;
          if g_Config.nMonGenRate <= 0 then g_Config.nMonGenRate := 10;
          ...
          if nMakeMonsterCount > 0 then begin
    ...
    //now do the actual monster processing
...
//  dwProcessTick:LongWord;
  ...
  begin
    ...
    MainOutMessage(format('Online: (%d/%d) %d', [nOnlineCount2, nOffLineCount, nOnlineCount]));
...
function TUserEngine.GetStdItem(nItemIdx, playerlevel: Integer; playerjob: Byte): pTStdItem;
begin
  Result := nil;
  Dec(nItemIdx);
  if (nItemIdx >= 0) and (StdItemList.Count > nItemIdx) then begin
    ...
    if (Result.Name <> '') and (playerlevel > 0)  then begin
  ...
  begin
    if MonItem.boGold then begin
      ...
      if CopyToUserItemFromIdxEx(MonItem.ItemIdent, UserItem) then begin
        ...
        if (StdItem.StdMode = tm_Ore) then begin
          if MonItem.Purity <> 0 then begin
            UserItem.Dura := MonItem.Purity * 1000;
          end;
        end;
        if (sm_HorseArm in StdModeEx) or (sm_ArmingStrong in StdModeEx) then begin
          ...
          if ((g_Config.nMonRandomAddValue < 100) and (Random(g_Config.nMonRandomAddValue {10}) = 0)) or (MonItem.ForceAdded) then begin
...
{$ENDIF}
      ...
      MainOutMessage('[Packet Attacks] ' + PlayObject.m_sCharName + ' ' + PlayObject.m_sIPLocal + ' ' + IntToStr(DefMsg.Ident));
    end;
  except
    MainOutMessage(sExceptionMsg);
  end;
...
begin
  ...
  case nMonRace of
    ...
    61: Cert := TWizClone.Create;
    62: Cert := TFrostTiger.Create;
    63: Cert := TKhazard.Create;
    64: Cert := TThunderTypeWT.Create;
    65: Cert := TCrystalSpider.Create;
    66: Cert := TYimoogi.Create;
    67: Cert := TYimoogiMaster.Create;
    68: Cert := TMagicZumaKing.Create;
    69: Cert := TBoneLord.Create;
    70: Cert := TMinoGuard.Create;
    71: Cert := TMinotaurKing.Create;
    72: Cert := TNodeMonster.Create;
    73: Cert := TOmaKing.Create;
    74: Cert := TDragonStatue.Create;
    75: Cert := TEvilMir.Create;
    76: Cert := TDragonPart.Create;
    ...
    129: Cert := TBlackFoxMan.Create;
    130: Cert := TRedFoxMan.Create;
    131: Cert := TWhiteFoxMan.Create;
    132: Cert := TElement.Create;
    133: Cert := TBigElement.Create;
    134: Cert := TBigNonPsnElement.Create;
    135: Cert := TTrapRock.Create;
    136: Cert := TTrapPart.Create;
    137: Cert := TGuardianRock.Create;
    138: Cert := TOrbSpirit.Create;
    150: Cert := TElectronicScolpionMon.Create;
    151: Cert := TDualEffectMonster.Create;
    152: Cert := TPillarMonster.Create;
    170: Cert := TDarkDevil.Create;
    ...
    183: Cert := TEventMob1.Create;
    184: Cert := TOmaGuardElite.Create;
    //185: Cert := TOmaKingSpirit.Create;
    186: Cert := TManEaterWorm.Create;
    187: Cert := TOrcWarLord.Create;
    188: Cert := TOrcRanger.Create;
    189,190,191,192: Cert := TOrcFighter.Create; //RACE 189 = Normal Attacker, 190 = ThrustAttacker, 191 = StunBlade Attacker, 192 = HalfMoon Attacker
    193: Cert := TOrcSorcerer.Create;
    194: Cert := TOrcGiant.Create;
    //150: Cert := TPlayMonster.Create;
    195: Cert := TIceDemon.Create;
    196: Cert := TIceDemon2.Create;
    197: Cert := TFrostCrunchSorcerer.Create;
    198: Cert := TIceStormSorcerer.Create;
    199: Cert := TFCISSorcerer.Create;
    200: Cert := TIceCommander.Create;
    201: Cert := TIceGeneral.Create;
    202: Cert := TIceNode.Create;
    203: Cert := TIceNode.Create;
  end;

  if Cert <> nil then begin

    Cert.m_CanDropItemList := MonInitialize(Cert, sMonName);
    //MonGetRandomItems(Cert, MonInitialize(Cert, sMonName));
    Cert.m_PEnvir := Envir;
      ...
      if MonGen.nRace > 0 then begin
        if Random(100) < MonGen.nMissionGenRate then begin
          ...
          for i := 0 to nCount - 1 do begin
            ...
            if Cert <> nil then begin
              Cert.m_dwRespawnTime := MonGen.dwZenTime;
              ...
              if (Mongen.nMissionX <> -1) and (Mongen.nMissionY <> -1) then begin
                Cert.m_nSpawnMissionX := Mongen.nMissionX;
                Cert.m_nSpawnMissionY := Mongen.nMissionY;
                Cert.m_boSpawnMission := True;
              end;

            end else begin //if for any reason the system fails to spawn the mob: retry
              Result:= False;
            end;
          ...
          for i := 0 to nCount - 1 do begin
            ...
            end else begin  //if for any reason the system failed to spawn the mob then it should retry
              Result:= False;
            end;
            if boTick and ((GetTickCount - dwStartTick) > g_dwZenLimit) then begin
              Result := False;
...
begin
 ...
 { for I := Low(PlayObject.m_AppendBagItems) to High(PlayObject.m_AppendBagItems) do begin
  ...
  //dont save returnitems cause npc index numbers change on reboots :(
    ...
    if MagicInfo <> nil then begin
      ...
      if HumMagics[i].nInterval > 0 then begin
        ...
        if nMagTick >= HumMagics[i].nInterval then
          ...
          UserMagic.dwInterval := TempTick + (HumMagics[i].nInterval - nMagTick); //sam possible point of error
      end
      else
        UserMagic.dwInterval := 0;
  ...
  for i := 0 to MonsterList.Count - 1 do begin
    ...
    if Monster <> nil then begin
      if (CompareText(Monster.sName, sMonName) = 0) and (BaseObject <> nil) then begin
        ...
        BaseObject.m_btAutoRev := Monster.btAutoRev;
        BaseObject.boCantGetStun := Monster.btCanStun = 1;
        BaseObject.boCantTDBStun := Monster.btCanTDBStun = 1;
        BaseObject.m_btTameValue := Monster.btTameValue;
        BaseObject.m_btSlowResist := Monster.btSlowResist;
        BaseObject.m_btCoreDefense := Monster.btCoreDefense;
        BaseObject.m_btPoisonDefense := Monster.btPoisonDefense;
        BaseObject.m_btNoTurnUndead := Monster.btNoTurnUndead;
        if BaseObject.m_btSlowResist > 100 then
          BaseObject.m_btSlowResist := 100;
...
//  Hour, Min, Sec, MSec: Word;
  ...
  AddList: TList;
  Event: TEvent;
  boWarr, boWizard, boTaos, boAssassin: Boolean;
                ...
                if PlayObject.m_GroupOwner <> nil then begin
                  for n := 0 to PlayObject.m_GroupOwner.m_GroupMembers.Count - 1 do begin
                    ...
                    if (GroupObject <> nil) and (not GroupObject.m_boGhost) and (GroupObject.m_PEnvir = Envir) then begin
                      ...
                      else if GroupObject.m_btJob = 3 then boAssassin := True
                           
                    end;
                  end;
                end;
                if boWarr and boWizard and boTaos and boAssassin then begin
    ...
    if s10[1] = '$' then begin
      ...
      if n10 > 0 then begin
        ...
        if nIdx > -1 then begin
          ...
          MainOutMessage('Script Const： ' + s10 + ' Not Found');
          sMsg := s14 + '????' + s18;
        end;
      end;
    end;
...
{TUserEngine - ChangeItemByLevel - 14/02/2013----------------------------------}
function TUserEngine.ChangeItemByLevel(tempitem: PTStdItem; playerlevel: Word): PTStdItem;
begin
  ...
  if playerlevel > 9 then
    Result := UserEngine.GetStdItem(tempitem.Name + inttostr(Playerlevel div 10));
  if Result = nil then Result := tempitem;
  if Result.Name = '' then Result := tempitem;

  //Result.Name := tempitem.Name;
end;

{TUserEngine - ChangeItemByJob - 14/02/2013------------------------------------}
function TUserEngine.ChangeItemByJob(tempitem: PTStdItem; playerjob: Byte): PTStdItem;
begin
  if tempitem.Name = '' then exit;
  if tempitem.Reserved <> 2 then exit;

  Result := tempitem;
  result := UserEngine.GetStdItem(tempitem.Name + inttostr(playerjob));
  if Result = nil then Result := tempitem;
  if Result.Name = '' then Result := tempitem;
