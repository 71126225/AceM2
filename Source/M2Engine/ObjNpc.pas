...
type
  ...
  private
    ...
    procedure ActionOfGroupPowerRate(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfGroupKillMonExpRate(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    ...
    procedure ActionOfOpenAuction(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    ...
    procedure ConditionOfDay(PlayObject: TPlayObject; QuestConditionInfo: pTQuestConditionInfo; var Result: Boolean);
    ...
    procedure ConditionOfCheckEctype(PlayObject: TPlayObject; QuestConditionInfo: pTQuestConditionInfo; var Result: Boolean);
...
//    bo574: Boolean;
  ...
  //m_GoodsList: TList; //0x588
    ...
    m_boUpgradeNow: Boolean;
    m_boGetBackUpgnow: Boolean;
    m_boSaveRecord: Boolean;
    m_LastSaveTick: longword;
  private
  ...
  public
    ...
    procedure ClientBuyItem(PlayObject: TPlayObject; nInt, nCount, nMakeIndex: Integer; boBindGold: Boolean;sName: string);
    ...
    procedure AddGoods(UserItem:pTUserItem; boLoading: Boolean = False);
    procedure BuyItem(User: TPlayObject; nInt, nFlag: Integer);
    procedure RequestSubList(User: TPlayObject; nint: integer);
    function CanHaveSubGoods(wIndex: integer): Boolean;
  end;
...
begin
  if m_Castle = nil then begin
    PlayObject.MenuMsg('NPC error: No castle found!');
    Exit;
  end;
...
begin
  if m_Castle = nil then begin
    PlayObject.SendDefMsg(Self, SM_MENU_OK, Integer(Self), 0, 0, 0, 'NPC not part of a Castle!');
    Exit;
  end;
...
//          nLabel := GetScriptIndex(s18);
        ...
        if CompareText(s18, sCASTLENAME) = 0 then begin
          ...
          if sMsg <> '' then begin
            ...
            s3C := 'CastleName changed...';
          end
          else begin
            s3C := 'You did not enter a new castle name.';
          end;
          PlayObject.SendDefMsg(Self, SM_MENU_OK, Integer(Self), 0, 0, 0, s3C);
        end
          ...
          case TUserCastle(m_Castle).WithDrawalGolds(PlayObject, StrToIntDef(sMsg, 0)) of
            -4: s3C := 'You have entered an invalid amount, please check and re-enter.';
            -3: s3C := 'This will exceed the maximum gold you are allowed to carry.\Please enter a lower amount.';
            -2: s3C := 'There is not enough gold in the castle bank.';
            -1: s3C := 'Only Guildchief of ' + TUserCastle(m_Castle).m_sOwnGuild + ' can use that service.';
          ...
          case TUserCastle(m_Castle).ReceiptGolds(PlayObject, StrToIntDef(sMsg, 0)) of
            -4: s3C := 'You have entered an invalid amount, please check and re-enter.';
            -3: s3C := 'This will exceed the maximum gold allowed to be held in the bank.\Please enter a lower amount.';
            -2: s3C := 'You do not have enough gold.';
            -1: s3C := 'Only Guildchief of ' + TUserCastle(m_Castle).m_sOwnGuild + ' can use that service.';
            1: GotoLable(PlayObject, 0, False);
          end;
          PlayObject.SendDefMsg(Self, SM_MENU_OK, Integer(Self), 0, 0, 0, s3C);
...
begin
  if m_Castle = nil then begin
    PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'NPC Error: No castle found!');
    Exit;
  end;
      ...
      if TUserCastle(m_Castle).m_Guard[n10].BaseObject = nil then begin
        if not TUserCastle(m_Castle).m_boUnderWar then begin
          ...
          if ObjUnit.BaseObject <> nil then begin
            ...
            PlayObject.SysMsg('Guard hired.', c_Red, t_Hint);
          end;
        end
        else begin
          PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'Unable to hire!');
        end;
      end
    end
    else begin
      PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'Guard already exists in that place.');
    end;
  end
  else begin
    PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'No guard available for this location.');
  end;
  {
      ...
      if TUserCastle(m_Castle).m_Archer[n10].BaseObject = nil then begin
        if not TUserCastle(m_Castle).m_boUnderWar then begin
          ...
          if ObjUnit.BaseObject <> nil then begin
            ...
            //TGuardUnit(ObjUnit.BaseObject).m_nY554 := ObjUnit.nY;
            TGuardUnit(ObjUnit.BaseObject).m_nDirection := 3;
            PlayObject.SysMsg('Archer hired.', c_Red, t_Hint);
          end;
        end
        else begin
          //PlayObject.SysMsg('现在无法雇佣.', c_Red, t_Hint);
          PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'You cannot hire it right now.');
        end;
      end
      else begin
        PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'Archers already exists in that place..');
        //PlayObject.SysMsg('已经雇佣.', c_Red, t_Hint);
      end;
    end
    else begin
      //PlayObject.SysMsg('指令错误.', c_Red, t_Hint);
      PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'No archer available at this location.');
    end;
  end
  else begin
    //PlayObject.SysMsg('城内资金不足.', c_Red, t_Hint);
    PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'Castle funds are insufficient.');
  end;
  {
...
begin
  ...
  if StdItem <> nil then begin
//    if CheckItemType(StdItem.StdMode2) then //unless we want to lock which items can be bought in an npc lol
      Result := StdItem.Price * m_npricerate div 100;
  end;
  //end;
end;

function TMerchant.CanHaveSubGoods(wIndex: integer): Boolean;
var
  modes: TStdModeEx;
  StdMode: TStdMode;
//  nStdIndex: integer;
begin
  result:=false;
  modes := UserEngine.GetStdItemModeEx(wIndex);
  StdMode:= UserEngine.GetStdItemMode(wIndex);
//  nStdIndex := UserEngine.GetStdItem(wIndex).StdMode2;
  if sm_Arming in modes then result:= true;
  if sm_ArmingStrong in modes then result:= true;
  if sm_HorseArm in modes then result:= true;
  if StdMode = tm_House then result:= true; //mounts
  if StdMode = tm_Restrict then result:= true;//cash shop limited use items
  if StdMode = tm_Amulet then result:= true;
  if StdMode = tm_Ore then result := true; //ores
   
end;

procedure TMerchant.RequestSubList(User: TPlayObject; nint: Integer);
var
  sSendMsg: string;
  Goods: pTGoods;
  UserItem : pTUserItem;
...
begin
  sSendMsg := '';
  nCount := 0;
  if (nint >= 0) and (nint < m_GoodsList.count) then begin
    Goods := m_GoodsList[nint];
    if (Goods.SubGoods = nil) or (Goods.SubGoods.Count = 0) then exit;//safety check
    for i := 0 to Goods.SubGoods.Count - 1 do begin
      UserItem := Goods.SubGoods[i];
      if UserItem = nil then continue;
      if UserItem.wIndex = 0 then continue;
      sSendMsg := sSendMsg + UserEngine.MakeClientItem(UserItem) + '/';
      inc(nCount);
    end;
    if sSendMsg <> '' then
      User.SendDefSocket(Self, SM_SENDSUBLIST, Integer(Self), nInt, nCount, 0, sSENDMSG);
  end;
end;

procedure TMerchant.BuyItem(User: TPlayObject; nInt, nFlag: Integer);
  var
    i: Integer;
    sSENDMSG: string;
    Goods: pTGoods;
  ...
  begin
    ...
    begin
      for i := 0 to m_GoodsList.Count - 1 do begin
        ...
        if Goods.SubGoods.Count > 0 then
          UserGoods.nStock := 30000 + UserGoods.nStock;//cheat, not like a servers gonna bother with a 30k stock :p
        sSENDMSG := sSENDMSG + EncodeBuffer(@UserGoods, SizeOf(TUserGoods)) + '/';
      end;
    end;
    //if sSENDMSG <> '' then
    User.SendDefSocket(Self, SM_SENDGOODSLIST, Integer(Self), nInt, nFlag, m_nPriceRate{LoWord(nFlag), HiWord(nFlag)}, sSENDMSG);
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if Goods.nCount > Goods.nStock then //dont lower count sort of deal
          Goods.nStock := Goods.nCount;
        if Goods.nStock > 5000 then Goods.nStock := 5000; //just so it doesnt reach insane amounts
        boNeedSave := True;
      end;
    end;
    if boNeedSave or (m_boSaveRecord and ((GetTickCount() - m_LastSaveTick) > 10*60*1000)) then begin
      m_boSaveRecord := False;
...
begin
  ...
  for i := 0 to m_GoodsList.Count - 1 do begin
    goods := m_GoodsList.Items[i];
    for ii := 0 to Goods.SubGoods.Count - 1 do begin
      Dispose(Goods.SubGoods[ii]);
    end;
    Goods.SubGoods.Free;
    Dispose(goods);
  end;
...
procedure TMerchant.ClientBuyItem(PlayObject: TPlayObject; nInt, nCount, nMakeIndex: Integer; boBindGold: Boolean;sName: string);
  procedure RemoveGoods(Goods: pTGoods);
  var
    i: integer;
    Good: pTGoods;
    nOldIndex: integer;
  begin
    nOldIndex := Goods.btIdx;
    //delete the goods that just ran out
    for i := 0 to m_GoodsList.Count - 1 do begin
      if (Goods = m_GoodsList[i]) then begin
        m_GoodsList.delete(i);
        break;
      end;
    end;
    if Goods.SubGoods <> nil then begin
      for i := 0 to Goods.SubGoods.Count - 1 do
        Dispose(pTUserItem(Goods.SubGoods[i]));
    end;
    Goods.SubGoods.Free;
    dispose(Goods);
    //reindex the excisting goods
    for i := 0 to m_GoodsList.Count - 1 do begin
      Good:= m_GoodsList[i];
      if Good.btIdx >= m_GoodsList.count  then begin
        Good.btIdx := nOldIndex;
        break;
      end;
    end;
  end;
  function FindItem(sItemName: string): pTGoods;
  var
    i:integer;
    goods:pTGoods;
  begin
    ...
    for i := 0 to m_GoodsList.Count - 1 do begin
      goods := m_GoodsList[i];
      if (goods <> nil) and (goods.sItemName = sItemName) then begin
        result := goods;
        exit;
      end;
    end;
...
begin
  ...
  try
    ...
    if (nInt >= 0) and (nInt < m_GoodsList.Count) then begin
      nCheckCode := 3;
      Goods := m_GoodsList.Items[nInt];
      if Goods.sItemName <> sName then begin
        Goods := FindItem(sName);
        if Goods = nil then begin
          n1C := 1;
          goto LbEnd;
        end;
      end;
      //check if we buying a specific item (and find it)
      if (nMakeIndex <> 0) and (Goods.SubGoods <> nil) then begin
        for I := 0 to Goods.SubGoods.count - 1 do begin
          if pTUserItem(Goods.SubGoods[i]).MakeIndex = nMakeIndex then begin
            UserItem := Goods.SubGoods[i];
            nCount := 1;//can only buy one at a time anyway
            nSubGoodIdx:=I;
            break;
          end;
        end;
        if nSubGoodIdx < 0 then begin
          n1C := 1;
          goto LbEnd;
        end;
      end;
      //check if there's subgoods in the goods we're buying from (and sell those first)
      if (UserItem = nil) and (Goods.SubGoods <> nil) and (Goods.SubGoods.count > 0) then begin
        i:= 0;
        while True do begin
          UserItem := Goods.SubGoods[i];
          ...
          inc(i);
          if i > Goods.SubGoods.count then begin //in theory this would be a horrible thing to happen but safety first
            UserItem := nil;
            break;
          end;
        end;
      end;
      //if there's no subgoods just buy a 'new' item
      if UserItem = nil then begin //we're just buying 'A' potion, doesnt matter which
        New(UserItem);
      ...
      begin
        ...
        if StdItem <> nil then begin
          ...
          if (sm_Superposition in StdItem.StdModeEx) and (StdItem.DuraMax > 1) then begin
            ...
            nCount := _Min(nCount, Goods.nStock);
            ...
            if nCount > 1 then //why would you be allowd to buy more then 1 and not even check if there's more then 1 stocked?
              nCount := 1;
          ...
          if (pGold^ >= nPrice) and (nPrice > 0) then begin
            nCheckCode := 7;
            if boPileUp then begin //stacked items
              nCheckCode := 8;
              ...
              if nBack <> -1 then begin
                ...
                if nBack = 2 then begin
                  ...
                  if Stditem.NeedIdentify = 1 then
                    AddGameLog(PlayObject, LOG_ADDITEM, Stditem.Name, UserItem.MakeIndex, UserItem.Dura, m_sCharName,
                      '0', BoolToStr(boBindGold), 'Buy', UserItem);
                  UserItem := nil;
                end
                else begin
                  DisPose(UserItem);
                ...
                nCheckCode := 10;
                n1C := 2;
                if nSubGoodIdx = -1 then begin //better not dispose of subgoods
                  DisPose(UserItem);
                  UserItem := nil;
                end;
                //DisPose(UserItem);
            ...
            else begin //items that dont stack
              nCheckCode := 11;
              n1C := 0;
              Dec(pGold^, nPrice);
              if m_boCastle or g_Config.boGetAllNpcTax then begin
              ...
              for I := 1 to nCount do begin
                ...
                if nBack <> -1 then begin
                  if nSubGoodIdx = -1 then //only make a new makeindex number if we are actualy buying a new item!
                    UserItem.MakeIndex := GetItemNumber();
                  PlayObject.SendAddItem(UserItem);
                  if Stditem.NeedIdentify = 1 then
                    AddGameLog(PlayObject, LOG_ADDITEM, Stditem.Name, UserItem.MakeIndex, UserItem.Dura, m_sCharName,
              ...
              if (nSubGoodIdx = -1) and (UserItem <> nil) then begin //better not dispose of subgoods
                DisPose(UserItem);
                UserItem := nil;
              end;
            end;
    ...
    if n1C = 0 then begin
      ...
      nCheckCode := 19;
      if Assigned(Goods) and (Goods.nStock > 0) then begin
        if nSubGoodIdx > -1 then begin
          //dispose(Goods.SubGoods[nSubGoodIdx]);
          Goods.SubGoods.Delete(nSubGoodIdx);
          m_boSaveRecord := true;
        end;
        Dec(Goods.nStock,nCount);
        if (Goods.nStock <= 0) and (Goods.nCount = 0) then begin
          RemoveGoods(Goods);
          //send client an updated list
          nFlag := 0;
          if m_boBuy then
            nFlag := nFlag or 1;
          if m_boSell then
          ...
          if m_boS_repair then
            nFlag := nFlag or 8;
          SendRefMsg(RM_DEFMESSAGE, SM_UPDATESHOP, Integer(Self),0,0,'');
          m_boSaveRecord := true;
        end;
      ...
      nCheckCode := 18;
      if (UserItem <> nil) and (nSubGoodIdx = -1) then
        Dispose(UserItem);
      nCheckCode := 20;
...
procedure TMerchant.UpgradeWapon(User: TPlayObject);
  ...
  begin
        ...
        if (pStdItem <> nil) and (pStdItem.StdMode in [tm_Necklace, tm_Ring, tm_ArmRing]) then begin
          ...
          case StdItem.StdMode of
            tm_Necklace: begin //004A0421
                nDc := StdItem.nDC + StdItem.nDC2;
                nSc := StdItem.nSC + StdItem.nSC2;
                nMc := StdItem.nMC + StdItem.nMC2;
              end;
            ...
            tm_ArmRing: begin
                nDc := StdItem.nDC + StdItem.nDC2 + 1;
                nSc := StdItem.nSC + StdItem.nSC2 + 1;
                nMc := StdItem.nMC + StdItem.nMC2 + 1;
              end;
          end;
    ...
    btDc := ROUND(nDcMin div 3 + nDcMax div 2);
    btSc := ROUND(nScMin div 3 + nScMax div 2);
    btMc := ROUND(nMcMin div 3 + nMcMax div 2);
    if DelItemList <> nil then
  ...
  try
    ...
    if UpgradeInfo <> nil then begin
      ...
      if ((UpgradeInfo.btDc >= UpgradeInfo.btMc) and (UpgradeInfo.btDc >= UpgradeInfo.btSc)) or (n1C = 0) then begin
        ...
        n10 := _MIN(85, n90 shl 3 + 10 + ((UpgradeInfo.UserItem.Value.btValue[tb_AC2] * 2) - (UpgradeInfo.UserItem.Value.btValue[tb_MAC2] * 2)) + (User.m_nBodyLuckLevel * 2));
...
begin
  //Result := nPrice div 2;
  Result := (nPrice * m_nPriceRate) div 200;
end;

procedure TMerchant.AddGoods(UserItem: pTUserItem; boLoading: Boolean = False);
var
  i : integer;
  StdItem: pTStdItem;
  Goods: pTGoods;
...
begin
  ...
  for i := 0 to m_GoodsList.Count - 1 do begin
    Goods:= m_GoodsList[i];
    if Goods.UserItem.wIndex = UserItem.wIndex then begin
      if UserItem.DuraMax = 0 then begin //used when loading after a reboot
        inc(Goods.nStock, UserItem.dura);
        exit;
      end;
      if CanHaveSubGoods(UserItem.wIndex) then //only added stat stuff basicaly
        Goods.SubGoods.Add(UserItem);
      if Goods.SubGoods.Count > nStockCap then begin //dont store more then 50 'unique' items
        Dispose(pTUserItem(Goods.SubGoods[nStockCap]));
        Goods.SubGoods.Delete(nStockCap);
      end else begin
        if (sm_Superposition in StdItem.StdModeEx) and (Goods.nStock < 5000) then
          inc(Goods.nStock,UserItem.Dura)
        else
          inc(Goods.nStock);
      end;
  ...
  //if the npc doesnt have our new item we sell to it yet, create it
  New(Goods);
  if UserEngine.CopyToUserItemFromNameEx(StdItem.Name, @Goods.UserItem) then begin
    Goods.sItemName := StdItem.Name;
    Goods.nCount := 0;
    if sm_SuperPosition in StdItem.StdModeEx then
      Goods.nStock := UserItem.dura
    else
      Goods.nStock := 1;
    Goods.dwRefillTime:= 60;//no need to restock this :p (but still save every once in a while)
    Goods.dwRefillTick:= GetTickCount();
    Goods.SubGoods := TList.Create;
    if boLoading and (UserItem.DuraMax = 0) {and (not CanHaveSubGoods(UserItem.wIndex))} then begin //used when loading after a reboot
      Goods.nStock := UserItem.dura;
    end else begin
      if CanHaveSubGoods(UserItem.wIndex) then
        Goods.SubGoods.Add(UserItem);
    end;
    Goods.btIdx := m_GoodsList.Count;
...
begin
  ...
  if (nPrice > 0) and (CheckItemType(UserEngine.GetStdItem(UserItem.wIndex).StdMode2)) then begin
    Stditem := Userengine.GetStdItem(UserItem.wIndex);
      ...
      if PlayObject.IncGold(nPrice) then begin
        ...
        if (Stditem.NeedIdentify = 1) then
          AddGameLog(PlayObject, LOG_DELITEM, Stditem.Name, UserItem.MakeIndex, UserItem.Dura, m_sCharName,
            '0', '0', 'Sell', UserItem);
        new(ReturnItem);
        ReturnItem.nNpcindex := integer(self);
        ReturnItem.nSellTick := GetTickCount();
        ReturnItem.UserItem := UserItem;
        PlayObject.m_ReturnItemList.Insert(0, ReturnItem);
        if PlayObject.m_ReturnItemList.Count > 3 then begin
          ReturnItem := PlayObject.m_ReturnItemList[3];
          ReturnNpc :=UserEngine.FindMerchant(TObject(ReturnItem.nNpcindex));
          if ReturnNpc <> nil then begin
            nOldCount := ReturnNpc.m_GoodsList.Count;
            ReturnNpc.AddGoods(ReturnItem.UserItem);
            if nOldCount <> ReturnNpc.m_GoodsList.count then begin
            //if there's a new item added to the npc list, check if we should update the clients list
              if (self = ReturnNpc) then begin
                nFlag := 0;
                if m_boBuy then
                  nFlag := nFlag or 1;
                if m_boSell then
          ...
          dispose(ReturnItem);
          PlayObject.m_ReturnItemList.delete(3);
        end;
        //in theory it should never run below :p
        if PlayObject.m_ReturnItemList.Count > MAXRETURNITEMSCOUNT then begin
          Dispose(pTUserItem(PlayObject.m_ReturnItemList[MAXRETURNITEMSCOUNT]));
          PlayObject.m_ReturnItemList.Delete(MAXRETURNITEMSCOUNT);
        end;
        Result := True;
      end
      else
          ...
          if UserItemLevelArr[i] <> nil then begin
            ...
            if StdItemLevelArr[i].NeedIdentify = 1 then
              AddGameLog(PlayObject, LOG_DELITEM, StdItemLevelArr[I].Name, UserItemLevelArr[I].MakeIndex,
                UserItemLevelArr[I].Dura, m_sCharName, '0', '0', 'Opening', UserItemLevelArr[I]);
...
begin
  ...
  if StdItem = nil then
    exit;
  if not CheckItemType(StdItem.StdMode2) then begin
    PlayObject.SendDefMsg(Self, SM_USERREPAIRITEM_FAIL, 3, 0, 0, 0, '');
    exit;
  end;
  ...
  if (nPrice > 0) and (UserItem.DuraMax > UserItem.Dura) and ((sm_Arming in StdItem.StdModeEx) or (sm_HorseArm in StdItem.StdModeEx)) then begin
    ...
    PlayObject.SendDefMsg(Self, SM_USERREPAIRITEM_FAIL, 2, 0, 0, 0, '');
end;
...
begin
  try
    ...
    for i := 0 to m_GoodsList.Count - 1 do begin
      Good := m_GoodsList[i];
      if Good.SubGoods <> nil then begin
        for ii := 0 to Good.SubGoods.Count - 1 do
          Dispose(pTUserItem(Good.subgoods[II]));
        Good.Subgoods.clear;
      end;
      Dispose(Good);
    end;
...
begin
  ...
  case ProcessMsg.wIdent of
    RM_MAGSTRUCK: begin
        ...
        m_dw568 := GetTickCount();
        Inc(n570);
        ProcessMonSayMsg('Destructive power is ' + IntToStr(nDamage) + ', Average  is ' + IntToStr(n56C div n570));
      end;
    RM_STRUCK: begin
        Inc(n56C, ProcessMsg.wParam);
        m_dw568 := GetTickCount();
        Inc(n570);
        ProcessMonSayMsg('Destructive power is ' + IntToStr(ProcessMsg.wParam) + ', Average  is ' + IntToStr(n56C div n570));
      end;
  end;
end;
...
begin
  ...
  if n570 > 0 then begin
    if (GetTickCount - m_dw568) > 3 * 1000 then begin
      ProcessMonSayMsg('Total destructive power is  ' + IntToStr(n56C) + ', Average destructive power is ' + IntToStr(n56C div n570));
      n570 := 0;
      n56C := 0;
    end;
  end;
...
begin
  ...
  if (AddPlay.m_MyGuild = nil) and (not Guild.IsFull) then begin
    if not (Guild.AddMember(AddPlay)) then begin
      AddPlay.SysMsg('Already joined this guild', c_Red, t_hint);
      exit;
    end;
    ...
    AddPlay.RefShowName();
    AddPlay.SysMsg('Successfully joined the guild: ' + Guild.m_sGuildName, c_red, t_hint);
  end;
end;
...
begin
  ...
  if nWuXin < 1 then begin
    if QuestActionInfo.sParam1 = 'Gold' then
      nWuXin := 1
    else if QuestActionInfo.sParam1 = 'Wood' then
      nWuXin := 2
    else if QuestActionInfo.sParam1 = 'Water' then
      nWuXin := 3
    else if QuestActionInfo.sParam1 = 'Fire' then
      nWuXin := 4
    else if QuestActionInfo.sParam1 = 'Earth' then
      nWuXin := 5;
  end;
...
begin
  ...
  if CompareLStr(QuestActionInfo.sParam1, sTAOS, 3) then
    nJob := 2;
  if CompareLStr(QuestActionInfo.sParam1, sASSASSIN, 3) then
    nJob := 3;
...
begin
  ...
  if nLable = -1 then begin
    ScriptActionError(PlayObject, QuestActionInfo.sParam6 + ' Does not exist！', QuestActionInfo, sSC_SENDCENTERMSG);
    Exit;
  end;
  if (nFColor in [0..255]) and (nBColor in [0..255]) and (nMode in [0..4]) then begin
    ...
    ScriptActionError(PlayObject, 'Parameter error', QuestActionInfo, sSC_SENDCENTERMSG);
end;
...
begin
  ...
  if ObjPlay = nil then begin
    ScriptActionError(PlayObject, QuestActionInfo.sParam1 + ' Not Online ', QuestActionInfo, sSC_HCALL);
    Exit;
  end;
  nIdx := g_ManageNPC.GetScriptIndex(QuestActionInfo.sParam2);
  if nIdx = -1 then begin
    ScriptActionError(PlayObject, QuestActionInfo.sParam2 + ' The Script not exist', QuestActionInfo, sSC_HCALL);
    Exit;
  end;
...
begin
  ...
  case QuestActionInfo.nParam1 of
    ...
    9: PlayObject.SendGroupText(sMsg);
  else begin
...
begin
  ...
  for I := High(PlayObject.m_MissionInfo) downto Low(PlayObject.m_MissionInfo) do begin
    if PlayObject.m_MissionInfo[I].sMissionName = '' then begin
      ...
      ScriptActionError(PlayObject, 'duplicate name', QuestActionInfo, sSC_ADDMISSION);
      exit;
    end;
  end;
  if nIdx <> -1 then begin
...
begin
  ...
  if FGiveItemList.Count > 0 then begin
    ...
    ScriptActionError(PlayObject, 'Missing Hook item', QuestActionInfo, sSC_CHANGEGIVEITEM);
    Exit;
  end;
  StdItem := nil;
  for I := 0 to ItemList.Count - 1 do begin
      ...
      if boUseItem then begin
        ...
        if (StdItem <> nil) and (StdItem.NeedIdentify = 1) then
          AddGameLog(PlayObject, LOG_UPDATEITEM, Stditem.Name, Useritem.MakeIndex, UserItem.Dura, m_sCharName,
            'ChangeGive', 'Hook', 'Script', Useritem);
      end
      else begin
        PlayObject.SendAddItem(UserItem);
        if (StdItem <> nil) and (StdItem.NeedIdentify = 1) then
          AddGameLog(PlayObject, LOG_UPDATEITEM, Stditem.Name, Useritem.MakeIndex, UserItem.Dura, m_sCharName,
            'ChangeGive', 'Give', 'Script', Useritem);
      end;
    end else
    if Stditem.StdMode = tm_House then begin
      case QuestActionInfo.nParam1 of
        ...
        12: begin
            UserItem.dwMaxExp := ChangeInteger(0, 2000000000, UserItem.dwMaxExp, QuestActionInfo.sParam2);
          end;
      end;
    end else begin
      case QuestActionInfo.nParam1 of
        0: begin
            ...
            if (StdItem.StdMode = tm_Ore) then begin//if its Ore then change min dura
              UserItem.Dura := ChangeInteger(0, High(Word), UserItem.Dura, QuestActionInfo.sParam2);
            end else
...
begin
  ...
  if (StdItem = nil) then begin
    ScriptActionError(PlayObject, 'Item does not exist', QuestActionInfo,
      sSC_TAKECOUNT);
    Exit;
  end;
...
begin
  ...
  if (CompareText(QuestActionInfo.sParam1, 'REQUESTUNMARRY') = 0) and (CompareText(QuestActionInfo.sParam2, 'FORCE') = 0) then begin
    ...
    if (CompareText(QuestActionInfo.sParam2, 'FORCE') = 0) then begin
      UserEngine.SendBroadCastMsg('[' + m_sCharName + ']: ' + ' hereby disbands the marriage between #7'
        + PlayObject.m_sCharName + '#7 and #7' + PlayObject.m_sDearName + '#7.', t_Say);
      ...
      if (QuestActionInfo.sParam2 = '') then begin
        if (PoseHuman <> nil) and (PlayObject.m_sDearName = PoseHuman.m_sCharName) then begin
          ...
          if PlayObject.m_boStartUnMarry and PoseHuman.m_boStartUnMarry then begin
            UserEngine.SendBroadCastMsg('[' + m_sCharName + ']: ' + ' confirms that #7'
              + PlayObject.m_sCharName + '#7 and #7' + PoseHuman.m_sCharName + '#7 have opted to divorse.', t_Say);
            PlayObject.m_sDearName := '';
            PoseHuman.m_sDearName := '';
            PlayObject.m_DearHuman := nil;
            PoseHuman.m_DearHuman := nil;
...
begin
  ...
  //nDay := StrToIntDef(QuestConditionInfo.sParam2, -1);
  nDoorStatus := -1;
  if CompareText(QuestConditionInfo.sParam1, 'Damage') = 0 then
    nDoorStatus := 0;
  if CompareText(QuestConditionInfo.sParam1, 'Open') = 0 then
    nDoorStatus := 1;
  if CompareText(QuestConditionInfo.sParam1, 'Close') = 0 then
    nDoorStatus := 2;
...
procedure TNormNpc.ConditionOfCheckEctype(PlayObject: TPlayObject; QuestConditionInfo: pTQuestConditionInfo; var Result: Boolean);
var
  sFBName: string;
  FBList: TList;
  nIdx, I: Integer;
...
begin
  ...
  sFBName := QuestConditionInfo.sParam1;
  if (sFBName = '') then begin
    ScriptConditionError(PlayObject, QuestConditionInfo, sSC_CHECKECTYPE);
    Exit;
  end;
  nIdx := g_FBMapManager.IndexOf(sFBName);
  if nIdx <> -1 then begin
    ...
    for I := 0 to FBList.Count - 1 do begin
      Envir := FBList[I];
      if (g_MapObjectCount[Envir.m_nMapIndex] <= 0) and (not Envir.m_boFBCreate) then
        Result := True;
    end;
  end;
end;
...
begin
  ...
  if nCheck < 0 then begin
    if QuestConditionInfo.sParam2 = 'Nothing' then
      nCheck := 0
    else if QuestConditionInfo.sParam2 = 'Gold' then
      nCheck := 1
    else if QuestConditionInfo.sParam2 = 'Wood' then
      nCheck := 2
    else if QuestConditionInfo.sParam2 = 'Water' then
      nCheck := 3
    else if QuestConditionInfo.sParam2 = 'Fire' then
      nCheck := 4
    else if QuestConditionInfo.sParam2 = 'Earth' then
      nCheck := 5;
  end;
...
begin
  ...
  if CompareLStr(QuestConditionInfo.sParam1, sASSASSIN, 3) then begin
...
begin
  ...
  else if CompareText(QuestConditionInfo.sParam1, 'Male') = 0 then begin
    btSex := 0;
  end
  else if CompareText(QuestConditionInfo.sParam1, 'WOMAN') = 0 then begin
    btSex := 1;
  end
  else if CompareText(QuestConditionInfo.sParam1, 'Female') = 0 then begin
    btSex := 1;
  end;
  PoseHuman := PlayObject.GetPoseCreate();
...
procedure TNormNpc.ActionOfOpenAuction(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
begin
  PlayObject.SendDefMsg(PlayObject, SM_OPENURL, 1, 600, 600, 0, g_Config.sAuctionUrl);
end;
...
begin
  ...
  FNpcCondition[nSC_CHECKDAYTIME] := ConditionOfDay;
  ...
  FNpcCondition[nSC_CHECKECTYPE] := ConditionOfCheckEctype;
  ...
  FNpcAction[nSC_GROUPKILLMONEXPRATE] := ActionOfGroupKillMonExpRate;
  FNpcAction[nSC_POWERRATE] := ActionOfPowerRate;
  FNpcAction[nSC_GROUPPOWERRATE] := ActionOfGroupPowerRate;
  ...
  FNpcAction[nSC_OPENAUCTION] := ActionOfOpenAuction;
end;
...
begin
  ...
  if CompareText(sParam1, 'CHANGEEXP') = 0 then begin
    ...
    PlayObject.SysMsg('Current Experience: ' + IntToStr(PlayObject.m_Abil.Exp) + '/' + IntToStr(PlayObject.m_Abil.MaxExp), c_Green, t_Hint);
    Exit;
  end;
  ...
  if CompareText(sParam1, 'CHANGELEVEL') = 0 then begin
    ...
    PlayObject.SysMsg('Current Level: ' + IntToStr(PlayObject.m_Abil.Level), c_Green, t_Hint);
    Exit;
  end;
...
begin
  ...
  case nIdx of
    ...
    nVAR_LASTKILLEDMOBNAME: begin
        sMsg := sub_49ADB8(sMsg, '<' + sVariable + '>', PlayObject.m_sLastKillMonName);
      end;
    nVAR_LASTGROUPKILLEDMOBNAME: begin
        sMsg := sub_49ADB8(sMsg, '<' + sVariable + '>', PlayObject.m_sLastGrpKillMonName);
      end;
    ...
    nVAR_BODYLUCKCOUNT: begin
        sText := IntToStr(PlayObject.m_nBodyLuckLevel);
        sMsg := sub_49ADB8(sMsg, '<' + sVariable + '>', sText);
    ...
    nVAR_LOGINLONG: begin
        sText := IntToStr((GetTickCount - PlayObject.m_dwLogonTick) div 60000) + 'Minutes';
        sMsg := sub_49ADB8(sMsg, '<' + sVariable + '>', sText);
        ...
        if Castle <> nil then begin
          if not Castle.m_boUnderWar then begin
            ...
            if sText <> '' then begin
              ...
              sMsg := 'No wars are scheduled';
          end
          else
            sMsg := 'War is currently in progress';
        end
        else begin
          sText := '????';
        end;
    ...
    nVAR_GUARDRULE: begin
        ...
        if Castle.m_boGuardRule then
          ...
          sText := 'Normal defense';
        sMsg := sub_49ADB8(sMsg, '<' + sVariable + '>', sText);
      end;
    nVAR_STORAGE2STATE: begin
        if PlayObject.m_boStorageOpen[1] then begin
          sText := FormatDateTime('"Date:" DD/M/YYYY "Time:" HH:NN:SS am/pm', PlayObject.m_dwStorageTime[1]);
        end
        else
          sText := 'Storage Locked';
        sMsg := sub_49ADB8(sMsg, '<' + sVariable + '>', sText);
      end;
    nVAR_STORAGE3STATE: begin
        if PlayObject.m_boStorageOpen[2] then begin
          sText := FormatDateTime('"Date:" DD/M/YYYY "Time:" HH:NN:SS am/pm', PlayObject.m_dwStorageTime[2]);
        end
        else
          sText := 'Storage Locked';
        sMsg := sub_49ADB8(sMsg, '<' + sVariable + '>', sText);
    ...
    nVAR_STORAGE4STATE: begin
        if PlayObject.m_boStorageOpen[3] then begin
          sText := FormatDateTime('"Date:" DD/M/YYYY "Time:" HH:NN:SS am/pm', PlayObject.m_dwStorageTime[3]);
        end
        else
          sText := 'Storage Locked';
        sMsg := sub_49ADB8(sMsg, '<' + sVariable + '>', sText);
    ...
    nVAR_STORAGE5STATE: begin
        if PlayObject.m_boStorageOpen[4] then begin
          sText := FormatDateTime('"Date:" DD/M/YYYY "Time:" HH:NN:SS am/pm', PlayObject.m_dwStorageTime[4]);
        end
        else
          sText := 'Storage Locked';
        sMsg := sub_49ADB8(sMsg, '<' + sVariable + '>', sText);
...
resourcestring
  sOutMessage =
    '[Script Error] %s Script command :%s NPC Name :%s Map :%s(%d:%d) Parameter 1:%s Parameter 2:%s Parameter 3:%s Parameter 4:%s Parameter 5:%s Parameter 6:%s';
begin
...
begin
  sMsg := 'Cmd:' + sCmd +
    ' NPC Name:' + m_sCharName +
    ' Map:' + m_sMapName +
    ' X Y:' + IntToStr(m_nCurrX) + ':' + IntToStr(m_nCurrY) +
    ' Parameter1:' + QuestConditionInfo.sParam1 +
    ' Parameter2:' + QuestConditionInfo.sParam2 +
    ' Parameter3:' + QuestConditionInfo.sParam3 +
    ' Parameter4:' + QuestConditionInfo.sParam4 +
    ' Parameter5:' + QuestConditionInfo.sParam5;
  MainOutMessage('[Script parameter error] ' + sMsg);
end;
...
var
  ...
  I: Integer;
begin
  sMsg := '[Multi-level script error] NPC Name:' + m_sCharName + ' ';
  for i := 0 to List.Count - 1 do begin
...
procedure TNormNpc.ActionOfGroupKillMonExpRate(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
var
  nRate: Integer;
  nTime: Integer;
...
begin
  ...
  nTime := StrToIntDef(QuestActionInfo.sParam2, -1);
  if (nRate < 0) or (nTime < 0) then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_GROUPKILLMONEXPRATE);
    Exit;
  end;
  if (PlayObject.m_GroupOwner <> nil) and (PlayObject.m_GroupOwner.m_GroupMembers.Count > 0) then begin
    for ii := 0 to PlayObject.m_GroupOwner.m_GroupMembers.Count - 1 do begin
      GroupObject := TPlayObject(PlayObject.m_GroupOwner.m_GroupMembers.Objects[ii]);
      if (not GroupObject.m_boDeath) and
        (not GroupObject.m_boGhost) then begin
        if (CompareText(QuestActionInfo.sParam3, '1') = 0) and not (GroupObject.m_sMapName = PlayObject.m_sMapName) then break;
        GroupObject.m_nKillMonExpRate := nRate;
        if g_Config.boExpIsCumulative then
          GroupObject.m_dwKillMonExpRateTime := GroupObject.m_dwKillMonExpRateTime + LongWord(nTime)
        else
          GroupObject.m_dwKillMonExpRateTime := LongWord(nTime);
        GroupObject.ChangeStatusMode(STATUS_EXP, True);
        if g_Config.boShowScriptActionMsg then begin
          GroupObject.SysMsg(format(g_sChangeKillMonExpRateMsg, [GroupObject.m_nKillMonExpRate / 100,
            GroupObject.m_dwKillMonExpRateTime]), c_Green, t_Hint);
        end;
      end;
    end;
  end else begin
  ...
  if n14 >= 0 then begin
    case n14 of //
      ...
      1000..1019: begin
          ...
          if g_boGameLogCustomVariable then begin
            AddGameLog(PlayObject, LOG_CUSTOMVARIABLE, SSTRING_CUSTOMVARIABLE, n14 - 1000, PlayObject.m_CustomVariable[n14 - 1000], m_sCharName,
              'mov', QuestActionInfo.sParam2, 'Script Mov', nil);
          end;
        end;
    else begin
  ...
  if n14 >= 0 then begin
    case n14 of //
      ...
      1000..1019: begin
          ...
          if g_boGameLogCustomVariable then begin
            AddGameLog(PlayObject, LOG_CUSTOMVARIABLE, SSTRING_CUSTOMVARIABLE, n14 - 1000, PlayObject.m_CustomVariable[n14 - 1000], m_sCharName,
              'mul', QuestActionInfo.sParam2 + '/' + QuestActionInfo.sParam3, 'Script mul', nil);
          end;
        end;
    else begin
  ...
  if n14 >= 0 then begin
    case n14 of //
      ...
      1000..1019: begin
          ...
          if g_boGameLogCustomVariable then begin
            AddGameLog(PlayObject, LOG_CUSTOMVARIABLE, SSTRING_CUSTOMVARIABLE, n14 - 1000, PlayObject.m_CustomVariable[n14 - 1000], m_sCharName,
              'percent', QuestActionInfo.sParam2 + '/' + QuestActionInfo.sParam3, 'Script percent', nil);
          end;
        end;
    else begin
...
begin
  ...
  if g_Config.boPowerIsCumulative then  
...
procedure TNormNpc.ActionOfGroupPowerRate(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
var
  nRate: Integer;
  nTime: Integer;
...
begin
  ...
  nTime := StrToIntDef(QuestActionInfo.sParam2, -1);
  if (nRate < 0) or (nTime < 0) then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_GROUPPOWERRATE);
    Exit;
  end;
  if (PlayObject.m_GroupOwner <> nil) and (PlayObject.m_GroupOwner.m_GroupMembers.Count > 0) then begin
    for ii := 0 to PlayObject.m_GroupOwner.m_GroupMembers.Count - 1 do begin
      ...
      if (not GroupObject.m_boDeath) and
        (not GroupObject.m_boGhost) then begin
        if (CompareText(QuestActionInfo.sParam3, '1') = 0) and not (GroupObject.m_sMapName = PlayObject.m_sMapName) then break;
        GroupObject.m_nPowerRate := nRate;
        if g_Config.boPowerIsCumulative then
          GroupObject.m_dwPowerRateTime := _MIN(High(Word),GroupObject.m_dwPowerRateTime + LongWord(nTime))
        else
          GroupObject.m_dwPowerRateTime := LongWord(nTime);
        GroupObject.ChangeStatusMode(STATUS_POW, True);
        if g_Config.boShowScriptActionMsg then begin
          GroupObject.SysMsg(format(g_sChangePowerRateMsg, [GroupObject.m_nPowerRate /
            100, GroupObject.m_dwPowerRateTime]), c_Green, t_Hint);
        end;
      end;
    end;
  end else begin
    PlayObject.m_nPowerRate := nRate;
    if g_Config.boPowerIsCumulative then
      PlayObject.m_dwPowerRateTime := _MIN(High(Word),PlayObject.m_dwPowerRateTime + LongWord(nTime))
    else
      PlayObject.m_dwPowerRateTime := LongWord(nTime);
    PlayObject.ChangeStatusMode(STATUS_POW, True);
    if g_Config.boShowScriptActionMsg then begin
...
begin
  ...
  if CompareText(sItemName, sSTRING_BINDGOLDNAME) = 0 then begin
    ...
    if g_boGameLogBindGold then
      AddGameLog(PlayObject, LOG_BINDGOLDCHANGED, sSTRING_BINDGOLDNAME, 0, PlayObject.m_nBindGold, m_sCharName,
        '+', IntToStr(nItemCount), 'Script Give', nil);
    Exit;
  end;
  if CompareText(sItemName, sSTRING_GOLDNAME) = 0 then begin
    ...
    if (sm_Superposition in StdItem.StdModeEx) and (StdItem.DuraMax > 1) then begin
      ...
      if UserEngine.CopyToUserItemFromNameEx(sItemName, UserItem32) then begin
        while True do begin
          ...
          nBack := PlayObject.AddItemToBag(UserItem, StdItem, False, m_sCharName, 'Buy', AddUserItem);
          if nBack = 2 then begin
            UserItem.MakeIndex := GetItemNumber;
            //FGiveItem := UserItem;
      ...
      if StdItem <> nil then begin
        ...
        if where in [0..MAXUSEITEMS - 1] then begin
          if UserEngine.CopyToUserItemFromIdx(StdItem.Idx + 1, @UserItem34) then begin
            ...
            PlayObject.SendAbility;
            PlayObject.SendSubAbility;
            m_DefMsg := MakeDefaultMsg(SM_TAKEON_AUTO, GetFeatureToLong, 0, 0, where);
            PlayObject.SendSocket(@m_DefMsg, GetFeatureEx + PlayObject.MakeClientItem(@UserItem34));
            PlayObject.FeatureChanged();
            if StdItem.NeedIdentify = 1 then
              AddGameLog(PlayObject, LOG_ADDITEM, Stditem.Name, UserItem34.MakeIndex, UserItem34.Dura, m_sCharName,
                'Give', '0', 'Buy', @UserItem34);
            exit;
          end;
        end;
      end;
...
begin
  ...
  if PlayObject.m_nScriptGotoCount > g_Config.nScriptGotoCountLimit {10} then begin
    MainOutMessage('[Script infinite loop] NPC:' + m_sCharName +
      ' MapXY:' + m_sMapName + '(' + IntToStr(m_nCurrX) + ':' +
      IntToStr(m_nCurrY) + ')' +
...
var
  ii, nGroupDistX, nGroupDistY: integer;
...
begin
  ...
  if PlayObject.m_GroupOwner.m_GroupMembers.Count > 0 then begin
    for ii := 0 to PlayObject.m_GroupOwner.m_GroupMembers.Count - 1 do begin
      GroupMember := TPlayObject(PlayObject.m_GroupOwner.m_GroupMembers.Objects[ii]);
      nGroupDistX := abs(GroupMember.m_nCurrX - m_nCurrX);
      nGroupDistY := abs(GroupMember.m_nCurrY - m_nCurrY);
      if (not GroupMember.m_boDeath) and
      (not GroupMember.m_boGhost) then begin
        if (PlayObject <> GroupMember) then begin
          if (m_PEnvir <> GroupMember.m_PEnvir) or (nGroupDistX > 20) or (nGroupDistY > 20) then continue;
        end;
        GroupMember.SpaceMove(QuestActionInfo.sParam1,QuestActionInfo.nParam2, QuestActionInfo.nParam3, 0);
      end;
    end;
  end;
  bo11 := True;
...
begin
  ...
  if PlayObject.m_GroupOwner.m_GroupMembers.Count > 0 then begin
    for ii := 0 to PlayObject.m_GroupOwner.m_GroupMembers.Count - 1 do begin
      GroupMember := TPlayObject(PlayObject.m_GroupOwner.m_GroupMembers.Objects[ii]);
      nGroupDistX := abs(PlayObject.m_nCurrX - GroupMember.m_nCurrX);
      nGroupDistY := abs(PlayObject.m_nCurrY - GroupMember.m_nCurrY);
      if (not GroupMember.m_boDeath) and
      (not GroupMember.m_boGhost) and
      (GroupMember.m_PEnvir.sMapName = PlayObject.m_PEnvir.sMapName) then begin
        if (nGroupDistX < 20) or (nGroupDistY < 20) then
          GroupMember.MapRandomMove(QuestActionInfo.sParam1, 0);
      end;
    end;
  end;
  bo11 := True;
...
begin
  ...
  if not (btWhere in [0..22]) then begin
    ScriptActionError(PlayObject, 'Equipment location set error', QuestActionInfo, sSC_HOOKITEM);
    Exit;
  end;
  if btWhere in [Low(THumanUseItems)..High(THumanUseItems)] then begin
    if (PlayObject.m_UseItems[btWhere].MakeIndex <= 0) or (PlayObject.m_UseItems[btWhere].wIndex <= 0) then begin
      ScriptActionError(PlayObject, 'The equipment location without data', QuestActionInfo, sSC_HOOKITEM);
      Exit;
    end;
  ...
  if n14 >= 0 then begin
    case n14 of //
      ...
      1000..1019: begin
          ...
          if g_boGameLogCustomVariable then begin
            AddGameLog(PlayObject, LOG_CUSTOMVARIABLE, SSTRING_CUSTOMVARIABLE, n14 - 1000, PlayObject.m_CustomVariable[n14 - 1000], m_sCharName,
              'inc', QuestActionInfo.sParam2, 'Script inc', nil);
          end;
        end;
      6000..6999: begin
  ...
  if n14 >= 0 then begin
    case n14 of //
      ...
      1000..1019: begin
          ...
          if g_boGameLogCustomVariable then begin
            AddGameLog(PlayObject, LOG_CUSTOMVARIABLE, SSTRING_CUSTOMVARIABLE, n14 - 1000, PlayObject.m_CustomVariable[n14 - 1000], m_sCharName,
              'dec', QuestActionInfo.sParam2, 'Script dec', nil);
          end;
        end;
      6000..6999: begin
...
begin
  ...
  else if CompareText(sMapMode, 'NOAFKMINE') = 0 then begin
    if (sParam1 <> '') then begin
...
resourcestring
  sVarFound = 'Variable %s already exists，Variable type :%s';
  sVarTypeError = 'Variable type error，Types of errors :%s This type of support(HUMAN、GUILD、GLOBAL)';
begin
  Result := False;
  boFoundVar := False;
...
procedure TNormNpc.ConditionOfDay(PlayObject: TPlayObject; QuestConditionInfo: pTQuestConditionInfo; var Result:
  Boolean);
var
  HourOfDay: integer;
  TypeOfDay: Integer;
...
begin
  ...
  case HourOfDay of
    0..1, 6..7, 12..13, 18..19: typeofday := 1;
    2, 8, 14, 20: typeofday := 2;
    3..4, 9..10, 15..16, 21..22: typeofday := 3;
    5, 11, 17, 23: typeofday := 4;
  end;
  ...
  if (QuestConditionInfo.sParam1 = 'DAY') then begin
    if (typeofday = 1) then result := True;
      nCheck := 1;
  end;
  if (QuestConditionInfo.sParam1 = 'SUNRISE') then begin
    if (typeofday = 2) then result := True;
      nCheck := 2;
  end;
  if (QuestConditionInfo.sParam1 = 'NIGHT') then begin
    if (typeofday = 3) then result := True;
      nCheck := 3;
  end;
  if (QuestConditionInfo.sParam1 = 'DUSK') then begin
    if (typeofday = 4) then result := True;
      nCheck := 4;
  end;
  if not (nCheck in [0..5]) then begin
    ScriptConditionError(PlayObject, QuestConditionInfo, sSC_CHECKDAYTIME);
    Exit;
  end;
end;
...
begin
  ...
  nCheck := StrToIntDef(QuestConditionInfo.sParam1, -1);
  if nCheck < 0 then begin
    if QuestConditionInfo.sParam1 = 'None' then
      nCheck := 0
    else if QuestConditionInfo.sParam1 = 'Gold' then
      nCheck := 1
    else if QuestConditionInfo.sParam1 = 'Wood' then
      nCheck := 2
    else if QuestConditionInfo.sParam1 = 'Water' then
      nCheck := 3
    else if QuestConditionInfo.sParam1 = 'Fire' then
      nCheck := 4
    else if QuestConditionInfo.sParam1 = 'Earth' then
      nCheck := 5;
  end;
  if not (nCheck in [0..5]) then begin
...
begin
  ...
  if (UserItem.wIndex <= 0) or (StdItem = nil) then begin
    PlayObject.SysMsg('You are not wearing the correct items.', c_Red, t_Hint);
    Exit;
  end;
  nRate := Random(nRate);
  ...
  if nRate <> 0 then begin
    PlayObject.SysMsg('Equipment upgrade failed.', c_Red, t_Hint);
    Exit;
  end;
  nAddPoint := nPoint;
  ...
  PlayObject.SendUpdateItem(UserItem);
  PlayObject.SysMsg('Equipment upgrade success', c_Green, t_Hint);
...
begin
  ...
  boDeleteExprie := CompareText(QuestConditionInfo.sParam6, 'Clear Up') = 0;
  ...
  if FileExists(sListFileName) then begin
    ...
    for i := 0 to LoadList.Count - 1 do begin
      ...
      if (CompareText(sHumName, PlayObject.m_sCharName) = 0) or
        ...
        begin
          if CompareText(QuestConditionInfo.sParam7, 'Year') = 0 then
            nDay := _MAX(0, YearsBetween(Now(), dOldDate) * CompareDateTime(Now(), dOldDate))
          else if CompareText(QuestConditionInfo.sParam7, 'Week') = 0 then
            nDay := _MAX(0, WeeksBetween(Now(), dOldDate) * CompareDateTime(Now(), dOldDate))
          else if CompareText(QuestConditionInfo.sParam7, 'Month') = 0 then
            nDay := _MAX(0, MonthsBetween(Now(), dOldDate) * CompareDateTime(Now(), dOldDate))
          else if CompareText(QuestConditionInfo.sParam7, 'Hour') = 0 then
            nDay := _MAX(0, HoursBetween(Now(), dOldDate) * CompareDateTime(Now(), dOldDate))
          else if CompareText(QuestConditionInfo.sParam7, 'Minute') = 0 then
  ...
  try
    if g_GuildManager.FindGuild(sGuildName) <> nil then begin
      if PlayObject.m_nGold >= g_Config.nGuildWarPrice then begin
        ...
        PlayObject.SendDefMessage(SM_MENU_OK, Integer(Self), 0, 0, 0, 'You do not have enough Gold.');
        //PlayObject.SysMsg('你没有足够的金币.', c_Red, t_Hint);
      end;
    end
    else begin
      //PlayObject.SysMsg('行会 ' + sGuildName + ' 不存在.', c_Red, t_Hint);
      PlayObject.SendDefMessage(SM_MENU_OK, Integer(Self), 0, 0, 0, 'The Guild[' + sGuildName + '] does not exist.');
    end;
  except
    on E: Exception do begin
    ...
    if PlayObject.IsGuildMaster and (Castle <> nil) and (not Castle.IsMember(PlayObject)) then begin
      ...
      if UserItem <> nil then begin
        ...
        if Castle.AddAttackerInfo(TGuild(PlayObject.m_MyGuild)) then begin
          ...
          PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'Unable to request War.');
        end;

      end
      else begin
        PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0,
          'You need one ' + g_Config.sZumaPiece + '.');
        //PlayObject.SysMsg('你没有' + g_Config.sZumaPiece + '.', c_Red, t_Hint);
      end;
    end
    else begin
      PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'Your request has been canceled.');
...
begin
  if m_Castle = nil then begin
    PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'NPC not part of a Castle.');
    //PlayObject.SysMsg('NPC不属于城堡.', c_Red, t_Hint);
    Exit;
  end;
  if TUserCastle(m_Castle).m_boUnderWar then begin
    PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'Can not be repaired during Siege.');
    exit;
  end;
  if TUserCastle(m_Castle).m_nTotalGold >= g_Config.nRepairDoorPrice then begin
    if TUserCastle(m_Castle).RepairDoor then begin
      ...
      PlayObject.SysMsg('Repair successful.', c_Red, t_Hint);
      //PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, '修理成功.');
    end
    else begin
      PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'Gates need repairing.');
      //PlayObject.SysMsg('城门不需要修理.', c_Green, t_Hint);
    end;
  end
  else begin
    PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'Insufficient funds.');
    //PlayObject.SysMsg('城内资金不足.', c_Red, t_Hint);
  end;
  {
...
begin
  if m_Castle = nil then begin
    PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'NPC is not part of a Castle.');
    //PlayObject.SysMsg('NPC不属于城堡.', c_Red, t_Hint);
    Exit;
  end;
  if TUserCastle(m_Castle).m_boUnderWar then begin
  ...
  if TUserCastle(m_Castle).m_nTotalGold >= g_Config.nRepairWallPrice then begin
    if TUserCastle(m_Castle).RepairWall(nWallIndex) then begin
      ...
      PlayObject.SendDefMsg(PlayObject, SM_MENU_OK, Integer(Self), 0, 0, 0, 'Walls need repairing.');
    end;
  end
  else begin
    //PlayObject.SysMsg('城内资金不足.', c_Red, t_Hint);
