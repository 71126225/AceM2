...
begin
  ...
  StringGridMonDropLimit.Cells[3, 0] := 'Start Time';
  ButtonMonDropLoadClick(ButtonMonDropLoad);

  StringGridUserCmd.Cells[0, 0] := 'CMD Name';
...
{$IF SoftVersion = VERDEMO}
  Caption := 'View a list of information [demo version, all the settings adjusted effectively, but can not save]';
{$IFEND}
  ...
  if sName <> '' then begin
    for I := 1 to StringGridMonDropLimit.RowCount - 1 do begin
      if CompareText(sName, StringGridMonDropLimit.Cells[0, I]) = 0 then begin
        Application.MessageBox('This item is already in the list！', 'Error Message', MB_OK +  MB_ICONINFORMATION);
        Exit;
      end;
    end;
...
var
  I: Integer;
begin
  if Application.MessageBox('Are you sure you want to clear？', 'Confirmation', MB_OKCANCEL + MB_ICONQUESTION) = IDOK then
  begin
    for I := 1 to StringGridMonDropLimit.RowCount - 1 do begin
  ...
  if ListBoxitemList2.ItemIndex >= 0 then begin
    for i := 0 to ListBoxGameLogList.Items.Count - 1 do begin
      if ListBoxGameLogList.Items.Strings[i] =
        ...
        Application.MessageBox('This item is already in the list!!!', 'Error Message', MB_OK + MB_ICONERROR);
        Exit;
      end;
    end;
...
{$IFEND}
  if
    Application.MessageBox('This setting will cause te items database to reload, Do you want to continue?', 'Confirmation', MB_YESNO + MB_ICONQUESTION) = mrYes then begin
    FrmDB.LoadItemsDB();
    RefLoadItems;
  end;
end;
  ...
  if sName <> '' then begin
    for I := 1 to StringGridUserCmd.RowCount - 1 do begin
      if CompareText(sName, StringGridUserCmd.Cells[0, I]) = 0 then begin
        Application.MessageBox('The Command name is already exists！', 'Error Message', MB_OK +  MB_ICONINFORMATION);
        Exit;
      end else
      if CompareText(sID, StringGridUserCmd.Cells[1, I]) = 0 then begin
        Application.MessageBox('The command identifies existing！', 'Error Message', MB_OK +  MB_ICONINFORMATION);
        Exit;
      end;
    end;
...
begin
  ...
  SaveList.Add(';Custom GM Commands');
  SaveList.Add(';List #1-9');
  Try
    g_UserCmdList.Clear;
    for I := 1 to StringGridUserCmd.RowCount - 1 do begin
...
begin
  if (ssCtrl in Shift) and (Key = word('F')) then begin
    sIPaddr := InputBox('Find Item', 'Please Enter the name of item', '');
...
begin
 { if ListBox3.Items.Count < 1 then begin
  ...
  for I := 0 to GroupBox10.ControlCount - 1 do begin
    if (GroupBox10.Controls[I] is TEdit) and (Trim(TEdit(GroupBox10.Controls[I]).Text) <> '') then begin
      if ListBoxSetItems.Items.IndexOf(Trim(TEdit(GroupBox10.Controls[I]).Text)) = -1 then begin
        Application.MessageBox(PChar(Trim(TEdit(GroupBox10.Controls[I]).Text) + 'Item Name setting error!'), 'Message', MB_OK + MB_ICONERROR);
        Exit;
      end;
  ...
  if nCount < 1 then begin
    Application.MessageBox('Need to specify at least a piece of equipment in order to suit!', 'Message', MB_OK + MB_ICONERROR);
    Exit;
  end;
  for I := 0 to ListViewSuit.Items.Count - 1 do begin
    Item := ListViewSuit.Items[I];
    if CompareText(Item.SubItems[0], sTempStr) = 0 then begin
      Application.MessageBox('Has the same suit and equipment configuration!', 'Message', MB_OK + MB_ICONERROR);
      Exit;
    end;
  end;
  sTempStr2 := '';
  ...
  if sTempStr2 = '' then begin
    Application.MessageBox('Need to set at least a suit property!', 'Message', MB_OK + MB_ICONERROR);
    Exit;
  end;
  Item := ListViewSuit.Items.Add;
...
begin
  if ListViewSuit.ItemIndex = -1 then begin
    Application.MessageBox('Please select the package you want to modify!', 'Message', MB_OK + MB_ICONERROR);
    Exit;
  end;
...
begin
  if ListViewSuit.ItemIndex = -1 then begin
    Application.MessageBox('Please select the package that you want to delete!', 'Message', MB_OK + MB_ICONERROR);
    Exit;
  end;
...
begin
  ...
  if (nAdminPerMission < 1) or (sAdminName = '') or not (nAdminPerMission in
    [0..10]) then begin
    Application.MessageBox('Incorrect values.', 'Error', MB_OK + MB_ICONERROR);
    EditAdminName.SetFocus;
    Exit;
  end;
{$IF VEROWNER = WL}
  if (sAdminIPaddr = '') then begin
    Application.MessageBox('Ip address required.', 'Error', MB_OK + MB_ICONERROR);
  ...
  try
    for i := 0 to UserEngine.m_AdminList.Count - 1 do begin
      if CompareText(pTAdminInfo(UserEngine.m_AdminList.Items[i]).sChrName, sAdminName) = 0 then begin
        Application.MessageBox('Admin already excists.', 'Error', MB_OK + MB_ICONERROR);
        Exit;
      end;
    end;
    New(AdminInfo);
