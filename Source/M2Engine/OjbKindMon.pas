...
interface
uses
  Windows, Classes, Grobal2, ObjBase, ObjMon, Envir, HUtil32;
...
procedure TAomaKingMonster.Run;
var
  nAttackDir:Integer;
  nX,nY:Integer;
begin
  if (CanPerformActions) then begin
    ...
    if m_TargetCret <> nil then begin
      ...
      if (nX <= 2) and (nY <= 2) then begin
        if Integer(GetTickCount - m_dwThrustTime) > (m_nNextHitTime + Random(m_nNextHitTime * 3)) then begin
          m_dwThrustTime:=GetTickCount;
...
procedure TAomaKingMonster.LightingAttack(nDir: Integer);
var
  WAbil:pTAbility;
...
begin
  ...
  if m_PEnvir.GetNextPosition(m_nCurrX,m_nCurrY,nDir,1,nSX,nSY) then begin
    ...
    WAbil:=@m_WAbil;
    nPwr:=(Random(SmallInt(HiWord(m_WAbil.MC) - LoWord(m_WAbil.MC)) + 1) +
      LoWord(m_WAbil.MC));
        ...
        for i := 0 to m_VisibleActors.Count - 1 do begin
          ...
          if BaseObject <> nil then begin
            if not BaseObject.m_boDeath then begin
              if IsProperTarget(BaseObject) and (not BaseObject.IsHidden or m_boCoolEye) and
