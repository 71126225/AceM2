...
type
  ...
  public
    constructor Create(Creat: TBaseObject; nX, nY: Integer; nType: Integer; nTime, nDamage: Integer; Magic: pTUserMagic);
    destructor Destroy; override;
    procedure Run(); override;
  end;
...
constructor TPoisonCloudEvent.Create(Creat: TBaseObject; nX, nY, nType, nTime,
  nDamage: Integer; Magic: pTUserMagic);
begin
  inherited Create(Creat.m_PEnvir, nX, nY, nType, nTime, True);
  if m_Envir <> nil then begin
    m_boSetMap := True;
...
destructor TPoisonCloudEvent.Destroy;
begin
  if (m_Envir <> nil) and m_boSetMap then begin
    m_boSetMap := False;
    m_Envir.SetFireBurn(m_nX, m_nY, False);
      ...
      for I := 0 to BaseObjectList.Count - 1 do begin
        ...
        if (TargeTBaseObject <> nil) and (m_OwnBaseObject <> nil) and (GetTickCount > TargeTBaseObject.m_dwFireBurnTick) and
          ...
          if TargeTBaseObject.m_btRaceServer = RC_PLAYOBJECT then
            ...
            TargeTBaseObject.m_dwFireBurnTick := GetTickCount + g_Config.nFireMonDamageTimeRate;
          TargeTBaseObject.SendMsg(m_OwnBaseObject, RM_MAGSTRUCK_MINE, 0, m_nDamage, 0, 0, '');
           nPoisonPower := (GetPower13(40) + GetRPow(m_OwnBaseObject.m_WAbil.SC) * 2) + Random(m_OwnBaseObject.m_nPoisonAttack);
           TargeTBaseObject.SendDelayMsg(m_OwnBaseObject, RM_POISON,
            POISON_DECHEALTH , nPoisonPower + Random(m_OwnBaseObject.m_nPoisonAttack), Integer(m_OwnBaseObject),
            ROUND(UserMagic.btLevel / 3 * (nPoisonPower /
            g_Config.nAmyOunsulPoint)) {UserMagic.btLevel}, '', 0);
        end;
      end;
    end;
    BaseObjectList.Free;
