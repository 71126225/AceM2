...
begin
  ...
  GridUserItem.Cells[9, 0] := 'Added States';
  ...
  GridUserItem.Cells[0, 6] := 'Left Brace';
  GridUserItem.Cells[0, 7] := 'Right Brace';
  ...
  GridUserItem.Cells[0, 12] := 'Boots';
  GridUserItem.Cells[0, 13] := 'Stone';
  GridUserItem.Cells[0, 14] := 'Mount';
  GridUserItem.Cells[0, 15] := 'µ¿æﬂ';

  GridBagItem.Cells[0, 0] := 'Item Slot';
  ...
  GridBagItem.Cells[9, 0] := 'Added States';

  GridStorageItem.Cells[0, 0] := 'Item Slot';
  ...
  GridStorageItem.Cells[9, 0] := 'Added States';
...
begin
  ...
  EditLogonLong.Text := IntToStr((GetTickCount - PlayObject.m_dwLogonTick) div (60 * 1000)) + ' ∑÷÷”';
...
begin
  ...
  if (nLevel < 0) or (nLevel > High(Word)) or (nGold < 0) or (nGold > MAXINTCOUNT) or (nBindGold < 0) or (nBindGold > MAXINTCOUNT)
    ...
    MessageBox(Handle, 'Input Data is incorrect.', 'Error', MB_OK);
    Exit;
  end;
  ...
  MessageBox(Handle, 'Character data has been saved', 'Message', MB_OK);
end;
