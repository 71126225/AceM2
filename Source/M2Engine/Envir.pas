...
{
********************************************************************************
********************vars required to read mapfiles******************************
********************************************************************************
}
//map version 0 (original mir2 wemade)
  ...
  //map version 3 (shanda's 2012 format) no more xors but loads of extra data
   TSdoMapInfo2012 = packed record
    ...
    AniTileTiles: byte;            //total amount of tiles used in a single animation (basicaly an offset value so frame 1 = tile 1, frame 2 = tile 2 + this value etc
...
{
********************************************************************************
**************************end of vars for mapfile reading***********************
********************************************************************************
}
...
//    nServerIndex: Integer; //0x14
    ...
    m_boFBIsJob: Boolean;
    m_density_of_dark: Integer;
    m_colour_of_dark: String;
  private
  ...
  public
    ...
    function MoveToMovingObject(nCX, nCY: Integer; Cert: TObject; nX, nY: Integer; boFlag: Boolean; boNeverStack: Boolean = False): Integer;
...
begin
  ...
  Result.m_density_of_dark := MapFlag.density_of_dark;
  Result.m_colour_of_dark := MapFlag.colour_of_dark;
  Result.m_boDAY := MapFlag.boDAY;
  Result.m_boTwilight := MapFlag.boTwilight;
  Result.m_nThunder := MapFlag.nThunder;
  Result.m_nLava := MapFlag.nLava;
  Result.m_nMineType := MapFlag.nMineType;
  ...
  Result.m_boAFKMINE := MapFlag.boAFKMINE;
          ...
          if not boNeedAdd then begin
            for I := 0 to MapCellInfo.ObjList.Count -1 do begin
              ...
              if OSObject.btType = OS_ITEMOBJECT then
                inc(nItemCount);
              if nItemCount > 5 then begin
                Result := nil;
                boNeedAdd := True;
              end;
            end;
          //and (MapCellInfo.ObjList.Count >= 5) then begin
...
function TEnvirnoment.MoveToMovingObject(nCX, nCY: Integer; Cert: TObject; nX, nY: Integer; boFlag: Boolean; boNeverStack: Boolean = False): Integer;
var
  MapCellInfo: pTMapCellinfo;
            ...
            if pTOSObject(MapCellInfo.ObjList.Items[I]).btType = OS_MOVINGOBJECT then begin
              ...
              if BaseObject <> nil then begin
                if not BaseObject.m_boGhost
                  ...
                  if boNeverStack or ((TBaseObject(Cert).m_btRaceServer <> RC_PLAYOBJECT) or (GetTickcount - pTOSObject(MapCellInfo.ObjList.Items[I]).dwAddTime > 700)) then begin
                    bo1A := False;
                    break;
                  end;
                end;
          ...
          if OSObject.btType = OS_MOVINGOBJECT then begin
            ...
            if BaseObject <> nil then begin
              ...
              if not (g_Config.boWarDisHumRun and (Castle <> nil) and (Castle.m_boUnderWar)) then begin
                if BaseObject.m_btRaceServer = RC_PLAYOBJECT then begin
                  if (g_Config.boRUNHUMAN and not m_boRUNHUMAN) or (not g_Config.boRUNHUMAN and m_boRUNHUMAN) or
                   (GetTickCount - OSObject.dwAddTime < 500) then
                    Continue;
              end
              else if BaseObject.m_btRaceServer = RC_NPC then begin
                  ...
                  //不允许穿过练功师和宝箱
                  if (g_Config.boRUNMON and not m_boRUNMON) or (not g_Config.boRUNMON and m_boRUNMON) or (GetTickCount - OSObject.dwAddTime < 500) then
                    Continue;
              end;
                // end;
              // end;
...
begin
  ...
  m_density_of_dark := 100;
  m_colour_of_dark := '255,255,255';
end;
...
//  fHandle: Integer;
  ...
  //used in identifying the type of map
  //boENMap, boNewMap: Boolean;
  bMapType: byte;
  //map data arrays
  ENMapData: array of array of TENMapInfo;
  MapData: array of array of TMapInfo;
...
begin
  ...
  if FileExists(sMapFile) then begin
    aMapFile := TFileStream.Create(sMapFile, fmOpenRead or fmShareDenyNone);
    //opened the file now read it
    try
      ...
      if (ENMapHeader.Title = '') then begin //mir3 wemade maps dont have a title
        ...
        m_nWidth := EIMapHeader.width;
        m_nHeight := EIMapHeader.height;
      end else
      if (ENMapHeader.Title = '(C) SNDA, MIR3.') then begin //shanda's mir3 map files have this title
        ...
        m_nWidth := ShandaEIHeader.width;
        m_nHeight := ShandaEIHeader.height;
      end else
      if Pos('Mir2 AntiHack',ENMapHeader.Title) = 1 then begin //wemades encrypted map format for labyrinths
        ...
        m_nWidth := AntiHackHeader.wWidth xor AntiHackHeader.wXOR;
        m_nHeight := AntiHackHeader.wHeight xor AntiHackHeader.wXOR;
      end else
      if ENMapHeader.Title = 'Map 2010 Ver 1.0' then begin
        bMapType := 1;
        m_nWidth := ENMapHeader.Width xor XORWORD;
        m_nHeight := ENMapHeader.Height xor XORWORD;
      end
      else begin
      ...
      MainOutMessage('Failed to read map header info for: ' + sMapFile);
    end;

    if (m_nWidth > 1000) or (m_nHeight > 1000) then exit;//cant read maps if they are over 1000*1000 coords
    Initialize(m_nWidth, m_nHeight);
    //read whatever format we have and convert it to type 2 for future processing
    SetLength(MapData, m_nWidth, m_nHeight);
    try
      case bMapType of
        0: begin //original mir2 type
          aMapFile.Seek(SizeOf(header), 0);
          SetLength(OldMapData, m_nWidth, m_nHeight);
          for i := 0 to m_nWidth - 1 do begin
            aMapFile.Read(OldMapData[i, 0], sizeof(TOldMapInfo) * m_nHeight);
            for j := 0 to m_nHeight - 1 do
              Move(OldMapData[i, j], MapData[i, j], SizeOf(TOldMapInfo));
          end;
        end;
        1: begin //shanda 2010 mir2 type
          SetLength(ENMapData, m_nWidth, m_nHeight);
          for i := 0 to m_nWidth - 1 do begin
            aMapFile.Read(ENMapData[i, 0], SizeOf(TENMapInfo) * m_nHeight);
            for j := 0 to m_nHeight - 1 do begin
              MapData[i, j].BkImg := ENMapData[i, j].BkImg xor XORWORD;
              if (ENMapData[i, j].BkImgNot xor $AA38) = $2000 then
                MapData[i, j].BkImg := MapData[i, j].BkImg or $8000;
              MapData[i, j].FrImg := ENMapData[i, j].FrImg xor XORWORD;
              MapData[i, j].DoorIndex := ENMapData[i, j].DoorIndex;
              MapData[i, j].DoorOffset := ENMapData[i, j].DoorOffset;
            end;
          end;
        end;
        2: begin //wemades newer mir2 type
          aMapFile.Seek(SizeOf(header), 0);
          for i := 0 to m_nWidth - 1 do begin
            aMapFile.Read(MapData[i, 0], sizeof(TMapInfo) * m_nHeight);

          end;
        end;
        3: begin //shanda's 2012 mir2 type
          aMapFile.Seek(SizeOf(header), 0);
          SetLength(supernewMapData, m_nWidth, m_nHeight);
          for i := 0 to m_nWidth - 1 do begin
            aMapFile.Read(supernewMapData[i, 0], sizeof(TsdoMapInfo2012) * m_nHeight);
            for j := 0 to m_nHeight - 1 do begin
              MapData[i, j].BkImg := supernewMapData[i, j].BkImg;
              MapData[i, j].FrImg := supernewMapData[i,j].FrImg;
              MapData[i, j].DoorIndex := supernewMapData[i,j].DoorIndex;
              MapData[i, j].DoorOffset := supernewMapData[i,j].DoorOffset;
            end;
          end;
        end;
        4: begin //wemades antihack mir2 type
          ...
          for i := 0 to m_nWidth - 1 do begin
            ...
            for j := 0 to m_nHeight - 1 do begin
              Move(OldMapData[i, j], MapData[i, j], SizeOf(TOldMapInfo));
              MapData[i,j].BkImg := MapData[i,j].BkImg xor AntiHackHeader.wXOR;
              MapData[i,j].MidImg := MapData[i,j].MidImg xor AntiHackHeader.wXOR;
              MapData[i,j].FrImg := MapData[i,j].FrImg xor AntiHackHeader.wXOR;
            end;
          end;
        end;
        5: begin //wemades mir3 type
          SetLength(EIMapData, m_nWidth, m_nHeight);
          aMapFile.Seek(SizeOf(EIMapHeader) + ((m_nWidth div 2) * (m_nHeight div 2))*sizeof(TEITileInfo),0);
          for i := 0 to m_nWidth - 1 do begin
            aMapFile.Read(EIMapData[i, 0], sizeof(TEIMapInfo) * m_nHeight);
            for j := 0 to m_nHeight - 1 do begin
              //MapData[i,j].DoorIndex := EIMapData[i,j].DoorIndex;
              //MapData[i,j].DoorOffset := EIMapData[i,j].DoorOffset;
              if Boolean(EIMapDAta[i,j].bflag and $01) = false then MapData[i,j].BkImg := $8000;
              if Boolean(EIMapDAta[i,j].bflag and $02) = false then MapData[i,j].FrImg := $8000;
            end;
          end;
        end;
        6: begin //shanda's mir3 type
          aMapFile.Seek(SizeOf(TShandaEIMapHeader), 0);
          SetLength(ShandaEIMapData, m_nWidth, m_nHeight);
          for i := 0 to m_nWidth - 1 do begin
            aMapFile.Read(ShandaEIMapData[i, 0], sizeof(TShandaEIMapInfo) * m_nHeight);
            for j := 0 to m_nHeight - 1 do begin
              if Boolean(ShandaEIMapData[i,j].bflag and $01) = false then MapData[i,j].BkImg := $8000;
              if Boolean(ShandaEIMapData[i,j].bflag and $02) = false then MapData[i,j].FrImg := $8000;
            end;
          end;
        end;
      end;
    except
      MainOutMessage('Failed to read map body for: ' + sMapFile);
    end;
    try
      for nW := 0 to m_nWidth - 1 do begin
          n24 := nW * m_nHeight;
          for nH := 0 to m_nHeight - 1 do begin
            if (MapData[nW, nH].BkImg) and $8000 <> 0 then begin
              MapCellInfo := @MapCellArray[n24 + nH];
              MapCellInfo.chFlag := 1;
            end;
            if MapData[nW, nH].FrImg and $8000 <> 0 then begin
              MapCellInfo := @MapCellArray[n24 + nH];
              MapCellInfo.chFlag := 2;
            end;
            if MapData[nW, nH].DoorIndex and $80 <> 0 then begin
              Point := (MapData[nW, nH].DoorIndex and $7F);
      ...
      MainOutMessage('Failed to convert map info for: ' + sMapFile);
    end;
    //cleanup
    ENMapData:= nil;
    MapData:= nil;
...
begin
  sMsg :=
    'MAP:%s(%s) DAY:%s TWILIGHT: %s DARK:%s SAFE:%s FIGHT:%s FIGHT3:%s QUIZ:%s NORECONNECT:%s(%s) MUSIC:%s(%d) EXPRATE:%s(%f) PKWINLEVEL:%s(%d) PKLOSTLEVEL:%s(%d) PKWINEXP:%s(%d) PKLOSTEXP:%s(%d) DECHP:%s(%d/%d) INCHP:%s(%d/%d)';
  sMsg := sMsg +
    ' DECGAMEGOLD:%s(%d/%d) INCGAMEGOLD:%s(%d/%d) INCGAMEPOINT:%s(%d/%d) RUNHUMAN:%s RUNMON:%s NEEDHOLE:%s NORECALL:%s NOGUILDRECALL:%s NODEARRECALL:%s NOMASTERRECALL:%s NODRUG:%s MINE:%s AFKMINE:%s NOPOSITIONMOVE:%s';
  Result := format(sMsg, [sMapName,
    sMapDesc,
      BoolToCStr(m_boDAY),
      BoolToCStr(m_boTwilight),
      BoolToCStr(m_boDARK),
      BoolToCStr(m_boSAFE),
