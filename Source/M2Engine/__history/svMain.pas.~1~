...
begin
  ...
  if FileExists('.\!servertable.txt') then begin
    ...
    ShowMessage('File !servertable.txt Not Found.');
  end;
end;
...
begin
  if g_MapEventNpc <> nil then begin
    ...
    //FrmDB.LoadMapEvent;
    MainOutMessage('QMapEvent Reloaded...');
  end;
end;
...
begin
  ...
  if nCode < 0 then begin
    MainOutMessage('Failed to reload Boxes .' + 'Code: ' + IntToStr(nCode));
  end else
    MainOutMessage('Boxes Reloaded Successfully...');
end;
...
begin
  FrmDB.LoadMapEvent;
  MainOutMessage('MapEvent Reloaded...');
end;
...
{$IF Var_Free = 1}      // - 免费版
  Caption := sCaption + 'Free');
{$ELSE}
  {$IF Public_Ver = Public_Test}    // - 10人限制测试版
    Caption := sCaption + 'TestMode';
  {$ELSE}
    if not g_boTestServer then Caption := sCaption + 'Release'
...
begin
  ...
  MainOutMessage('Mission Data Reloaded...');
end;
...
{$ELSE}
    FrmDB.Query.ConnectionString := 'Driver={SQL Server Native Client 10.0};Server=' + sSQLInstance + ';Database=' + sSQLDatabase + ';Trusted_Connection=yes;';//'Provider={SQLServer};Persist Security Info=False;Data Source=' + sODBCName; //g_sADODBString;
{$IFEND}
    FrmDB.UpdateDB();
    FrmDB.InitiateAuction();
    ...
    MemoLog.Lines.Add('NPC Constant Reloaded...');
    ...
    {$IFDEF PLUGOPEN}
    MemoLog.Lines.Add('Engine PlugIn Load...');
    ...
    MemoLog.Lines.Add('Loading Boxes...');
    nCode := FrmDB.LoadBoxs;
    if nCode < 0 then begin
      MemoLog.Lines.Add('Error Loading Boxes .' + 'Code: ' + IntToStr(nCode));
      exit;
    end;
    MemoLog.Lines.Add('Boxes Loaded...');

    MemoLog.Lines.Add('Loading MiniMap...');
    nCode := FrmDB.LoadMinMap;
    if nCode < 0 then begin
      MemoLog.Lines.Add('Error Loading MiniMap.' + 'Code: ' + IntToStr(nCode));
      exit;
    end;
    ...
    if nCode < 0 then begin
      MemoLog.Lines.Add('Error Loading Magic Database.' + 'Code: ' + IntToStr(nCode));
      exit;
    end;
    MemoLog.Lines.Add(format('Magic Database Loaded: (%d) Magics...', [nCode]));

    MemoLog.Lines.Add('Loading Map Information...');
    nCode := FrmDB.LoadMapInfo;
    FrmDB.GetColour('black');
    if nCode < 0 then begin
      MemoLog.Lines.Add('Error Loading Map Info.' + 'Code: ' + IntToStr(nCode));
      exit;
    end;
    MemoLog.Lines.Add(format('Map Information Loaded: (%d) Maps ', [g_MapManager.Count]));

    MemoLog.Lines.Add('Loading MapQuest...');
    nCode := FrmDB.LoadMapQuest;
    if nCode < 0 then begin
      MemoLog.Lines.Add('Failed to load MapQuest.');
      exit;
    end;
    MemoLog.Lines.Add('MapQuest Loaded...');

    MemoLog.Lines.Add('Loading Monster Database...');
    nCode := FrmDB.LoadMonsterDB;
    if nCode < 0 then begin
      MemoLog.Lines.Add('Error Loading Monster DB.' + 'Code: ' + IntToStr(nCode));
      exit;
    end;
    MemoLog.Lines.Add(format('Monster Database Loaded: (%d) Monsters...',
    ...
    MemoLog.Lines.Add(format('Monster Spawn Lists Loaded: (%d) Spawns...', [UserEngine.m_MonGenList.Count]));
  ...
  //  MemoLog.Lines.Add(format('加载无限仓库数据库成功(%d/%d)...', [g_BigStorageList.HumManCount,g_BigStorageList.RecordCount]));
    ...
    if nCode < 0 then begin
      MemoLog.Lines.Add('Failed to load Unbind List.' + 'Code: ' +
        IntToStr(nCode));
      exit;
    end;
    MemoLog.Lines.Add('Unbind List Reloaded...');
    ...
    MemoLog.Lines.Add('Loading LineHint...');
    if not LoadLineHint(g_Config.sNoticeDir + 'LineHint.txt') then begin
      MemoLog.Lines.Add('Failed to load LineHint.');
    end;
    MemoLog.Lines.Add('LineHint Loaded...');


    MemoLog.Lines.Add('Loading Banned Apps...');
    if not LoadBannedAppList(g_Config.sEnvirDir + 'BannedApps.txt') then begin
      MemoLog.Lines.Add('Failed to load Banned Apps.');
    end;
    MemoLog.Lines.Add('Banned Apps Loaded...');

    RobotManage := TRobotManage.Create;
    MemoLog.Lines.Add('Robot Loaded...');
    ...
    MemoLog.Lines.Add('Guild Information Loaded...');
...
{$IFEND}
    ...
    SendGameCenterMsg(SG_STARTOK, 'M2Server Started Successfuly...');
    //RegSocket.Active := True;
  except
    on E: Exception do
...
{$IFEND}
    ...
    if nCode < 0 then begin
      MemoLog.Lines.Add('Load MakeMagic Error .' + 'Code: ' + IntToStr(nCode));
      exit;
    end;
    MemoLog.Lines.Add('MakeMagic Loaded...');

    nCode := FrmDB.LoadMerchant;
    if nCode < 0 then begin
      MemoLog.Lines.Add('Load Merchants Error .' + 'Code: ' + IntToStr(nCode));
      exit;
    end;
    MemoLog.Lines.Add('Merchants Loaded...');

    nCode := FrmDB.LoadNpcs;
    if nCode < 0 then begin
      MemoLog.Lines.Add('Load NPCs Error .' + 'Code: ' + IntToStr(nCode));
      exit;
    end;
    MemoLog.Lines.Add('NPCs Loaded...');
    ...
    MemoLog.Lines.Add('Shop Engine Loaded...');

    InitCoralWry();
    MemoLog.Lines.Add('IP Query Loaded...');

    FrmDB.LoadMissionData();
    MemoLog.Lines.Add('Mission Data Loaded...');

    FrmDB.LoadMapDesc();
    MemoLog.Lines.Add('Map Desc Loaded...');
    FrmDB.LoadUserCmd();
    MemoLog.Lines.Add('User Commands Loaded...');

    LoadFilterSayList();
    LoadFilterGuildList();
    LoadFilterShopList();
    MemoLog.Lines.Add('Filter List Loaded...');
    {$IFDEF PLUGOPEN}
    MemoLog.Lines.Add('Initializing PlugIn Engine...');
    PlugInEngine.InitPlugIn;
    {$ENDIF}
    nCode := UserEMail.Initialize;
    MemoLog.Lines.Add('EMail Data Loaded(' + IntToStr(nCode) + '/' + IntToStr(UserEMail.EMailCount) + '/' +
...
{$ENDIF}
    ...
    MainOutMessage('Failed to start server.');
  end;
end;
   {
...
begin
  FrmDB.LoadMakeMagic();
  MainOutMessage('MakeMagic Reloaded...');
end;
...
begin
  ...
  MainOutMessage('Chat Filter reloaded...');
  LoadFilterGuildList();
  MainOutMessage('Guild List Filter Reloaded...');
  LoadFilterShopList();
  MainOutMessage('Shop List Filter Reloaded...');
end;
...
resourcestring
  sCloseServer = '%s [Closing the server(%d/%d/%d)...]';
  sCloseServer1 = '%s [Server is closed]';
begin
...
begin
  nCode := FrmDB.LoadMakeItem;
  if nCode < 0 then begin
    MainOutMessage('Failed to reload MakeItem .' + 'Code: ' + IntToStr(nCode));
    exit;
  end;
  MainOutMessage('MakeItem Reloaded...');
  if Application.MessageBox('Are you sure you want to reload Merchants?', 'Message', MB_OKCANCEL + MB_ICONQUESTION) = IDOK then
  begin
    FrmDB.ReLoadMerchants();
    UserEngine.ReloadMerchantList();
    MainOutMessage('Merchants Reloaded.');
  end;
end;
...
{$IFEND}
  ...
  g_BannedAppList:= TStringList.Create;
  ...
  CloseFile(F);
  Caption := '';
  MemoLog.Lines.Add('Loading Configuration Info...');
...
{$IFEND}
    ...
    for i := 0 to g_MineTable.Count - 1 do begin
      for ii := 0 to pTMineInfo(g_MineTable[i]).DetailList.Count - 1 do begin
        Dispose(pTMineDetails(pTMineInfo(g_MineTable[i]).DetailList[ii]));
      end;
      FreeAndNil(pTMineInfo(g_MineTable[i]).DetailList);
    end;
    except
      //expecting errors if detaillist isnt created but tbh we're closing server anyway :p
    end;
    FreeAndNil(g_MineTable);
    FreeAndNil(g_MineTable);
...
begin
  ...
  MainOutMessage('DBServer(' + Socket.RemoteAddress + ':' + IntToStr(Socket.RemotePort) + ')Connected...');
...
begin
  ...
  MainOutMessage('DBServer(' + Socket.RemoteAddress + ':' +
...
begin
  ...
  FrmDB.LoadMonSayMsg();
  MainOutMessage('Mon Say reloaded...');
end;
...
begin
  if Application.MessageBox('Are you sure you want to clear the Log file?',
    'Confirm', MB_YESNO + MB_ICONQUESTION) = mrYes then begin
    MemoLog.Clear;
  end;
end;
...
begin
  ...
  Try
    ...
    MainOutMessage('Save Offline list to OffLineList.txt file...');
  Finally
    SaveList.Free;
  End;
...
begin
  FrmDB.LoadDefiniensConst;
  MainOutMessage('DefiniensConst Reloaded...');
end;
...
begin
  FrmDB.ReLoadMerchants();
  UserEngine.ReloadMerchantList();
  MainOutMessage('All Merchants Reloaded...');
  UserEngine.ReloadNpcList();
  MainOutMessage('All NPCs Reloaded...');
end;
...
begin
  try
    ...
    if not InputQuery('IP area query ',' enter the IP address:', sIPaddr) then
      Exit;
    if not IsIPaddr(sIPaddr) then begin
      Application.MessageBox('The IP address format entered is incorrect!!!',' Error message', MB_OK + MB_ICONERROR);
      Exit;
    end;
    {if not IsIPaddr(sIPaddr) then begin
    ...
    MemoLog.Lines.Add(format('%s:%s', [sIPaddr, 'Unknown']));
  end;
end;
...
begin
  if g_FunctionNPC <> nil then begin
    ...
    MainOutMessage('QFunction Reloaded...');
  end;
end;
...
begin
  if g_ManageNPC <> nil then begin
    ...
    MainOutMessage('QManage Reloaded..');
  end;
end;
...
begin
  if g_RobotNPC <> nil then begin
    ...
    MainOutMessage('Robot Manage Reloaded...');
  end;
end;
...
begin
  SendGameCenterMsg(SG_STARTOK, 'Game Starup Successfully...');
end;
{$IFEND}
