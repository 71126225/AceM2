...
resourcestring
  ...
  sWarStopTimeMsg = '[%s War has %d minutes go]';
...
var
  ...
  s10: string;
resourcestring
  sGetCastleMsg = '[%s Is(#6FFFF/8#6%s#5)Occupied by]';
begin
  ...
  MainOutMessage(format('[%s is(%s)occupied by]', [m_sName, m_sOwnGuild]));
end;
...
var
  ...
  s14: string;
resourcestring
  sWallWarStop = '[%s War has finished]';
begin
  m_boUnderWar := False;
...
begin
  ...
  if List14.Count > 0 then begin
    for i := 1 to List14.Count - 1 do begin
      if (not TPlayObject(List14.Items[i]).m_boAdminMode) then begin //dont allow gamemaster mode in sw
        Guild := TGuild(TPlayObject(List14.Items[i]).m_MyGuild);
        if Guild = nil then break;
        //this check makes it so if there's multiple guilds inside palace, it  checks first guild that's fighting this war
        if Guild.m_AttackCastle = Self then break;
      end;
    end;
    if (Guild <> nil) and (Guild <> m_MasterGuild) and (Guild.m_AttackCastle = Self) then begin
      ...
      for i := 1 to List14.Count - 1 do begin
        PlayObject := TPlayObject(List14.Items[i]);
        if PlayObject.m_boAdminMode then continue; //dont allow gamemaster mode to count in sw
        if PlayObject.m_MyGuild <> nil then begin
          if (PlayObject.m_MyGuild = Guild) then continue;          
          if (Guild.IsAllyGuild(TGuild(PlayObject.m_MyGuild))) and (m_MasterGuild.IsAllyGuild(TGuild(PlayObject.m_MyGuild)) = false) then
            continue;//if the person standing inside is an ally of attacker (and not defender) then he can help capture it
        end;
...
begin
  ...
  Try
    ...
    nCode := 2;
    if m_MasterGuild <> nil then
      m_MasterGuild.SendGuildMsg(Guild.m_sGuildName + ' has challenged you to a castle war at ' +
        FormatDateTime('YYYY/MM/DD ', AttackerInfo.AttackDate) + IntToStr(g_Config.nStartCastlewarTime) + 'hour.');
    nCode := 3;
    SaveAttackSabukWall();
    nCode := 4;
    if (m_MasterGuild <> nil) and (m_MasterGuild.IsAllyGuild(Guild)) then begin
      ...
      nCode := 6;
      Guild.SendGuildMsg(m_MasterGuild.m_sGuildName + ' has been removed as an ally.');
      nCode := 7;
      Guild.DelAllyGuild(m_MasterGuild);
      nCode := 8;
      m_MasterGuild.SendGuildMsg(Guild.m_sGuildName + ' has been removed as an ally.');
      nCode := 9;
      Guild.RefMemberName();
      nCode := 10;
      m_MasterGuild.RefMemberName();
