...
begin
  //  Result := False;
  {$region 'check if person is already in guild'}
  if IsMember(PlayObject.m_sCharName) then begin
    Result := False;
    exit;
  end;
  {$endregion}
    ...
    if not IsAllyGuild(Guild) then begin
      ...
      for I := 0 to m_GuildWarList.Count - 1 do begin
        if pTWarGuild(m_GuildWarList.Objects[I]).Guild = Guild then begin
          ...
          SendGuildMsg('War with [' + Guild.m_sGuildName + '] guild has been extended for another' + IntToStr(g_Config.dwGuildWarTime div 1000 div 60) + ' hours');
          break;
        end;
      end;
      if WarGuild = nil then begin
        ...
        SendGuildMsg('War with [' + Guild.m_sGuildName + '] has stared, duraction:' + IntToStr(g_Config.dwGuildWarTime div 1000 div 60) + ' hours');
        RefMemberName();
        IncSocietyIndex;
      end;
      Result := WarGuild;
...
begin
  SendGuildMsg('[' + Guild.m_sGuildName + '] is no longer at war with your guild');
end;
...
begin
  ...
  if m_btGuildLevel < MAXGUILDLEVEL then begin
    ...
    if (m_nMoneyCount >= GuildExp.nGold) and (m_nBuildPoint >= GuildExp.nBuildPoint) and
      ...
      SendGuildMsg('Congratualations: your guild reached level' + IntToStr(m_btGuildLevel) + '.');
      Result := True;
    end;
  end;
  ...
  case nWork of
    0: begin  //deposit
      if (nGold > 9999) and (PlayObject.m_nGold >= nGold) then begin
        ...
        SendGuildMsg('[' + PlayObject.m_sCharName + '] deposited ' + IntToStr(nGold) + 'Gold to guild funds');
      end else
        Result := 2;
    end;
    1: begin
      if PlayObject.m_nGuildRankNo = 1 then begin
        if (nGold > 0) and (m_nMoneyCount >= nGold) then begin
          ...
          if IntegerChange(PlayObject.m_nGold, nGold, INT_ADD) then;
            PlayObject.GoldChanged;
            SendGuildMsg('[' + PlayObject.m_sCharName + '] withdrew ' + IntToStr(nGold) + 'Gold from guild funds');
        end else
          Result := 3;
      end else
        Result := 1;
            ...
            if nPlayGold > 0 then begin
              ...
              for i := 0 to SendList.Count - 1 do begin
                ...
                SendObject.SendMsg(SendObject, RM_GUILDMESSAGE, 0, g_Config.nGuildMsgFColor, g_Config.nGuildMsgBColor,
                    0, 'You have recieved ' + IntToStr(nPlayGold) + 'Gold from guilds funds');
              end;
            end else
              Result := 3;
          end;
...
begin
  ...
  try
    ...
    MainOutMessage('Failed to save guild information. ' + sFileName);
  end;
  SaveList.Free;
end;
...
begin
  ...
  IncInfoIndex;
  RefGuildMemberMaxCount;
  SendGuildMsg('Guild level changed to ' + IntToStr(m_btGuildLevel) + '.');
end;
...
//  Result := -1;
  ...
  while (True) do begin
    ...
    if sRankInfo[1] = '#' then begin //title line
      ...
      if Length(sRankName) > 16 then //max rank name = 16 chars
      ...
      if sRankName = '' then begin
        Result := -3; //RankName needs something entered
        ClearRankList(GuildRankList, False);
        Exit;
      end else
      if not CheckEnglishChr(sRankName) then begin
        Result := -8; //RankName invalid chars
        ClearRankList(GuildRankList, False);
        Exit;
      end else
      if not (nRankNo in [1..99]) then begin
        Result := -11; //RankName max rank is 99
        ClearRankList(GuildRankList, False);
        Exit;
      end else
      if RankArr[nRankNo] then begin
        Result := -7; //RankName cannot have the same rank twice
        ClearRankList(GuildRankList, False);
        Exit;
      end;
    ...
    while (True) do begin //将成员名称加入职称表里
      ...
      if sMemberName <> '' then begin
        if nRankNo = 1 then begin
          Inc(nAdminCount);
          if UserEngine.GetPlayObject(sMemberName) <> nil then
            boLeaderOnline := True;                           
        end
  ...
  if not boLeaderOnline then begin
    Result := -12;
    ClearRankList(GuildRankList, False);
    Exit;
  end;
  ...
  if nAdminCount < 1 then begin
    Result := -5; //RankName at least 1 leader
    ClearRankList(GuildRankList, False);
    Exit;
  end;
  if nAdminCount > 2 then begin
    Result := -4; //RankName max 2 leader
    ClearRankList(GuildRankList, False);
    Exit;
  end;
...
var
  ...
  sGuildName: string[GuildNameLen];
  I: Integer;
  Guild: TGuild;
begin
  if FileExists(g_Config.sGuildFile) then begin
    ...
    for I := m_GuildList.Count - 1 downto 0 do begin
      ...
      if not Guild.LoadGuild() then begin
        MainOutMessage(Guild.m_sGuildName + ' Error reading.');
        Guild.Free;
        m_GuildList.Delete(I);
      end else
      {if Guild.m_nActivityPoint <= 0 then begin
        Guild.DeleteGuild;
        MainOutMessage(Guild.m_sGuildName + ' deleted due to inactivity.');
        Guild.Free;
        m_GuildList.Delete(I);
      end else  }
    ...
    MainOutMessage('Total Guilds: ' + IntToStr(m_GuildList.Count));
  end
  else begin
