...
begin
  ...
  begin
    ...
    if not InputQuery(Format('%s Of %s', [sItemName, sCaption]), format('Enter %s:', [sCaption]), sValue) then
      exit;
  ...
  begin
    EditCompoundInfoForm.Caption := Format('%s Of %s', [sItemName, sCaption]);
