...
//    m_nHungerStatus: Integer; //0x0F8
    ...
    m_boMaxHP: Boolean;
    m_boMaxMP: Boolean;
    m_wStatusArrValue: array[0..15] of Word; //0x218
    m_dwStatusArrTimeOutTick: array[0..15] of LongWord;
    // :Tarry220;           //0x220
    m_wAppr: Word; //0x238
    ...
    boCantGetStun: Boolean; //0x2BF
    boCantTDBStun: Boolean;
    m_btSlowResist: Byte;
    m_btCoreDefense: Byte;
    m_btPoisonDefense: Byte;
    m_btTameValue : Byte;
    m_btNoTurnUndead : Byte;
...
//    m_bo316: Boolean; //0x316
    ...
    m_boSpawnMission: Boolean; //0x338
    m_nSpawnMissionX: Integer; //0x33C
    m_nSpawnMissionY: Integer; //0x340
    m_btMissionDir: Byte;
    m_boHideMode: Boolean; //0x344  隐身戒指(Byte)
    m_boHideItemMode: Boolean;
    ...
    warning_for_pet_untame_tick: LongWord;
    chance_to_retame_tick: LongWord;
    chance_to_retame: Boolean;
    chances_to_retame_count: Integer;
    previous_slave_owner: TBaseObject;
...
//    m_dwItemWuXinExpRate: LongWord;
    ...
    m_nStaggerTime: Integer;
    m_nSlowOn                 :Boolean; // Stop Slow stacking
    m_nnonfrzWalkSpeed        :Integer;
    m_nnonfrzNextHitTime      :Integer;
    m_boCounterAttackPending:       Boolean;
    m_nCounterAttackTimer:          Integer;
    m_boBeginCasting:               Boolean;
    m_boHallucinationMode:          Boolean;
    m_dwHallucinationModeTick:      LongWord;
    m_dwHallucincationModeInterval: LongWord;

    m_PoisonTarget: TBaseObject;
    m_PoisonTick: LongWord;
    m_boEnergyShieldActive: Boolean;
    m_btEnergyShieldLevel: Byte;
    m_nCrystalArmour: byte;
    m_boNoDuraLoss: Boolean;
    m_nPoisonAttack: byte;
    m_nFreezingAttack: byte;
    m_nMoveDropX: Integer;
    m_nMoveDropY: Integer;

    FollowObject: TBaseObject;
    m_nFollowDistance : Integer;
    m_dwFollowActivity: LongWord;
    m_dwRespawnTime: LongWord;
  protected
  ...
  public
    ...
    function IsTrapped(TurnOnOff: Boolean): Boolean;
   ...
   // function IsVisibleHuman(): Boolean;
    ...
    function GetFeatureEx: string; virtual;
    ...
    function GetHitStruckDamage(Target: TBaseObject; nDamage: Integer; boTDB: boolean = False): Integer;
    function GetStunDamage(Target: TBaseObject; nDamage: Integer): Integer;
...
//    procedure sub_4BC87C();
    ...
    function CharPushed(nDir, nPushCount: Integer): Integer; virtual;
    function CharPushedcbo(nDir, nPushCount: Integer): Integer; virtual;
    ...
    procedure DamageBubbleDefence(type_of_effect: Byte);
    ...
    procedure StruckDamage(var nDamage: Integer; AttackBase: TBaseObject); virtual;
    ...
    function  ASpeedUp(nPower, nTime: Integer): Boolean;
    function  AgilityUp(nPower, nTime: Integer): Boolean;
    ...
    function MagDebuffArea(starting_x, starting_y, range_of_damage, duration_of_effect, power_of_effect: Integer; skill_level, type_of_attack: Integer): Integer;
    ...
    function MagChangeAbility(amount_to_increase, duration_of_added_effect, type_of_effect: Integer; user_magic: pTUserMagic; target_object: TBaseObject = nil): Boolean;
    ...
    function IsProperTargetSKILL_ENTRAPMENT(nLevel: Integer; BaseObject: TBaseObject): Boolean; virtual;
    function IsProperTargetSKILL_SLASHINGBURST(BaseObject: TBaseObject; nTargetX, nTargetY: Integer): Boolean; virtual;
    ...
    procedure KillClone(sCloneName:String);
    procedure OpenHallucinationMode(nTime: Integer);
    procedure BreakHallucinationMode();
    function CanPerformActions(): Boolean;

    procedure GetMapFollowObject(range: Integer);
    function CanFollowObject(FollowObj: TBaseObject): Boolean;
  end;
//{$ENDREGION}
...
uses M2Share, Guild, HUtil32, EDcodeEx, ObjNpc, IdSrvClient, ItmUnit, Event,
  ObjMon, ObjMon3, ObjMon4, LocalDB, Castle, svMain, {$IFDEF PLUGOPEN}PlugOfEngine,{$ENDIF} ObjPlay;
...
begin
 ...
 { //m_MagicPowerHitSkill := nil;
  ...
  chances_to_retame_count := 0;
  chance_to_retame_tick := GetTickCount();
  chance_to_retame := false;
  warning_for_pet_untame_tick := GetTickCount();
  previous_slave_owner := nil;
  ...
  m_nStaggerTime := GetTickCount;

  m_boCounterAttackPending  :=   false;
  m_nCounterAttackTimer     :=   0;
  m_boBeginCasting          :=   False;
  m_boHallucinationMode     :=   False;
  m_dwHallucinationModeTick :=   0;
  m_boEnergyShieldActive    :=   false;

  m_PoisonTarget := nil;
  m_PoisonTick := GetTickCount;
  m_nCrystalArmour := 0;
  m_boNoDuraLoss := false;
  m_nPoisonAttack := 0;
  m_nFreezingAttack :=0;
  m_nMoveDropX := 0;
  m_nMoveDropY := 0;
  ...
  FollowObject := nil;
  m_nFollowDistance := 0;
  m_dwFollowActivity := GetTickCount;
  m_dwRespawnTime := 1;

  m_boSpawnMission := False;
 end;
...
procedure TBaseObject.UseLamp; //Samuel light
var
  nOldDura: Integer;
  nDura: Integer;
...
begin
  ...
  if not m_boFixedHideMode then
    SendRefMsg(RM_TURN, MakeWord(m_btDirection, m_nLight), m_nCurrX, m_nCurrY, 0, '');
end;
...
begin
  ...
  if nDura <= 0 then begin
    ...
    SysMsg(UserEngine.GetStdItemName(m_UseItems[U_WEAPON].wIndex) + '''s durability has dropped to 0.',c_Red,t_Notice);
    RecalcAbilitys();
    SendAbility;
    SendSubAbility;
    FeatureChanged();
...
begin
  nLevel := 0;
  if (m_WAbil.Level > 0) then
    nLevel := m_WAbil.Level;
  StdItem := UserEngine.GetStdItem(Item.wIndex,nLevel,m_btJob);
  ...
  Inc(AddAbility.wAddAttack, StdItemA.AddAttack);
  Inc(AddAbility.wDelDamage, StdItemA.DelDamage);
  Inc(AddAbility.btDeadliness, StdItemA.Deadliness);
  //if Item.Value.btWuXin > 0 then begin
  if not g_Config.boCloseWuXin then begin
    Inc(AddAbility.wAddWuXinAttack, StdItemA.AddWuXinAttack);
    Inc(AddAbility.wDelWuXinAttack, StdItemA.DelWuXinAttack);
  end;
  //end;
  inc(AddAbility.wFreeze, StdItemA.Freezing);
  inc(AddAbility.wPoisonAttack, StdItemA.PoisonAttack);
  ...
  Inc(AddAbility.wHitPoint, StdItemA.HitPoint); //acc
  Inc(AddAbility.wSpeedPoint, StdItemA.SpeedPoint); //agil
  ...
  if StdItemA.StdMode <> tm_dress then
    inc(AddAbility.btHoly, StdItemA.Holy);
  if StdItemA.StdMode = tm_Weapon then begin
    Inc(AddAbility.btWeaponStrong, StdItemA.Strong);
  end else
  if (StdItemA.StdMode = tm_boot) then begin
    inc(AddAbility.nWearWeight, StdItemA.Strong);
  end else if (StdItemA.StdMode <> tm_Dress) then
    inc(AddAbility.nBagWeight, StdItemA.Strong);
  if (StdItemA.StdMode = tm_Boot) then
      inc(AddAbility.nHandWeight, StdItemA.Luck)
  else
    Inc(AddAbility.btLuck, StdItemA.Luck);

  if StdItemA.HitSpeed >= 10 then begin //aspeed
...
begin
  ...
  if BaseObject.m_btRaceServer = RC_PLAYOBJECT then begin
    ...
    {$region 'colors while inside a castle war zone'}
    if (Castle <> nil) and Castle.m_boUnderWar and m_boInFreePKArea and BaseObject.m_boInFreePKArea then begin
      ...
      if Castle.IsAttackGuild(TGuild(m_MyGuild)) then begin
        ...
        //guild thats not attacking or defending, check alliances
        ...
        //if the guild we are seeing is not an ally, check if we are allied with defenders
          if (Castle.m_MasterGuild <> nil) then begin
            if TGuild(m_MyGuild).IsAllyGuild(Castle.m_MasterGuild) then begin
            //if we are allied with the guild owning sw, then attackers are orange for us
  ...
  if BaseObject.m_btRaceServer = RC_NPC then begin
    ...
    if BaseObject.m_boHolySeize then
      Result := $7D;
    if BaseObject.m_boHallucinationMode then
      Result := $FD;
  end;
end;
...
begin
  Result := False;
  if (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_COBWEB] = 0) and (m_wStatusTimeArr[POISON_FREEZE] = 0) and (not m_boBeginCasting) then
    Result := True;
  if (m_btRaceServer <> RC_PlayObject) and (m_wStatusTimeArr[POISON_STUN] <> 0) and (not boCantTDBStun) then
    Result := False;
end;

function TBaseObject.IsTrapped(TurnOnOff: Boolean): Boolean;
begin
  Result := False;
  if (TurnOnOff) then begin
    m_boCanWalk := False;
    m_boCanRun := False;
    m_boShoulderDash := False;
    m_boTrapped := True;
    Result := True;
  end else begin
    m_boCanWalk := True;
...
function TBaseObject.CanPerformActions(): Boolean;
begin
  Result := False;
...
begin
  try
    ...
    if not m_boGhost then begin
      ...
      SendMessage.dwDeliveryTime := GetTickCount() -1; //should make it possible to keep track i hope
      SendMessage.BaseObject := BaseObject;
      SendMessage.boLateDelivery := False;
      if sMsg <> '' then begin
        try
              ...
              if OSObject.btType = OS_MOVINGOBJECT then begin
                ...
                if BaseObject <> nil then begin
                  ...
                  if (not BaseObject.m_boGhost) and (not BaseObject.m_boFixedHideMode) and
                    ...
                    nCheckCode := 17;
                    if (m_btRaceServer < RC_ANIMAL) or (m_Master <> nil) or m_boCrazyMode or m_boHallucinationMode or m_boWantRefMsg or
...
function TBaseObject.GetFeatureEx(): String;
...
begin
  ...
  if m_UseItems[U_WEAPON].wIndex > 0 then begin
    ...
    if StdItem <> nil then begin
      if StdItem.Shape > 128 then highweapon := 1;
    end;
  end;
  Features.DressEffect := m_btDressEffType;
  Features.HighWeapon := highweapon;
  Features.HorseType := 0;
  if m_boOnHorse then
    Features.HorseType := m_btHorseType;
  Features.Job := m_btJob;
  Features.StrengthenName := m_btStrengthenName;
  Result:=EncodeBuffer(@Features, SizeOf(TFeatures));
end;
...
begin
  SendRefMsg(RM_FEATURECHANGED, 0, GetFeatureToLong, 0, 0, GetFeatureEx())
end;
...
begin
  if m_boOnHorse then begin //if we are on a horse, we might wanna get off
    ...
    if boOnHorse and m_PEnvir.m_boNotHorse then begin
      SysMsg('You cant Ride The Horse in this Map...', c_Red, t_Hint);
      Exit;
    end;
    ...
    if boOnHorse and (not m_boCanOnHorse) and (g_Config.nTakeOnHorseUseTime > 1) then begin
      if (m_UseItems[u_House].btAliveTime > 0)  then begin
        SysMsg('The horse is recovering, temporarily unable to ride!', c_Red, t_Hint);
      ...
      if m_btRaceServer = RC_PLAYOBJECT then
        TPlayObject(Self).SendDefMessage(SM_SHOWBAR, Integer(Self), 2, CM_CLICKBOX, g_Config.nTakeOnHorseUseTime, 'Mounting...');
...
begin
  ...
  for i := 0 to m_SlaveList.count -1 do begin
    ...
    if BaseObject is TWizClone then
      exit;
  end;
    GetFrontPosition(nX,nY);
    MonObj := UserEngine.RegenMonsterByName(m_PEnvir{.sMapName}, nX, nY, sMonName);
    if (MonObj <> nil) and (MonObj is TWizClone) then begin
      ...
      if MonObj is TWizClone then
        TWizClone(MonObj).SetMpLoss(nMpLoss);

      MonObj.m_btGender       := m_btGender;
      MonObj.m_btHair         := m_btHair;
      MonObj.m_btRaceImg      := m_btRaceImg;
      MonObj.m_UseItems       := m_UseItems;
      MonObj.m_btJob          := m_btJob;
      MonObj.m_nPkPoint       := m_nPkPoint;
      MonObj.m_sCharName      := m_sCharName;
      MonObj.m_btNameColor    := 246;
      MonObj.m_MyGuild        := m_MyGuild;
      MonObj.m_sGuildRankName := m_sGuildRankName;
      MonObj.m_btDressEffType := m_btDressEffType;

      MonObj.FeatureChanged;
      m_SlaveList.Add (MonObj);
      Result := MonObj;
    end;
end;

procedure TBaseObject.KillClone(sCloneName:String);
var
  i:Integer;
begin
  for i:=m_SlaveList.Count -1 downto 0 do begin
    if (TBaseObject(m_SlaveList.Items[i]) is TCloneObject) then begin
      TcloneObject(m_SlaveList.Items[i]).m_nDieReason := 3;
      //if TBaseObject(m_SlaveList.Items[i]) is TWizClone then begin
      //  TWizClone(m_SlaveList.Items[i]).m_boNextTurnDie:=True;
      //end else
      TCloneObject(m_SlaveList.Items[i]).Die();
      Break;
    end;
  end;
end;
...
begin
  ...
  if (m_btRaceServer <> RC_PLAYOBJECT) and (not m_PEnvir.GetFireBurn(m_nCurrX, m_nCurrY)) and (not m_boNotInSafe) then begin
    SendRefMsg(nIdent, MakeWord(m_btDirection, m_nLight), m_nCurrX, m_nCurrY, 0, '');
    exit;
  end;
  ...
  try
    ...
    if Event <> nil then begin
      if Event.m_OwnBaseObject <> nil then begin
        ...
        end else if Event.m_nEventType = ET_POISONCLOUD then begin
          if GetTickCount > m_dwPoisonCloudTick then begin
            if Event.m_OwnBaseObject.IsProperTarget(Self) then begin
              if m_btRaceServer = RC_PLAYOBJECT then m_dwPoisonCloudTick := GetTickCount + g_Config.nFirePlayDamageTimeRate
              else  m_dwPoisonCloudTick := GetTickCount + g_Config.nFireMonDamageTimeRate;
              SendMsg(Event.m_OwnBaseObject, RM_MAGSTRUCK_MINE, 0, Event.m_nDamage, 0, 0, '');
            end;
          end;
        end else
...
begin
  m_btDirection := nDir;
  SendRefMsg(RM_TURN, MakeWord(nDir,m_nLight), m_nCurrX, m_nCurrY, 0, '');
end;
...
{$ENDIF}
  ...
  try
    ...
    begin
      ...
      with AttackBaseObject do begin
        if (m_btRaceServer <> RC_PLAYOBJECT) then begin
      ...
      if (AttackBaseObject <> nil) and (AttackBaseObject.m_btRaceServer = RC_PLAYOBJECT) then begin
        TPlayObject(AttackBaseObject).m_sLastKillMonName := m_sCharName;
        tExp := AttackBaseObject.CalcGetExp(m_Abil.Level, m_dwFightExp);

        if (TPlayObject(AttackBaseObject).m_GroupOwner <> nil) and //Change groups last grp kill mob.
          (TPlayObject(AttackBaseObject).m_GroupOwner.m_GroupMembers.Count > 0) then begin
          for ii := 0 to TPlayObject(AttackBaseObject).m_GroupOwner.m_GroupMembers.Count - 1 do begin
            GroupObject := TPlayObject(TPlayObject(AttackBaseObject).m_GroupOwner.m_GroupMembers.Objects[ii]);
            TPlayObject(GroupObject).m_sLastGrpKillMonName := m_sCharName;
          end;
        end;
        ...
        if AttackBaseObject.m_MyGuild <> nil then begin
  ...
  end;  //end of exp + killquest code
  
  //pk penalty's and rewards code
  ...
  try
      ...
      if not guildwarkill then begin
        if (g_Config.boKillHumanWinLevel or g_Config.boKillHumanWinExp or m_PEnvir.m_boPKWINLEVEL or m_PEnvir.m_boPKWINEXP) and
          ...
          if not AttackBaseObject.IsGoodKilling(Self) then begin
            ...
            //增加PK值
            AttackBaseObject.SysMsg(g_sYouMurderedMsg , c_Red, t_Hint);
            ...
            if PKLevel < 1 then
              if Random(5) = 0 then
                AttackBaseObject.MakeWeaponUnlock;
          end
          else
            AttackBaseObject.SysMsg(g_sYouProtectedByLawOfDefense , c_Green, t_Hint);
        end;
    ...
    MainOutMessage(sExceptionMsg2);
  end; //end of pk penalty's and reawrds code

  //spirit items break on death
  if m_LastHiter <> nil then begin
    if (not m_bopirit) and (m_LastHiter.m_btRaceServer >= RC_MONSTER) and (not m_PEnvir.m_boFightZone) then
      TPlayObject(self).ProcessSpiritSuite();
  end;
  ...
  try
    ...
    if m_btRaceServer <> RC_PLAYOBJECT then begin
      //dont let mobs drop in contest zones (allows for servers to have events there)
      if (not m_PEnvir.m_boFightZone) and (not m_PEnvir.m_boFight3Zone) and (not m_boAnimal) then begin
        ...
        if (AttackBaseObject <> nil) and (m_Master = nil) then begin
          ...
          //gold goes straight to bag, not on floor
          nCheckCode := 1018;
          if (TPlayObject(AttackBaseObject).GainGold(m_nGold)) then begin
            nCheckCode := 1003;
          {
    ...
    if not m_PEnvir.m_boFightZone then TPlayObject(self).BreakStones();

    SendRefMsg(RM_DEATH, MakeWord(m_btDirection, m_nLight), m_nCurrX, m_nCurrY, 1, '');
  except
...
begin
  m_dwPKTick := 0;
  m_boDeath := False;
  SendRefMsg(RM_ALIVE, MakeWord(m_btDirection, m_nLight), m_nCurrX, m_nCurrY, 0, '');
end;
...
begin
  if (BaseObject <> nil) and (BaseObject.m_btRaceServer = RC_PLAYOBJECT) and (TPlayObject(BaseObject).m_AttackState = as_Dare) then
    Exit;
  //baseobject = mob/person attacking us
  //self = 'mob/person' being hit
  if not BaseObject.IsProperTarget(self) then exit;
  ...
  if m_btRaceServer >= RC_ANIMAL {50} then begin
    if (m_Master <> nil) and (m_Master.m_btRaceServer = RC_PLAYOBJECT) then begin
      ...
      if (m_btRaceServer > RC_PEACENPC {15}) and (m_btRaceServer < RC_ANIMAL{50}) then
        Result := True;
      if (BaseObject.m_Master <> nil) and (BaseObject.m_btRaceServer <> RC_CAMION) and (BaseObject.m_Master.m_btRaceServer = RC_PLAYOBJECT) then
        Result := True;
    end;
    if m_boCrazyMode then
      Result := True;
    if m_boHallucinationMode then
      Result := True;
  end
  {$endregion}
  {$region 'player code'}
          ...
          if BaseObject.m_btRaceServer = RC_PLAYOBJECT then begin
            ...
            if TPlayObject(Self).m_boMaster then begin
              for I := 0 to TPlayObject(Self).m_MasterList.Count - 1 do begin
                if TPlayObject(Self).m_MasterList.Objects[I] = BaseObject then begin
                  Result:=False;
                  break;
                end;
              end;
            ...
            if TPlayObject(BaseObject).m_boMaster then begin
              for I := 0 to TPlayObject(BaseObject).m_MasterList.Count - 1 do begin
                if TPlayObject(BaseObject).m_MasterList.Objects[I] = Self then begin
                  Result:=False;
                  break;
                end;
              end;
...
begin
  ...
  //if we are trying to hit a player that is offline/in duel mode/...
  ...
  //check if the target are trying to attack is something that can be attacked
  Result := IsAttackTarget(BaseObject);
  //players are sometimes prevented from fighting eachother?
  if Result then begin
  ...
  if Result then begin
    if (BaseObject <> nil) and
      ...
      //if we are a player + the target has a master (and the master is a player)
      if BaseObject.m_Master = Self then begin
        if m_btAttatckMode <> HAM_ALL {0} then //only kill your own pet on attack all
          Result := False;
      end else begin
        Result := IsAttackTarget(BaseObject.m_Master);//if the target has a master: we should 'try' to hit the master
        if InSafeZone or BaseObject.InSafeZone then //safezone overrules all
          Result := False
      end;
    end else if (BaseObject <> nil) and (BaseObject.m_btRaceServer = RC_CAMION) and (BaseObject.m_Master = nil) then begin
      if InSafeZone or BaseObject.InSafeZone then
        Result := False;
    end;
  end;
...
function TBaseObject.IsProperTargetSKILL_ENTRAPMENT(nLevel: Integer; BaseObject:
  TBaseObject): Boolean;
begin
  Result := False;
  if (BaseObject = nil) or (BaseObject = Self) then
...
function TBaseObject.IsProperTargetSKILL_SLASHINGBURST(BaseObject: TBaseObject; nTargetX,
...
begin
  ...
  if BaseObject.m_boDeath and (BaseObject.m_btRaceServer = RC_PLAYOBJECT) then begin
    if TPlayObject(BaseObject).m_boAllowReAlive then
      ...
      SysMsg('Target does not allow Resurrection.', c_Green, t_Hint);
  end;
end;
...
function TBaseObject.IsHidden(): boolean;
begin
//  result:= false;
  result := m_boHideMode;
  //if you're wearing clear item: always hide
  if (m_boHideItemMode) then result := true;
end;
...
procedure TBaseObject.OpenHallucinationMode(nTime:Integer); //Hallucination
begin
  m_boHallucinationMode:=True;
  m_dwHallucinationModeTick:=GetTickCount();
  m_dwHallucincationModeInterval:=nTime * 1000;
  RefNameColor();
end;

procedure TBaseObject.BreakHallucinationMode; //Hallucination
begin
  if m_boHallucinationMode then begin
    m_boHallucinationMode:=False;
    m_Master := nil;
    RefNameColor();
  end;
...
function TBaseObject.GetHitStruckDamage(Target: TBaseObject; nDamage: Integer; boTDB: boolean = False):
  Integer;
var
  n14: Integer;
  nMagLevel, old_user_status: Integer;
  nRate: Integer;
begin
  m_boUseGodShield := False;
  ...
  if (m_btLifeAttrib = LA_UNDEAD) and (Target <> nil) then begin
    Inc(nDamage, Target.m_AddAbil.btHoly);
  end;
  if (nDamage > 0) and (m_boAbilMagBubbleDefence) and (not boTDB) then begin
    nDamage := ROUND((nDamage / 1.0E2) * (m_btMagBubbleDefenceLevel + 2) * 8.0);
    DamageBubbleDefence(STATE_BUBBLEDEFENCEUP);
  end;

  if (nDamage > 0) and (m_boEnergyShieldActive) and (m_wAbil.HP < m_wAbil.MaxHP) then begin
    Inc(m_nIncHealth, nDamage);
    DamageBubbleDefence(STATE_ENERGYSHIELD);
    nDamage := 1;
  end;
  ...
  if (Result > 0) and Assigned(Target) and Assigned(Target.m_Master) then
    Self.SetLastHiter(Target);
end;

function TBaseObject.GetStunDamage(Target: TBaseObject; nDamage: Integer): Integer;
begin
  if m_wStatusTimeArr[POISON_STUN] > 0 then begin
    if nDamage > 0 then
      nDamage := Round(nDamage * g_config.nPoisonStunDamage);
  end;
  Result:= nDamage;
end;
...
var
  ...
  UserMagic: pTUserMagic;
//  nDander: Word;
  nPower, nChance: Integer;
  BaseObjectList: TList;
begin
  if m_boBeginCasting then begin
    PlayObject := TPlayObject(Self);
    PlayObject.SendDefMessage(SM_CLOSEBAR, 0, 0, 0, 0, '');
    PlayObject.m_boTryResurrect := False;
    PlayObject.m_CastingDelayTick := 0;
    PlayObject.m_ResurrectionTarget := nil;
    m_boBeginCasting := False;
  end;

  nDamage := _Max(0,nDamage - m_btCoreDefense);   

  if m_boCounterAttackPending then begin
    if GetTickCount >= m_nCounterAttackTimer then begin
      m_boCounterAttackPending := False;
      self.SysMsg('Counter attack has been disabled.',c_Green,t_Hint);
    end else begin
      PlayObject := TPlayObject(self);
      UserMagic := PlayObject.GetMagicInfo(SKILL_COUNTERATTACK);

      nPower := GetAttackPower(LoWord(m_WAbil.DC), SmallInt((HiWord(m_WAbil.DC) - LoWord(m_WAbil.DC))));
      nPower := Round(nPower - (nPower / 100 * UserMagic.MagicInfo.wPower));
      nChance := 50 + (10 * UserMagic.btLevel);

      if Random(100 - nChance) = 0 then begin
        BaseObjectList := TList.Create;
        PlayObject.GetMapBaseObjects(PlayObject.m_PEnvir,PlayObject.m_nCurrX,PlayObject.m_nCurrY,1,BaseObjectList);
        for I := 0 to BaseObjectList.Count - 1 do begin
          BaseObject := TBaseObject(BaseObjectList.Items[i]);
          if BaseObject = AttackBase then begin
            ...
            SendRefMsg(RM_10205, 0, m_nCurrX, m_nCurrY, 35, '');
            nDamage := Round(nDamage - (nDamage / 100 * UserMagic.MagicInfo.wPower));
          end;
        end;
      end;
    end;
  ...
  if  (m_btRaceServer = RC_PLAYOBJECT) and (AttackBase <> nil) then begin
    if (m_nCrystalArmour > 0) and not m_boOnHorse then begin
      if (Random(100) <= m_nCrystalArmour) then begin //crystal armour succes rate
        //succesfull in doing counterattack
        SendRefMsg(RM_10205,0,m_nCurrX,m_nCurrY,25,'');
        AttackBase.SendRefMsg(RM_STRUCK,nDamage,AttackBase.m_WAbil.HP,AttackBase.m_WAbil.MaxHP,Integer(Self),'');
        AttackBase.StruckDamage(nDamage,self);
        exit;
      end;
      if (Random(100) <= m_nCrystalArmour) then begin //crystal armour succes rate
        //succesfull in healing
        IncHealthSpell(nDamage,0);
        exit;
      end;
    end;
  end;
  ...
  bo19 := False;
  bo20 := False;
  //players lose durability
  if (m_btRaceServer = RC_PLAYOBJECT) and (not m_boNoDuraLoss) then begin
    if m_boOnHorse and (m_UseItems[U_House].wIndex > 0) then begin
      //on horse: make horse items lose dura
      for I := Low(m_UseItems[U_House].HorseItems) to High(m_UseItems[U_House].HorseItems) do begin
        if (m_UseItems[U_House].HorseItems[I].wIndex > 0) and (Random(10) = 0) then begin
          ...
          if ndura = 0 then continue; //stops dura from spamming
          Dec(nDura, nDam);
          if nDura <= 0 then begin
            ...
            //TPlayObject(Self).SendUpdateItem(@m_UseItems[U_House]);
            SysMsg(UserEngine.GetStdItemName(m_UseItems[U_House].HorseItems[I].wIndex) + '''s durability has dropped to 0.',c_Red,t_Notice);
            m_UseItems[U_House].HorseItems[I].Dura:=0;
            bo19 := True;
...
begin
  Result := m_sCharName + ' ' +
    ...
    'Experience:' + IntToStr(m_Abil.Exp) + ' ' +
    'Health:' + IntToStr(m_WAbil.HP) + '-' + IntToStr(m_WAbil.MaxHP) + ' ' +
    'Mana:' + IntToStr(m_WAbil.MP) + '-' + IntToStr(m_WAbil.MaxMP) + ' ' +
...
begin
  ...
  if nType < MAX_STATUS_ATTRIBUTE then begin
    ...
    if nType = Poison_DecHealth then //why care about green poison if it's not green we doing?
      m_btGreenPoisoningPoint := _Max(0,nPoint - m_btPoisonDefense);
    ...
    if m_wStatusTimeArr[nType] > 0 then begin
      if m_wStatusTimeArr[nType] < nTime then begin
        ...
        if nType = POISON_SLOWDOWN then ChangeStatusMode(STATUS_SLOWDOWN, True);
        if nType = POISON_FREEZE then ChangeStatusMode(STATUS_FREEZE, True);
        if nType = POISON_STUN then ChangeStatusMode(STATUS_STUN, True);
  ...
  for I := 0 to m_ItemList.Count - 1 do begin
    ...
    if StdItem <> nil then begin
      if (sm_Superposition in StdItem.StdModeEx) and (StdItem.DuraMax > 1) then
        Inc(Result,StdItem.Weight * UserItem.Dura)
      else
...
function TBaseObject.ASpeedUp(nPower, nTime: Integer): Boolean;
var
  nMin,nSec:Integer;
begin
  Result:=False;

  m_wStatusArrValue[14]:= nPower;
  m_dwStatusArrTimeOutTick[14]:=GetTickCount + nTime * 1000;
  ChangeStatusMode(STATUS_HASTE, True);
  SysMsg(format('Attack Speed increased for %d seconds.', [nTime]), c_Green, t_Hint);
...
function TBaseObject.AgilityUp(nPower, nTime: Integer): Boolean;
var
  nMin,nSec:Integer;
begin
  Result:=False;
  ...
  ChangeStatusMode(STATUS_LIGHTBODY, True);
  SysMsg(format('Agility has increased for %d seconds.', [nTime]), c_Green, t_Hint);
...
function TBaseObject.MagChangeAbility(amount_to_increase, duration_of_added_effect, type_of_effect: Integer; user_magic: pTUserMagic; target_object: TBaseObject = nil): Boolean;
var
  minutes,seconds:Integer;
  enhance_object: TBaseObject;
begin
  Result:=False;

  enhance_object := self;

  if not (target_object = nil) then
    enhance_object := target_object;

  minutes := duration_of_added_effect div 60;
  seconds := duration_of_added_effect mod 60;

  case type_of_effect of
     //Curse
    9: begin //DC, SC, MC Decrease (curse)
      enhance_object.m_wStatusArrValue[type_of_effect] := amount_to_increase;
      enhance_object.m_dwStatusArrTimeOutTick[type_of_effect]:=GetTickCount + duration_of_added_effect * 1000;
      enhance_object.ChangeStatusMode(STATUS_CURSE, True);
      enhance_object.SysMsg('Your abilities have been temporarily decreased for ' + inttostr(minutes) + ' minutes and ' + inttostr(seconds) + ' seconds.',c_Green,t_Hint);
    end;
    10: begin //DC Increase(Rage) //rage
      enhance_object.m_wStatusArrValue[type_of_effect] := amount_to_increase;
      enhance_object.m_dwStatusArrTimeOutTick[type_of_effect]:=GetTickCount + duration_of_added_effect * 1000;
      enhance_object.ChangeStatusMode(STATUS_RAGE, True);
      enhance_object.SysMsg('Attack power increased for ' + inttostr(minutes) + ' minutes and ' + inttostr(seconds) + ' seconds.',c_Green,t_Hint);
    end;
    11: begin //MC Increase(MagicBooster) //MagicBooster
      enhance_object.m_wStatusArrValue[type_of_effect] := amount_to_increase;
      enhance_object.m_dwStatusArrTimeOutTick[type_of_effect]:=GetTickCount + duration_of_added_effect * 1000;
      enhance_object.ChangeStatusMode(STATUS_MAGICBOOSTER, True);
      enhance_object.SysMsg('Magic power increased for ' + inttostr(minutes) + ' minutes and ' + inttostr(seconds) + ' seconds.',c_Green,t_Hint);
    end;
    12: begin //DC/MC/SC Increase(UltimateEnhancer) //UltimateEnhancer
      enhance_object.m_wStatusArrValue[type_of_effect] := amount_to_increase;
      enhance_object.m_dwStatusArrTimeOutTick[type_of_effect]:=GetTickCount + duration_of_added_effect * 1000;
      enhance_object.ChangeStatusMode(STATUS_UE, True);
      case enhance_object.m_btJob of
        0 :  enhance_object.SysMsg('Attack power increased for ' + inttostr(minutes) + ' minutes and ' + inttostr(seconds) + ' seconds.',c_Green,t_Hint);
        1 : enhance_object.SysMsg('Magic power increased for ' + inttostr(minutes) + ' minutes and ' + inttostr(seconds) + ' seconds.',c_Green,t_Hint);
        2 : enhance_object.SysMsg('Soul power increased for ' + inttostr(minutes) + ' minutes and ' + inttostr(seconds) + ' seconds.',c_Green,t_Hint);
      end;
    end;
    13: begin //AC Increase(ProField) //ProField
      enhance_object.m_wStatusArrValue[type_of_effect] := amount_to_increase;
      enhance_object.m_dwStatusArrTimeOutTick[type_of_effect]:=GetTickCount + duration_of_added_effect * 1000;
      enhance_object.ChangeStatusMode(STATUS_PROFIELD, True);
      enhance_object.SysMsg('Physical armour increased for ' + inttostr(minutes) + ' minutes and ' + inttostr(seconds) + ' seconds.',c_Green,t_Hint);
    end;
  end;
  enhance_object.RecalcAbilitys();
  enhance_object.SendAbility;
  enhance_object.SendSubAbility;
  enhance_object.FeatureChanged;
  Result:=True;
end;

function TBaseObject.MagDebuffArea(starting_x, starting_y, range_of_damage, duration_of_effect, power_of_effect: Integer; skill_level, type_of_attack: Integer): Integer;
var
  i, j, k: Integer;
  left_x, top_y, right_x, bottom_y, nLevelBasedChance, nActualSlowChance, nCurseChance: Integer;
  map_cell_info: pTMapCellinfo;
  map_cell_object: pTOSObject;
  target_object: TBaseObject;
begin
  Result := 0;
  left_x := starting_x - range_of_damage;
  right_x := starting_x + range_of_damage;
  top_y := starting_y - range_of_damage;
  bottom_y := starting_y + range_of_damage;
  nCurseChance := 180;
  for i := left_x to right_x do begin
    for j := top_y to bottom_y do begin
      if m_PEnvir.GetMapCellInfo(i, j, map_cell_info) and (map_cell_info.ObjList <> nil) then begin
        for k := 0 to map_cell_info.ObjList.Count - 1 do begin
          map_cell_object := map_cell_info.ObjList.Items[k];
          if (map_cell_object <> nil) and (map_cell_object.btType = OS_MOVINGOBJECT) then begin
            target_object := TBaseObject(map_cell_object.CellObj);
          if (target_object <> nil) and (not target_object.m_boGhost) and (not target_object.m_boDeath) and ((target_object.m_btRaceServer >= RC_ANIMAL) or (target_object.m_btRaceServer = RC_PLAYOBJECT )) then begin
              nLevelBasedChance := target_object.m_Abil.Level - m_Abil.Level;
              if nLevelBasedChance > 10 then
                nLevelBasedChance := 10;
              if nLevelBasedChance < -10 then
                nLevelBasedChance := -10;

              nActualSlowChance := (14 + (skill_level + 1) * 8) - nLevelBasedChance;
              nActualSlowChance := nActualSlowChance - Round((nActualSlowChance / 100) * target_object.m_btSlowResist);

              if target_object.m_btRaceServer <> RC_PLAYOBJECT then
                nCurseChance := 90;

              if Random(nCurseChance) < nActualSlowChance then begin
                if IsProperTarget(target_object) then begin
                  case type_of_attack of
                    0: begin //curse
                      if (Target_object.m_wStatusTimeArr[POISON_SLOWDOWN] = 0) then begin
                        target_object.MakePosion(POISON_SLOWDOWN,duration_of_effect,0);
                        Target_object.MagChangeAbility(power_of_effect, duration_of_effect, 9, nil);
                      end;
                    end;
                    1: begin //plague
                      if (Random(50) > target_object.m_btAntiPoison) and (Random(3) = 0) then target_object.MakePosion(POISON_SLOWDOWN, Round(duration_of_effect div 3), 0);
                      if (Random(50) > target_object.m_btAntiPoison) and (Random(2) = 0) then target_object.MakePosion(POISON_DECHEALTH, Round(duration_of_effect div 3) + random(m_nPoisonAttack), 2);
                      if (Random(50) > target_object.m_btAntiPoison) and (Random(2) = 0) then target_object.MakePosion(POISON_DAMAGEARMOR, Round(duration_of_effect div 3), 0);
                      if (Random(50) > target_object.m_btAntiPoison) and (Random(5) = 0) then target_object.MakePosion(POISON_STONE, _MIN(3, Round(duration_of_effect div 3)), 0);
                    end;
                  end;
                  if target_object.IsProperTarget(self) then
                    target_object.SetTargetCreat(self);
                  if (target_object.m_btRaceServer = RC_PLAYOBJECT) and (m_btRaceServer = RC_PLAYOBJECT) then begin
                    TPlayObject(target_object).SetPKFlag(TPlayObject(self));
                    target_object.SetLastHiter(self);
                  end;
                  Inc(Result);
                end;
              end
...
begin
  ...
  case nType of
    ...
    STATE_ENERGYSHIELD: begin
      m_boEnergyShieldActive := True;
      m_btEnergyShieldLevel := nLevel;
    end;
  end;
...
begin
                     ...
                     case BaseObject.m_btJob of
                       0: begin
                           if BaseObject.m_wStatusArrValue[0] = 0 then
                             BaseObject.m_wStatusArrValue[0] := MakeLong(LoWord(BaseObject.m_WAbil.DC), HiWord(BaseObject.m_WAbil.DC) - 2 - (BaseObject.m_Abil.Level div 7));
                           BaseObject.m_boDC := boState;
                         end;
                       1: begin
                           if BaseObject.m_wStatusArrValue[1] = 0 then
                             BaseObject.m_wStatusArrValue[1] := MakeLong(LoWord(BaseObject.m_WAbil.MC), HiWord(BaseObject.m_WAbil.MC) - 2 - (BaseObject.m_Abil.Level div 7));
                           BaseObject.m_boMC := boState;
                         end;
                       2: begin
                           //if BaseObject.m_wStatusArrValue[2] = 0 then
                             BaseObject.m_wStatusArrValue[2] := 50;
                             BaseObject.m_dwStatusArrTimeOutTick[2]:=GetTickCount + 20 * 1000;
  ...
  BaseObject.SendAbility;
  BaseObject.SendSubAbility;
...
//          if m_boDC then
//            SysMsg('Increase Attack Power for ' + IntToStr(nSec) + ' Seconds', c_Green, t_Hint)
//          else
//            SysMsg('Decrease Attack Power for ' + IntToStr(nSec) + ' Seconds', c_Green, t_Hint);
...
//          if m_boMC then
//            SysMsg('Increase Magic Power for ' + IntToStr(nSec) + ' Seconds', c_Green, t_Hint)
//          else
//            SysMsg('Decrease Magic Power for ' + IntToStr(nSec) + ' Seconds', c_Green, t_Hint);
...
//          if m_boSC then
//            SysMsg('Increase Soul Power for ' + IntToStr(nSec) + ' Seconds', c_Green, t_Hint)
//          else
//            SysMsg('Decrease Soul Power for ' + IntToStr(nSec) + ' Seconds', c_Green, t_Hint);
...
procedure TBaseObject.DamageBubbleDefence(type_of_effect: Byte);
begin
  if m_wStatusTimeArr[type_of_effect] > 0 then begin
    if m_wStatusTimeArr[type_of_effect] > 3 then
      Dec(m_wStatusTimeArr[type_of_effect], 3)
    else
      m_wStatusTimeArr[type_of_effect] := 1;
  end;
end;
  ...
  case m_btJob of
    4: begin
        ...
        m_Abil.MaxMP := _MIN(High(Word), 13 + ROUND(((nLevel /
          g_Config.nLevelValueOfTaosMP) * 6)));

        m_Abil.MaxWeight := 50 + ROUND((nLevel / 3.5) * nLevel);
        m_Abil.MaxWearWeight := _MIN(High(Word), 15 + ROUND((nLevel / 33) * nLevel));
        m_Abil.MaxHandWeight := _MIN(High(Word), 12 + ROUND((nLevel / 45) * nLevel) + Round(nLevel / 7));

        m_Abil.DC := MakeLong(1, _MAX(1, 1 + Round(nLevel / 8)));
        m_Abil.MC := 0;
        m_Abil.SC := 0;
        m_Abil.AC := MakeLong(0, Round(nLevel / 12));
        m_Abil.MAC := MakeLong(0 + Round(nLevel / 6), 1 + Round(nLevel / 4));
      end;
    3: begin
        m_Abil.MaxHP:=_MIN(High(Word),14 + ROUND(((nLevel /
        g_Config.nLevelValueOfAssassinHP + g_Config.nLevelValueOfAssassinHPRate) * nLevel)));

        m_Abil.MaxMP:=_MIN(High(Word),11 + Round(nlevel * g_Config.nLevelValueOfAssassinMP));

        m_Abil.MaxWeight:= 30 + ROUND((nLevel / 3) * nLevel);
        m_Abil.MaxWearWeight:= 15 + ROUND((nLevel / 33) * nLevel);
        m_Abil.MaxHandWeight:= 12 + ROUND((nLevel / 29) * nLevel);

        m_Abil.DC:=MakeLong(1,_MAX(1, 1 + round(nLevel / 8)));
        m_Abil.MC:=0;
        m_Abil.SC:=0;
        m_Abil.AC:=0;
        m_Abil.MAC:=MakeLong(Round(0 + nLevel div 8), 1 + Round(nLevel div 6));
      end;
    2: begin
...
begin
  ...
  try
    ...
    if (nNX >= 0) and ((m_PEnvir.m_nWidth - 1) >= nNX) and
      ...
      bo29 := True;
      //prevents mobs from walking into firewall if bo2ba is set
...
begin
  ...
  if MapItemA <> nil then begin
    ...
    if (sm_Superposition in UserEngine.GetStdItemModeEx(MapItem.UserItem.wIndex)) and (MapItem.UserItem.Dura > 1) then
      SendRefMsg(RM_ITEMSHOW, MapItem.Looks, Integer(MapItem), nX, nY, 'O' + MapItem.Name + '(' + IntToStr(MapItem.UserItem.Dura) + ')')
    else
     SendRefMsg(RM_ITEMSHOW, MapItem.Looks, Integer(MapItem), nX, nY, 'O' + MapItem.Name);
    if m_btRaceServer = RC_PLAYOBJECT then begin
      if boFalg then
        s20 := 'Drop'
      else
  ...
  for i := 0 to nPushCount - 1 do begin
    ...
    if m_PEnvir.CanWalk(nX, nY, False) then begin
      if m_PEnvir.MoveToMovingObject(m_nCurrX, m_nCurrY, Self, nX, nY, False,True) > 0 then begin
        ...
        m_nCurrY := nY;
        GetStartType();
        SendRefMsg(RM_PUSH, nBackDir, m_nCurrX, m_nCurrY, MakeWord(1, m_nLight), '');
        OnAction(AT_Pushed);
        Inc(Result);
        if m_btRaceServer >= RC_ANIMAL then
          m_dwWalkTick := m_dwWalkTick + 800;
...
begin
  ...
  if StdItem <> nil then begin
    ...
    MapItem.Reserved := 0;
    MapItem.Reserved := ItemUnit.HasRandomStat(UserItem);
    ...
    GetDropPosition(m_nCurrX - m_nMoveDropX, m_nCurrY + m_nMoveDropY, nScatterRange, dx, dy);
    pr := m_PEnvir.AddToMap(dx, dy, OS_ITEMOBJECT, TObject(MapItem));
    if pr = MapItem then begin
      if MapItem.Reserved = 0 then
        if (sm_Superposition in UserEngine.GetStdItemModeEx(MapItem.UserItem.wIndex)) and (MapItem.UserItem.Dura > 1) then
          SendRefMsg(RM_ITEMSHOW, MapItem.Looks, Integer(MapItem), dx, dy, 'O' + MapItem.Name + '(' + IntToStr(MapItem.UserItem.Dura) + ')')
        else
          SendRefMsg(RM_ITEMSHOW, MapItem.Looks, Integer(MapItem), dx, dy, 'O' + MapItem.Name)
      else begin
        if (sm_Superposition in UserEngine.GetStdItemModeEx(MapItem.UserItem.wIndex)) and (MapItem.UserItem.Dura > 1) then
          SendRefMsg(RM_ITEMSHOW, MapItem.Looks, Integer(MapItem), dx, dy, 'X' + MapItem.Name + '(' + IntToStr(MapItem.UserItem.Dura) + ')')
        else
          SendRefMsg(RM_ITEMSHOW, MapItem.Looks, Integer(MapItem), dx, dy, 'X' + MapItem.Name);
      end;
      if boDieDrop then
        logcap := 'Drop'
      else
...
begin
  ...
  if m_btRaceServer = 76 then sShowName := ''; //Dont show any name for evil mirs parts
...
var
  ...
  nOldLight, templight: Integer;
  i, k, nIndex: Integer;
  StdItem, Torch: pTStdItem;
  ...
  boMirNeck,boMirSword,boMirBoots,boMirBelt,boMirHelmet,boMirArmour: Boolean;
  nMirBrace,nMirRing: byte;
...
begin
  ...
  if m_btRaceServer = RC_PLAYOBJECT then begin
    if g_SetItemsArr <> nil then
      FillChar(g_SetItemsArr[0], Length(g_SetItemsArr), False);
    boSetItem := False;
    for i := Low(THumanUseItems) to High(THumanUseItems) do begin
      if (m_UseItems[i].wIndex <= 0) then continue;//no item so no stats
      if (i <> U_CHARM) and (i <> U_House) and (m_UseItems[i].Dura <= 0) then continue; //mounts (house) doesnt care about dura, stones have different setup
      StdItem := UserEngine.GetStdItem(m_UseItems[i].wIndex, m_WAbil.Level, m_btJob);
      if StdItem = nil then Continue;
      if (i = U_CHARM) and (StdItem.DuraMax <> 0) and (m_UseItems[i].Dura <= 0) then continue;
      ...
      if (sm_ArmingStrong in StdItem.StdModeEx) or (StdItem.StdMode = tm_Light) then begin
        ...
        case CheckSource of
          ...
          191: m_boNoDuraLoss := true;
          192: boMirNeck  := true;
          193: boMirSword := true;
          194: boMirBoots := true;
          195: boMirBelt  := true;
          196: boMirHelmet := true;
          197: boMirArmour := true;
          198: inc(nMirBrace);
          199: inc(nMirRing);
          200: boMundane1 := true;
          201: boMundane2 := true;
          ...
          207: boSmash2   := true;
          208: boSmash3   := true;
          209: boHwanDevil1 := true;
          210: boHwanDevil2 := true;
          211: boHwanDevil3 := true;
          212: boFiveString1 := true;
          213: boFiveString2 := true;
          214: boFiveString3 := true;
          215: boBone1    := true;
          216: boBone2    := true;
          217: boBone3    := true;
          218: boBug1     := true;
          219: boBug2     := true;
          220: boBug3     := true;
          221: m_nCrystalArmour := StdItem.Holy;
          222: boWhiteGold1 := true;
          223: boWhiteGold2 := true;
          224: boWhiteGold3 := true;
          225: boWhiteGold4 := true;
          226: boWhiteGold5 := true;
          227: boRedJade1   := true;
          228: boRedJade2   := true;
          229: boRedJade3   := true;
          230: boRedJade4   := true;
          231: boRedJade5   := true;
          232: boNephrite1  := true;
          233: boNephrite2  := true;
          234: boNephrite3  := true;
          235: boNephrite4  := true;
          236: boNephrite5  := true;
          237: boWhiteGoldH1 := true;
          238: boWhiteGoldH2 := true;
          239: boWhiteGoldH3 := true;
          240: boWhiteGoldH4 := true;
          241: boWhiteGoldH5 := true;
          242: boRedJadeH1   := true;
          243: boRedJadeH2   := true;
          244: boRedJadeH3   := true;
          245: boRedJadeH4   := true;
          246: boRedJadeH5   := true;
          247: boNephriteH1  := true;
          248: boNephriteH2  := true;
          249: boNephriteH3  := true;
          250: boNephriteH4  := true;
          251: boNephriteH5  := true;
          252: boPurity1     := true;
          253: boPurity2     := true;
          254: boPurity3     := true;
        end;
...
{$IFEND}
  ...
  if nStrengthenCount >= g_Config.Strength10 then m_btStrengthenName := 10
  else if nStrengthenCount >= g_Config.Strength9 then m_btStrengthenName := 9
  else if nStrengthenCount >= g_Config.Strength8 then m_btStrengthenName := 8
  else if nStrengthenCount >= g_Config.Strength7 then m_btStrengthenName := 7
  else if nStrengthenCount >= g_Config.Strength6 then m_btStrengthenName := 6
  else if nStrengthenCount >= g_Config.Strength5 then m_btStrengthenName := 5
  else if nStrengthenCount >= g_Config.Strength4 then m_btStrengthenName := 4
  else if nStrengthenCount >= g_Config.Strength3 then m_btStrengthenName := 3
  else if nStrengthenCount >= g_Config.Strength2 then m_btStrengthenName := 2
  else if nStrengthenCount >= g_Config.Strength1 then m_btStrengthenName := 1
  ...
  if (m_bopirit) then begin
    inc(m_AddAbil.nHitSpeed,2);
    inc(m_AddAbil.DC, 2);
    inc(m_AddAbil.DC2, 5);
  end;
  ...
  if boHwanDevil2 and boHwanDevil3 then begin
    ...
    if boHwanDevil1 then begin
      inc(m_AddAbil.MC);
      inc(m_AddAbil.MC2,2)
    end;
  end;
  if boPurity2 and boPurity3 then begin //Holy +3, Sc 1-2
    inc(m_AddAbil.btHoly, 3);
    if boPurity1 then begin
      inc(m_AddAbil.SC);
      inc(m_AddAbil.SC2,2);
    end;
  end;
  ...
  if boNephriteH1 and boNephriteH2 and boNephriteH3 and boNephriteH4 and boNephriteH5 then begin
    ...
    inc(m_AddAbil.HP,15);
    inc(m_AddAbil.MP,20);
    inc(m_AddAbil.wHitPoint);
    inc(m_AddAbil.btHoly);
  end;
  if (nMirRing = 2) then begin
    inc(m_AddAbil.AC2);
    inc(m_AddAbil.MAC2);
  end;
  if (nMirBrace = 2) then begin
    inc(m_AddAbil.AC);
    inc(m_AddAbil.MAC);
  end;
  if boMirNeck and (nMirRing > 0) and (nMirBrace > 0) then begin
    inc(m_AddAbil.AC2);
    inc(m_AddAbil.MAC2);
    Inc(m_WAbil.MaxWearWeight,17);
    Inc(m_WAbil.MaxWeight,30);
  end;
  if boMirNeck and (nMirRing > 1) and (nMirBrace > 1) then begin
    inc(m_AddAbil.AC2);
    inc(m_AddAbil.MAC2);
    Inc(m_WAbil.MaxWearWeight,10);
    Inc(m_WAbil.MaxWeight,20);
  end;
  if boMirArmour and boMirHelmet and boMirSword then begin
    inc(m_AddAbil.DC2,2);
    inc(m_AddAbil.MC2);
    inc(m_AddAbil.SC2);
    inc(m_AddAbil.wSpeedPoint);
  end;
  if boMirArmour and boMirBelt and boMirBoots then begin
    inc(m_AddAbil.DC2);
    inc(m_AddAbil.MC2);
    inc(m_AddAbil.SC2);
    inc(m_WAbil.MaxHandWeight,17);
  end;
  if boMirArmour and boMirBelt and boMirBoots and boMirHelmet and boMirSword then begin
    inc(m_AddAbil.DC);
    inc(m_AddAbil.DC2);
    inc(m_AddAbil.MC);
    inc(m_AddAbil.MC2);
  ...
  if boMirArmour and boMirBelt and boMirBoots and boMirHelmet and boMirSword and
  (nMirRing = 2) and (nMirBrace = 2) and boMirNeck then begin
    inc(m_AddAbil.AC2);
    inc(m_AddAbil.MAC2);
    inc(m_WAbil.MaxWeight,70);
    inc(m_AddAbil.HP,70);
    inc(m_AddAbil.MP,80);
    inc(m_AddAbil.wAntiMagic,6);
    inc(m_AddAbil.wAntiPoison,6);
    inc(m_AddAbil.btLuck,2);
    inc(m_AddAbil.nHitSpeed,2);
  end;                  
...
//  end;
  ...
  if ishidden() then begin //if we are hiding atm
    if not boOldHideMode then begin //if we where not hiding before recalc
      m_nCharStatus := GetCharStatus();
      StatusChanged();
    end;
  end
  else begin
    if boOldHideMode then begin //if we are not hiding now, but where hiding before
  ...
  nOldLight := m_nLight; //Samuel read torch stats.
   if (m_UseItems[U_RIGHTHAND].wIndex > 0) and (m_UseItems[U_RIGHTHAND].Dura > 0) then begin
    Torch := UserEngine.GetStdItem(m_UseItems[U_RIGHTHAND].wIndex);
    m_nLight := Torch.Effect;
  end else begin
    m_nLight := 0;
  end;

  if (m_UseItems[U_DRESS].wIndex > 0) and (m_UseItems[U_DRESS].Dura > 0) then begin
    Torch := UserEngine.GetStdItem(m_UseItems[U_DRESS].wIndex);
    templight := Torch.Effect;
  end else begin
    templight := 0;
  end;
  if templight > m_nLight then m_nLight := templight;

  if nOldLight <> m_nLight then
    SendRefMsg(RM_CHANGELIGHT, 0, 0, 0, 0, '');

  Inc(m_btAddAttack, m_AddAbil.wAddAttack);
  Inc(m_btDelDamage, m_AddAbil.wDelDamage);
  Inc(m_btAddWuXinAttack, m_AddAbil.wAddWuXinAttack);
  Inc(m_btDelWuXinAttack, m_AddAbil.wDelWuXinAttack);
  ...
  Inc(m_WAbil.MaxWearWeight, m_AddAbil.nWearWeight);
  Inc(m_Wabil.MaxHandWeight, m_AddAbil.nHandWeight);
  inc(m_WAbil.MaxWeight, m_AddAbil.nBagWeight);
  inc(m_nFreezingAttack, m_AddAbil.wFreeze);
  inc(m_nPoisonAttack, m_AddAbil.wPoisonAttack);
//  Inc(m_btHoly, m_AddAbil.btHoly); //no idea why this is the only stat the files dont really use as they should
  m_nHitSpeed := m_AddAbil.nHitSpeed;
  m_nItemExp := m_AddAbil.btExpRate;
  ...
  if boFiveString1 and boFiveString2 and boFiveString3 then
    dec(m_AddAbil.btHPorMPRate,30);
  m_AddAbil.btHPorMPRate := _Max(0,m_AddAbil.btHPorMPRate);
  ...
  if m_wStatusArrValue[6] > 0 then begin
    if m_boAC then begin
      m_WAbil.AC := MakeLong(LoWord(m_WAbil.AC), HiWord(m_WAbil.AC) + 2 + m_wStatusArrValue[6] {n21A})
    end
    else begin
      if LoWord(m_WAbil.AC) > (HiWord(m_WAbil.AC) - m_wStatusArrValue[6]) then
        m_WAbil.AC := MakeLong(LoWord(m_WAbil.AC), LoWord(m_WAbil.AC) {n218})
      else
        m_WAbil.AC := MakeLong(LoWord(m_WAbil.AC), HiWord(m_WAbil.AC) - m_wStatusArrValue[6] {n218})
    end;
  end;

  if m_wStatusArrValue[8] > 0 then begin
    if m_boMC then begin
      m_WAbil.MC := MakeLong(LoWord(m_WAbil.MC) + 2 + m_wStatusArrValue[8], HiWord(m_WAbil.MC)  {n219});
    end
    else begin
      if LoWord(m_WAbil.MC) > (HiWord(m_WAbil.MC) - m_wStatusArrValue[8]) then begin
        m_WAbil.MC := MakeLong(LoWord(m_WAbil.MC), LoWord(m_WAbil.MC) {n218});
      end
      else begin
        m_WAbil.MC := MakeLong(LoWord(m_WAbil.MC), HiWord(m_WAbil.MC) - m_wStatusArrValue[8] {n218});
      end;
    end;
  end;
  //Rage
  if m_wStatusArrValue[10] > 0 then begin
      m_WAbil.DC := MakeLong(LoWord(m_WAbil.DC), HiWord(m_WAbil.DC) + m_wStatusArrValue[10]);
  end;
  //MagicBooster
  if m_wStatusArrValue[11] > 0 then begin
      m_WAbil.MC := MakeLong(LoWord(m_WAbil.MC) + m_wStatusArrValue[11], HiWord(m_WAbil.MC) + m_wStatusArrValue[11])
  end;
  //UltimateEnhancer
  if m_wStatusArrValue[12] > 0 then begin
    case m_btJob of
      0 : m_WAbil.DC := MakeLong(LoWord(m_WAbil.DC), HiWord(m_WAbil.DC) + m_wStatusArrValue[12] {n218});
      1 : m_WAbil.MC := MakeLong(LoWord(m_WAbil.MC), HiWord(m_WAbil.MC) + m_wStatusArrValue[12] {n218});
      2 : m_WAbil.SC := MakeLong(LoWord(m_WAbil.SC), HiWord(m_WAbil.SC) + m_wStatusArrValue[12] {n218});
    end;
  end;

  //ProField
  if m_wStatusArrValue[13] > 0 then begin
      m_WAbil.AC := MakeLong(LoWord(m_WAbil.AC), HiWord(m_WAbil.AC) + m_wStatusArrValue[13]);
  end;

  //Haste
  if m_wStatusArrValue[14] > 0 then begin
      m_nHitSpeed := m_nHitSpeed + m_wStatusArrValue[14];
  end;

  //LightBody
  if m_wStatusArrValue[15] > 0 then begin
      m_btSpeedPoint := m_btSpeedPoint + m_wStatusArrValue[15];
  end;

  //Curse
  if m_wStatusArrValue[9] > 0 then begin
    if HiWord(m_WAbil.DC) > Trunc((HiWord(m_WAbil.DC)/100) * m_wStatusArrValue[9]) then
      m_WAbil.DC  := MakeLong(LoWord(m_WAbil.DC),HiWord(m_WAbil.DC) - Trunc((HiWord(m_WAbil.DC)/100) * m_wStatusArrValue[9]))
    else
      m_WAbil.DC :=MakeLong(LoWord(m_WAbil.DC),0);

    if HiWord(m_WAbil.MC) > Trunc((HiWord(m_WAbil.MC)/100) * m_wStatusArrValue[9]) then
      m_WAbil.MC  := MakeLong(LoWord(m_WAbil.MC),HiWord(m_WAbil.MC) - Trunc((HiWord(m_WAbil.MC)/100) * m_wStatusArrValue[9]))
    else
      m_WAbil.MC :=MakeLong(LoWord(m_WAbil.MC),0);

    if HiWord(m_WAbil.SC) > Trunc((HiWord(m_WAbil.SC)/100) * m_wStatusArrValue[9]) then
      m_WAbil.SC  := MakeLong(LoWord(m_WAbil.SC),HiWord(m_WAbil.SC) - Trunc((HiWord(m_WAbil.SC)/100) * m_wStatusArrValue[9]))
    else
      m_WAbil.SC :=MakeLong(LoWord(m_WAbil.SC),0);
  end;
...
begin
  ...
  try
    ...
    if (m_btRaceServer = RC_PLAYOBJECT) then begin
      if GetTickCount() - m_dwRunTick > m_nRunTime then begin
    ...
    if not m_boDeath then begin
      nCheckCode := 2003;
      {$region 'code causing you to die or get revived'}
      ...
      {$region 'code letting you do slowly regen hp/mp (not healing or pot related)'}
      if not (m_wStatusTimeArr[POISON_DECHEALTH] > 0) then begin //ripman lie and mana regen
        if (m_WAbil.HP < m_WAbil.MaxHP) and (m_nHealthTick >= g_Config.nHealthFillTime) then begin
          n18 := (m_WAbil.MaxHP div _max(1,(75 - m_nHealthRecover))) + 1;
        ...
        if (m_WAbil.MP < m_WAbil.MaxMP) and (m_nSpellTick >= g_Config.nSpellFillTime) then begin
          n18 := (m_WAbil.MaxMP div _max(1,(18 - m_nSpellRecover))) + 1;
      ...
      if m_nSpellTick >= g_Config.nSpellFillTime then
        m_nSpellTick := 0;
      nCheckCode := 1009; //ripman till here life and mana regen
      end;
      {$endregion}
    end
    else begin
    {$region 'code making your corpse invisible to others'}
      nCheckCode := 1010;
      if m_btRaceServer = RC_PLAYOBJECT then begin
  ...
  {$region 'player only: hp/mp regen from equiped stone'}
  try
    boSendChange := False;
  ...
  {$endregion}
  {$region 'potion or healing hp/mp regen'}
  try
    if not m_boDeath and ((m_nIncSpell > 0) or (m_nIncHealth > 0) or (m_nIncHealing > 0)) then begin
      ...
      if ((GetTickCount - m_dwIncHealthSpellTick) >= dwInChsTime) and not m_boDeath then begin
        ...
        dwC := 0;//above line is there to compensate for a server lagging behind (but causes negative values)
    ...
    //check if hp or mp went over max and make sure that's not possible :p
    boNeedRecalc := False;
    if m_WAbil.HP > m_WAbil.MaxHP then begin
      boNeedRecalc := True;
  ...
  try
    {$region 'code to check if your different targets, hitter, other objects still exicst/are near you'}
    if (m_TargetCret <> nil) then begin
      ...
      (abs(m_TargetCret.m_nCurrX - m_nCurrX) > 15) or
        (abs(m_TargetCret.m_nCurrY - m_nCurrY) > 15) or
        (self.m_Master = m_TargetCret) then begin  //fix for poisoned pets attack masters.
        m_TargetCret := nil;
      end;
    end;
    if (m_LastHiter <> nil) then begin
    ...
    {$endregion}
    {$region 'code for the pet wagon thing'}
    if m_Master <> nil then begin
      m_boNoItem := True;
      //宝宝变色
    ...
    nCheckCode := 3013;
    if m_boHallucinationMode and ((GetTickCount() - m_dwHallucinationModeTick) > m_dwHallucincationModeInterval) then begin
      BreakHallucinationMode();
    end;
  ...
  try
    ...
    //warning for pets untaming
    if (GetTickCount - warning_for_pet_untame_tick) > 60000 then begin
      if m_Master <> nil then begin
        if (m_dwMasterRoyaltyTick < GetTickCount + g_Config.slave_untaming_warning_start) then begin
          untame_warning_time := Round((m_dwMasterRoyaltyTick - GetTickCount) / 60000);
          if untame_warning_time < 1 then
            m_Master.SysHintMsg('Your pet ' + m_sCharName + ' is going to untame in less than a minute.', c_Red)
          else
            m_Master.SysHintMsg('Your pet ' + m_sCharName + ' is going to untame in less than ' + inttostr(untame_warning_time + 1) + ' minute(s).', c_Red);
        end;
      end;
      warning_for_pet_untame_tick := GetTickCount();
    end;

    //chance of a quick retame
    if (chance_to_retame) then begin
      if (GetTickCount - chance_to_retame_tick) > 10000 then begin
        Inc(chances_to_retame_count, 1);
        if chances_to_retame_count >= 3 then begin
          chance_to_retame := False;
          chances_to_retame_count := 0;
          previous_slave_owner :=  nil;
        end;
    ...
    if (GetTickCount - m_dwCheckRoyaltyTick) > 10000 then begin
      ...
      if m_Master <> nil then begin
        ...
        if (GetTickCount > m_dwMasterRoyaltyTick) then begin
          ...
          chance_to_retame := true;
          previous_slave_owner := m_Master;
          m_Master := nil;
        ...
        if m_dwMasterTick <> 0 then begin
          if (GetTickCount - m_dwMasterTick) > 24 * 60 * 60 * 1000 then begin
            m_WAbil.HP := 0;
          end;
        end;
      end;
    ...
    for i := Low(m_dwStatusArrTick) to High(m_dwStatusArrTick) do begin //004C832E
      if (m_wStatusTimeArr[i] > 0) and (m_wStatusTimeArr[i] < 60000) then begin
        if (GetTickCount() - m_dwStatusArrTick[i]) > 1000 then begin
          ...
          if i in [POISON_DECHEALTH,POISON_DAMAGEARMOR,POISON_STONE,POISON_COBWEB,POISON_SLOWDOWN,POISON_FREEZE,POISON_STUN] then
            Dec(m_dwStatusArrTick[i], m_nPoisonRecover * 10);
          if (m_wStatusTimeArr[i] = 0) then begin
            boChg := True;
            case i of
              STATE_TRANSPARENT: begin
                  ...
                  if ishidden() then
                     m_wStatusTimeArr[STATE_TRANSPARENT {8 0x70}] := 6 * 10 * 1000;
                  ChangeStatusMode(STATUS_HIDEMODE, ishidden());
                end;
              ...
              POISON_SLOWDOWN: ChangeStatusMode(STATUS_SLOWDOWN, False);
              POISON_FREEZE: ChangeStatusMode(STATUS_FREEZE, False);
              POISON_STUN: ChangeStatusMode(STATUS_STUN, False);
            end;
          end;
        end;
      end;
      ...
      if m_wStatusArrValue {218} [i] > 0 then begin
        if GetTickCount() > m_dwStatusArrTimeOutTick {220} [i] then begin
          ...
          case i of
            ...
            6: begin
                SysMsg('Physical armour returned to normal', c_Green, t_Hint);
                ChangeStatusMode(STATUS_AC, False);
            end;
            8: begin
                ChangeStatusMode(STATUS_MC,False);
            ...
            9: begin
              SysMsg('Temporarily decreased abilities have now returned to normal.', c_Green, t_Hint);
            end;
            //Rage
            10: begin
              SysMsg('Attack Power returned to normal.', c_Green, t_Hint);
              ChangeStatusMode(STATUS_RAGE, False);
            end;
            //MagicBooster
            11: begin
              SysMsg('Magic Power returned to normal.', c_Green, t_Hint);
              ChangeStatusMode(STATUS_MAGICBOOSTER, False);
            end;
            //UltimateEnhancer
            12: begin
              case m_btJob of
              0 : SysMsg('Attack Power returned to normal.', c_Green, t_Hint);
              1 : SysMsg('Magic Power returned to normal.', c_Green, t_Hint);
              2 : SysMsg('Soul Power returned to normal.', c_Green, t_Hint);
              end;
              ChangeStatusMode(STATUS_UE, False);
            end;
            //ProField
            13: begin
              SysMsg('Physical armour returned to normal.', c_Green, t_Hint);
              ChangeStatusMode(STATUS_PROFIELD, False);
            end;
            //Haste
            14: begin
              SysMsg('Attack Speed returned to normal.', c_Green, t_Hint);
              ChangeStatusMode(STATUS_HASTE, False);
            end;
            //LightBody
            15: begin
              SysMsg('Agility has returned to normal.', c_Green, t_Hint);
              ChangeStatusMode(STATUS_LIGHTBODY, False);
            end;
          end;
        end;
      end;
 ...
 { try
  ...
  try     //ripman
    if (GetTickCount - m_dwPoisoningTick) > g_Config.dwPosionDecHealthTime {2500} then begin
      m_dwPoisoningTick := GetTickCount();
      if (m_wStatusTimeArr[POISON_DECHEALTH] > 0) then begin
        if (m_PoisonTarget <> nil) and (m_PoisonTarget is TPlayObject) and (self is TPlayObject) then begin
          if m_PoisonTarget.m_boGhost and (m_PoisonTarget is TPlayObject) and (self is TPlayObject) then begin
            m_wStatusTimeArr[POISON_DECHEALTH]:=_Min(1,m_wStatusTimeArr[POISON_DECHEALTH]);
          end;
            TPlayObject(self).SetPKFlag(TPlayObject(m_PoisonTarget));
            m_LastHiter := m_PoisonTarget;
        end;
      end else if m_PoisonTarget <> nil then m_PoisonTarget:=nil;
      if (m_PoisonTarget <> nil) and m_PoisonTarget.m_boGhost then m_PoisonTarget := nil;
      if m_wStatusTimeArr[POISON_DECHEALTH] > 0 then begin
        if m_boAnimal then Dec(m_nMeatQuality, 1000);
        if (m_WAbil.HP > (m_Abil.MaxHP / 100)) and ((m_btGreenPoisoningPoint + 1) < m_WAbil.HP) then begin   // Green poison does not recuce HP below 1%
          DamageHealth(m_btGreenPoisoningPoint + 1);
          if m_btPoisonDefense = 0 then begin
            m_nHealthTick := 0;
            m_nSpellTick := 0;
          end;
          HealthSpellChanged();
...
{$IF CHANGEMAPMODE = OLDMAPMODE}
      SendMsg(Self, RM_CHANGEMAP, 0, Integer(Envir), 0, 0, g_MapManager.GetMainMap(Envir));
      if nInt = 1 then SendRefMsg(RM_SPACEMOVE_SHOW2, MakeWord(m_btDirection, m_nLight), m_nCurrX, m_nCurrY, 0, '')
      else if nInt >= 2 then SendRefMsg(RM_SPACEMOVE_SHOW3, MakeWord(m_btDirection, m_nLight), m_nCurrX, m_nCurrY, nInt, '')
      else SendRefMsg(RM_SPACEMOVE_SHOW, MakeWord(m_btDirection, m_nLight), m_nCurrX, m_nCurrY, 0, '');
{$ELSE}
      if m_boOldChangeMapMode then SendMsg(Self, RM_CHANGEMAP, 0, Integer(Envir), 0, 0, g_MapManager.GetMainMap(Envir))
      else SendMsg(Self, RM_CHANGEMAP, 0, Integer(OldEnvir), 0, 0, g_MapManager.GetMainMap(Envir));
    ...
    case ProcessMsg.wIdent of
      RM_MAGSTRUCK,
        RM_MAGSTRUCK_MINE: begin //10025
          if (ProcessMsg.wIdent = RM_MAGSTRUCK) and (m_btRaceServer >= RC_ANIMAL) and (not boCantGetStun) {and (m_Abil.Level < 35)} then
          begin
            m_dwWalkTick := m_dwWalkTick + 600 + LongWord(Random(1000));
          end;
      ...
      RM_CHANGELIGHT: begin
        TPlayObject(self).SendDefMessage(SM_CHANGELIGHT, Integer(ProcessMsg.BaseObject), TBaseObject(ProcessMsg.BaseObject).m_nLight, 0, 0, '');
      end;
      RM_LAMPCHANGEDURA: begin //10138 004D9F0B
        TPlayObject(self).SendDefMessage(SM_LAMPCHANGEDURA,
          ProcessMsg.nParam1,
          0,
          0,
          0,
      ...
      RM_CLIENTDATAFILE: begin
        m_DefMsg := MakeDefaultMsg(SM_CLIENTDATAFILE, ProcessMsg.wParam,ProcessMsg.nParam1,ProcessMsg.nParam2,ProcessMsg.nParam3);
        TPlayObject(self).SendSocket(@m_DefMsg,ProcessMsg.sMsg);
      end;
          ...
          if MakePoisonInfo <> nil then begin
            ...
            if (TargeTBaseObject <> nil) and (abs(nTargetX - m_nCurrX) <= nRage) and (abs(nTargetY - m_nCurrY) <= nRage) then begin
              ...
              if (m_Master <> nil) and (TargeTBaseObject <> m_Master) and
                (TargeTBaseObject.m_btRaceServer = RC_PLAYOBJECT) and (m_Master.m_btRaceServer = RC_PLAYOBJECT) then begin
...
begin
  ...
  if MonObj <> nil then begin
    ...
    MonObj.m_dwWalkTick := GetTickCount + 2000;
...
begin
  if Magic <> nil then begin
    if Magic.MagicInfo.wMagicId = SKILL_REVELATION then begin
      if Magic.btLevel >= 2 then
        m_boAbilSeeHealGauge := True;
    end else
    if Magic.MagicInfo.wMagicID in [SKILL_FENCING, SKILL_SPIRITSWORD, SKILL_SLAYING, SKILL_FATALSWORD] then begin
      RecalcAbilitys();
      SendAbility;
      SendSubAbility;
    end;
...
begin
  if AClientVersion <= 0 then
    exit;
  m_nClientVersion := AClientVersion;
end;
...
function TBaseObject.CanFollowObject(FollowObj: TBaseObject): Boolean;
begin
  Result := False;
  if (FollowObj <> nil) and (m_Master = nil) and
  (not FollowObj.m_boDeath) or
  (not FollowObj.m_boGhost) then begin
    if m_TargetCret = nil then Result := True;
    if (integer(GetTickCount - m_dwFollowActivity) > 60000 * 2) then begin
      if FollowObj.m_TargetCret = nil then begin
        FollowObject := nil;
        Result := False;
        exit;
      end;
    ...
    if (abs(m_nCurrX - FollowObject.m_nCurrX) >= m_nFollowDistance) or
    (abs(m_nCurrY - FollowObject.m_nCurrY) >= m_nFollowDistance) then begin
      FollowObject := nil;
      Result := False;
      exit;
    end;
  end else FollowObject := nil;
end;

procedure TBaseObject.GetMapFollowObject(range:integer);
var
Actors: TList;
BaseObject: TBaseObject;
CallObject: TAnimalObject;
I: Integer;
begin
  if m_Master <> nil then exit;
  Actors := TList.Create;
  GetMapBaseObjects(m_PEnvir, m_nCurrX, m_nCurrY, range, Actors);
  if Actors.Count > 0 then begin
    for i:= Actors.Count-1 downto 0 do begin
    BaseObject := TBaseObject(Actors.Items[I]);
      if not (BaseObject.m_btRaceServer in [RC_PLAYOBJECT, RC_GUARD, RC_NPC, RC_ARCHERGUARD]) and
      (BaseObject <> Self) and
      (BaseObject.m_Master = nil) and
      (BaseObject.m_TargetCret = nil) and
      (BaseObject.FollowObject = nil) then begin
        if BaseObject <> nil then begin
          CallObject := TAnimalObject(BaseObject);
          CallObject.FollowObject := Self;
          CallObject.m_nFollowDistance := range + 3;
          CallObject.m_dwFollowActivity := GetTickCount();
        end;
        Actors.Delete(i);
      end;
    end;
...
begin
  ...
  for i := 0 to BaseObjectList.Count - 1 do begin
    ...
    if IsProperTarget(BaseObject) then begin
      nDamage := 0;
      nHitPower := BaseObject.GetStunDamage(Self, nHitPower);
...
begin
  ...
  if ProcessMsg.wIdent = RM_STRUCK then begin
    if (ProcessMsg.BaseObject = Self) and (TBaseObject(ProcessMsg.nParam3 {AttackBaseObject}) <> nil) then begin
      ...
      BreakHolySeizeMode();
      if (m_Master <> nil) and (m_Master.m_btRaceServer = RC_PLAYOBJECT) and
