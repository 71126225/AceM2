...
// Create D3D Device
  ...
  //should really figure out why this breaks sysutils
  if (Failed(FD3D.CreateDevice(D3DADAPTER_DEFAULT,D3DDEVTYPE_HAL,FWndParent, D3DCREATE_SOFTWARE_VERTEXPROCESSING,FD3DPP^,FD3DDevice))) then begin
    PostError('Can''t create D3D device');
    Exit;
  end;
...
//    Exit;
  ...
  GfxUpdateParameters;
  FD3DDevice.Reset(FD3DPP^); //this code is breaking alot but no idea why, fix below is there i guess
  asm
    finit;
  end;
...
begin
  Gfx_RenderQuadrangle4Color(X, Y, X + Width, Y, X + Width, Y + Height, X, Y + Height, Color, Color, Color, Color, Filled, BlendMode);
end;
